...
for my $key (@CATEGORIES) {
	...
	system("perl $send_event --parm 'url http://$host:$port/submit' --parm 'user $ENV{USER}' --parm 'passwd $ENV{USER}' --parm 'catlabel $key' http://uei.opennms.org/products/bluebird/internal/rtc/subscribe $server");
}
...
sub do_exit {
	for my $key (@CATEGORIES) {
		...
		system(" perl  $send_event --parm 'url http://$host:$port/submit' --parm 'user $ENV{USER}' --parm 'passwd $ENV{USER}' --parm 'catlabel $key' http://uei.opennms.org/products/bluebird/internal/rtc/unsubscribe $server");
	}
	Gtk->exit(0);
}
...
sub handle_connection {
	...
	for my $line (split(/\r?\n/, $text)) {
		if ($line =~ /<catlabel>(.*)<\/catlabel>/) {
			$label = decode_entities($1);
		} elsif ($line =~ /<catvalue>(.*)<\/catvalue>/) {
			$value = decode_entities($1);
		} elsif ($line =~ /<nodesvccount>(\d+)<\/nodesvccount>/) {
			$services += $1;
		} elsif ($line =~ /<nodesvcdowncount>(\d+)<\/nodesvcdowncount>/) {
			$outages += $1;
		}
	}
