...
class cci:
    ...
    def get_Workflow_yaml(self,workflow,leadingSpace=0,enable_filters=True,raw_output=True)->str:
        if raw_output:
            return self._workflow.get_workflow_yaml(workflow,leadingSpace,enable_filters)
        else:
            return "\n".join(self._workflow.get_workflow_yaml(workflow,leadingSpace,enable_filters))
