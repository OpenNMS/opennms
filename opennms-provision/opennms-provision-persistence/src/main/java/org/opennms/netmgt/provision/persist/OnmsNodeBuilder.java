...
public class OnmsNodeBuilder  {
    ...
    public OnmsNode buildNode(RequisitionNodeEntity requisitionNode) {
        ...
        nodeBldr.setForeignSource(requisitionNode.getForeignSource());
        nodeBldr.setForeignId(requisitionNode.getForeignId());
        nodeBldr.setLocation(requisitionNode.getLocation());
        nodeBldr.getAssetRecord().setBuilding(requisitionNode.getBuilding());
        nodeBldr.getAssetRecord().getGeolocation().setCity(requisitionNode.getCity());
        ...
        for(final RequisitionInterfaceEntity ifaceReq : requisitionNode.getInterfaces()) {
            ...
            final NetworkBuilder.InterfaceBuilder ifBldr = bldr.addInterface(ipAddr);
        ...
        for(final Map.Entry<String, String> eachAsset : requisitionNode.getAssets().entrySet()) {
            bldr.setAssetAttribute(eachAsset.getKey(), eachAsset.getValue());
        }
