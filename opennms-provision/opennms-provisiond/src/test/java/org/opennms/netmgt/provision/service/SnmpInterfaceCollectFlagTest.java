...
public class SnmpInterfaceCollectFlagTest implements MockSnmpAgentAware {
    ...
    public void testPopulateWithSnmpAndNodeScan() throws Exception {
        ...
        List<OnmsSnmpInterface> snmpIfaces = getSnmpInterfaceDao().findAll();
        assertEquals(1, snmpIfaces.size());
        for(OnmsSnmpInterface snmpIface : snmpIfaces) {
            System.err.println(snmpIface);
        }
        for(OnmsIpInterface iface : getInterfaceDao().findAll()) {
            System.err.println(iface);
            assertEquals("C", iface.getSnmpInterface().getCollect());
        }
        ...
        snmpIfaces = getSnmpInterfaceDao().findAll();
        assertEquals(6, snmpIfaces.size());
        for(OnmsSnmpInterface snmpIface : snmpIfaces) {
            System.err.println(snmpIface);
            //assertEquals("Unexpected collect state for snmpIface"+snmpIface, "C", snmpIface.getCollect());
        }
