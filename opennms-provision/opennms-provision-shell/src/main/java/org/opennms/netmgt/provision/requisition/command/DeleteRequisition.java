...
public class DeleteRequisition implements Action {
    ...
    @Option(name = "-e", aliases = "--empty", description = "Empty the requisition of all nodes / delete all nodes from the requisition.  Send a 'uei.opennms.org/internal/importer/reloadImport' event or use 'opennms:synchronize-requisition' to then synchronize the requisition and delete all the nodes.")
    boolean Empty = false;
    ...
    public Object execute() {
        try {
            ...
            if (req == null) {
                System.out.println("Requisition '" + requisitionName + "' does not exist");
                return null;
            }
            if (Empty && yesreally) {
                int nodeCount = 0;
                ...
                System.out.println("Requisition " + requisitionName + " is not empty.  Empty it with '--empty', and optionally send an appropriate 'uei.opennms.org/internal/importer/reloadImport' event or use 'opennms:synchronize-requisition' to synchronize it to the database.");
            }
            else {
