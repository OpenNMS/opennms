...
NOTE : 29.0.5 consolidated IPC features into one feature on Minion. This groups all IPC features into either `openms-core-ipc-jms` or `opennms-core-ipc-kafka`.
When using kafka, you need to disable `opennms-core-ipc-jms` instead of disabling individual features like `opennms-core-ipc-sink-camel`.
