...
=== Additional config needed when using Kafka/Grpc for Minion Communication.

Minion no longer uses OpenNMS Rest API to get Trapd Config updates. It uses Twin mechanism to sync with Trapd config.
Minion needs additional config both on Minion and OpenNMS to enable Twin. Refer to deployment section from the docs.
