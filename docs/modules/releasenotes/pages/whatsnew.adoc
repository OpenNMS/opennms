...
To preserve local changes in `/etc/opennms`, copy that folder to `/opt/opennms/etc` before upgrading, and preserve file ownership and permissions. 

Prepare `/opt/opennms/etc` with the following commands:

```
sudo install -d -g opennms -o opennms /opt/opennms/etc
