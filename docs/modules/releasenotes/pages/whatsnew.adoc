...
* writing OpenNMS configuration through the REST API (`POST`/`PUT`/`DELETE` on `/rest/config/**`, including the alarm northbounder configurations) now requires `ROLE_ADMIN`; previously an account with only `ROLE_REST` could write these. Update any integration that used a `ROLE_REST` account to change configuration to use a `ROLE_ADMIN` account. Reading configuration and all other `ROLE_REST` data operations are unchanged.
