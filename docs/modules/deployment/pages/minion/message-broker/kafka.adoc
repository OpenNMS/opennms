...
[source, console]
----
sudo vi /opt/minion/etc/featuresBoot.d/disable-activemq.boot
----
...
[source, console]
----
sudo vi /opt/minion/etc/featuresBoot.d/kafka.boot
----
...
<1> Adjust the IPs or FQDNs and Kafka port (9092) to match your broker configuration.
...
If you chose to use a different prefix in core (for example, to use a single Apache Kafka cluster with multiple {page-component-title} instances), change the topic prefix by setting the `org.opennms.instance.id` system property in `/opt/minion/etc/custom.system.properties`:
[source, properties]
----
