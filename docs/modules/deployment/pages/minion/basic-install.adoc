...
* Create credentials to secure the communication between the Minion and the Core instance
* Configuring {page-component-title} Core instance with embedded ActiveMQ message broker for {page-component-title} Minion communication
* Verfying the setup

== Requirements

* Linux physical server or a virtual machine running a supported Linux operating system
* Internet access to download the RPM or Debian packages
...
* Identical version numbers for {page-component-title} Core instance and {page-component-title} Minion 
* {page-component-title} Core instance is installed and communication to the REST (8980/tcp) and ActiveMQ (616161/tcp) endpoints are possible

Configuring a Minion requires the following information and you have to adapt them accordingly:

* URL for your OpenNMS {page-component-title} Core instance.
  To have an example we use `\http://core-instance-ip:8980/opennms` here.
* ActiveMQ endpoint and if it's SSL or plain text TCP, this example uses `TCP` without SSL using the default ActiveMQ port `61616`.
  The Core instance network endpoint is indicated with `core-instance-ip`.
* Credentials for the Core instance and Minion communication.
  In this guide we use `my-minion-user` and `my-minion-password`.

#TODO: Add hint to run all from a root shell#
