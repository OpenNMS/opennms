...
JMX must be enabled on the Cassandra nodes and made accessible from {page-component-title} in order to collect these metrics (see https://cassandra.apache.org/doc/4.0/cassandra/operating/security.html#jmx-access[JMX access] in the Cassandra documentation).
