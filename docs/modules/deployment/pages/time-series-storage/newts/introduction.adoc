...
It enables link:https://en.wikipedia.org/wiki/Scalability#Horizontal[horizontal scaling] for your time series database and is an alternative to link:http://oss.oetiker.ch/rrdtool/[RRDtool].

[IMPORTANT]
====
Newts and its Apache Cassandra backend should be considered deprecated.
...
For new deployments, and when planning an upgrade path for an existing one, we suggest looking at the xref:time-series-storage/timeseries/prometheus-remotewrite.adoc[Prometheus RemoteWrite plugin], which stores metrics in any Prometheus `remote_write`-compatible time series database.
