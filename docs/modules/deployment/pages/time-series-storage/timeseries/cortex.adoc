...
+
NOTE: If you are deploying to a container, instead of setting the `org.opennms.timeseries.strategy` via a properties file, update the environment variables of your container config:
+
[source, yaml]
    environment:
