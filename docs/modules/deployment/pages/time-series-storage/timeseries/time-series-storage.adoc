...
In order for the dual write plugin to work, you must install a time series feature, either Newts or an integration plugin, and leave the `org.opennms.timeseries.strategy` value as `rrd`.
