...
Must be a power of two. (It will override by org.opennms.timeseries.config.buffer_size, unless it is -1.)
| 8192
...
| Type of buffer. (RINGBUFFER / OFFHEAP) +
RINGBUFFER is a high performance queue. It will only keep data in memory. if the queue overflow, it will drop data. +
OFFHEAP is a queue that will store data in memory. However, when queue overflow, it will persist extra data into disk.
| RINGBUFFER

| org.opennms.timeseries.config.offheap.batch_size (OFFHEAP)
| Data block size for OFFHEAP queue.
| 8192

| org.opennms.timeseries.config.offheap.path (OFFHEAP)
| Location for storing offheap data. Please make sure it is a high performance disk for high loading system.
| /tmp

| org.opennms.timeseries.config.offheap.max_file_size (OFFHEAP)
