...
+
Expected outcome: `apt-cache` does not show an error pertaining to the {page-component-title} repo; `apt show {page-component-title}-core` shows an available current package version.

. Install the PostgreSQL 12 version of iplike: `apt -y install iplike-pgsql12`
+
...
. Upgrade the {page-component-title} packages to the new version: `apt -y upgrade {page-component-title}-core {page-component-title}-webapp-jetty`
+
Expected result: A lot of rpmnew files; `rpm -q {page-component-title}-core` reports the new version is installed.

. Install Java 11: `apt -y install java-11-openjdk java-11-openjdk-devel`
