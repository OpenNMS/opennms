...
Download and install OpenJDK packages (OpenJDK 21 here but please refer to system requirements):

[source,console]
----
dnf install --downloadonly --downloaddir=/tmp/opennms_rpms java-21-openjdk-devel
...
If you need additional OpenNMS plugins (e.g., for Asset Inventory, etc), download these packages from the OpenNMS repositories and include them in your installation files.
...
After completing the package installation, proceed with OpenNMS configuration as described in our xref:core/install.adoc[Set up the core instance] documentation.
