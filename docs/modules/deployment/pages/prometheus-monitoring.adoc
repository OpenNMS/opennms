...
TIP: Replace `$\{MINION_HOME}` with `/opt/minion` on RPM and `/opt/minion` on Debian-based Linux distributions.
...
TIP: Replace `$\{SENTINEL_HOME}` with `/opt/sentinel` on RPM and `/opt/sentinel` on Debian-based Linux distributions.
