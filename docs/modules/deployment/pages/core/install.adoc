...
{page-component-title} {page-component-version} requires Java {compatible-javajdk}.

Many supported distributions ship an OpenJDK {compatible-javajdk} build in their own repositories.
Refer to your operating system's package manager to search and find a compatible version (for example, `java-{compatible-javajdk}-openjdk-devel` on RHEL-based systems, or `openjdk-{compatible-javajdk}-jdk` on Debian and Ubuntu).

If your distribution does not provide OpenJDK {compatible-javajdk}, or you would rather use a vendor build, Eclipse Temurin from Adoptium is an option that we can recommend.
See the https://adoptium.net/installation[Adoptium installation documentation] for packages and instructions for your platform.
