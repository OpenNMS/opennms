.FIPS mode
If your environment requires FIPS mode enabled, you must enable the `PKCS12` keystore type for the xref:operation:deep-dive/admin/configuration/scv-configuration.adoc[Secure Credentials Vault] and xref:operation:deep-dive/admin/configuration/scv-configuration.adoc#configure-keystore-type-and-file-name[convert the existing keystore] to `PKCS12`, or create a fresh keystore.
[source, console]
----
echo "org.opennms.features.scv.keystore.type=PKCS12" | sudo tee -a /opt/opennms/etc/opennms.properties.d/scv.properties
----
...
.Detect and assign Java environment and persist in $\{OPENNMS_HOME}/etc/java.conf
[source, console]
----
sudo ${OPENNMS_HOME}/bin/runjava -s
----

.Initialize the database and detect system libraries persisted in $\{OPENNMS_HOME}/etc/libraries.properties
[source, console]
----
