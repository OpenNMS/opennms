.FIPS mode
If your environment requires FIPS mode enabled, you must enable the `PKCS12` keystore type for the xref:operation:deep-dive/admin/configuration/scv-configuration.adoc[Secure Credentials Vault] prior to creating the keystore:
[source, console]
----
