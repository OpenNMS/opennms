...
CAUTION: Don't use our Docker examples with high workloads with critical data in production.
         The examples can be used as a starting point to get things up and running in a test environment.
         Configuration of the services are not tuned for production workloads so they can run also on small hardware specifications.
endif::[]
