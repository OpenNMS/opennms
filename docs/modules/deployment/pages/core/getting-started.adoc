...
[source, console]
----
sudo -u opennms /path/to/opennms/bin/scvcli set postgres opennms opennms
sudo -u opennms /path/to/opennms/bin/scvcli set postgres-admin postgres opennms
----
...
image::deployment/core/data-sources.png[Data sources collection permission prompt displaying opt-in and opt-out buttons,400]
