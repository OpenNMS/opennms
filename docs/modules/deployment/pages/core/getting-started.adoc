...
The following components and settings are required for a new {page-component-title} instance:
...
IMPORTANT: On Debian, you must install and configure `sudo` yourself.
...
The first time you sign in, {page-component-title} prompts for permission to allow the Data Choices module to collect and publish anonymous usage statistics to https://stats.opennms.org:
...
NOTE: Statistics collection and publication happen only if an admin user opts in.
Admin users can xref:operation:deep-dive/admin/configuration/data-collection.adoc#disable-data-collection[enable or disable statistics collection] at any time.
...
Your account's password is updated.
