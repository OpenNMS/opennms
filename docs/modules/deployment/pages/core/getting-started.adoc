...
Click *Show Me What is Being Sent* to see xref:operation:deep-dive/admin/housekeeping.adoc#ga-data-collection[what information we collect].
Statistics collection and publication happen only if an admin user opts in.

NOTE: Admin users can xref:operation:deep-dive/admin/housekeeping.adoc#disable-data-collection[enable or disable statistics collection] at any time.
