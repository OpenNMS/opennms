...
* If in usage: A SELinux policy that permits binding to the ICMP service.
+
Note that failure to include the appropriate SELinux policy may generate an ICMP permission error when starting OpenNMS (see xref:reference:configuration/selinux.adoc[SElinux and ICMP permissions]).
ifeval::["{page-component-title}" == "Horizon"]
...
* xref:reference:configuration/selinux.adoc[SElinux and ICMP permissions]
* xref:reference:configuration/receive-snmp-traps.adoc[Receive SNMP traps]
* xref:reference:configuration/install-jdbc-driver.adoc[Install JDBC driver]
