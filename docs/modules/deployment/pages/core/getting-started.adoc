...
As noted in the requirements above, you must have SELinux to run {page-component-title}, as servers that have a SELinux policy configured will deny permission for {page-component-title} to bind to the ICMP service.
...
If the command is missing, run the `dnf install policycoreutils-devel` package.
