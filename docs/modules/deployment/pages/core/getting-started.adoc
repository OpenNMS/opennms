...
IPLIKE is a group of SQL statements (stored procedure) that provides more flexible searching of IP addresses.
It uses a flexible search format that allows you to search IP addresses by any of the octets of a TCP/IP address.
An asterisk (*) in place of any octet matches any value for that octet.
Indicate ranges with two numbers separated by a dash, using a comma for list demarcation.
...
You cannot mix lists and ranges within the same octet.
For example, the following filter is not allowed:
...
The following example uses iplike in a query's where clause:

`iplike(ipaddr, '192.8.*.*')`

//add IPLIKE detailed example from the wiki?
