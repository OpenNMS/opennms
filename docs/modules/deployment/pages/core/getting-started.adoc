...
The OpenNMS {page-component-title} Core services run as an unprivileged user and can't bind on port numbers below 1024 without escalated privileges.
...
You can verify your firewall and port forwarding configuration by sending an SNMP trap from a remote system to your OpenNMS {page-component-title} Core instance:
...
If you run on Debian or Ubuntu, you have to install the `snmp-utils` package.
