...
+
Note that failure to do so may generate an ICMP permission issue when starting OpenNMS.
(See https://opennms.discourse.group/t/h29-wont-start-with-permission-error-to-open-icmp-socket/2387/1[H29+ won't start with permission error to open ICMP socket] on Discourse.)
ifeval::["{page-component-title}" == "Horizon"]
