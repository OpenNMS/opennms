...
IMPORTANT: Immediately change the `admin` account password to a secure one.
...
.Data sources collection permission
image::deployment/core/data-sources.png[prompt,300]

Click *Show me what is being sent* to see what xref:operation:deep-dive/admin/housekeeping/introduction.adoc#ga-data-collection[information we collect].
Statistics collection and publication happen only if an admin user opts in.

NOTE: Admin users can xref:operation:deep-dive/admin/housekeeping/introduction.adoc#disable-data-collection[enable or disable usage statistics collection] at any time.
...
We encourage you to use {page-component-title} with a personalized account instead of the `admin` account.
This allows you to see who is completing tasks like clearing or acknowledging alarms, instead of everything being done by the `admin user.
