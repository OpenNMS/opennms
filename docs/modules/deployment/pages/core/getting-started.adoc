...
In order to achieve this you need to add the credentials by using the `scvcli` command line utility and reference these credentials in your `opennms-datasources.xml` file.
...
NOTE: If you need SNMP Trap listener on port 162/udp directly, see the "Binding to privileged ports" steps in <<core/getting-started.adoc#set-up-core-instance, Set up the Core Instance>>.
