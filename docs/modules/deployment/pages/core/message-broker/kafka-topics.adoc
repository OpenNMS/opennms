...
Sink Topics (optional - feature dependent):
  OpenNMS.Sink.Events                                          [EXISTS]
  OpenNMS.Sink.Syslog                                          [NOT CONFIGURED]
  OpenNMS.Sink.Trap                                            [EXISTS]
  OpenNMS.Sink.DeviceConfig                                    [EXISTS]

Sink Topics (telemetry - optional):
  OpenNMS.Sink.Telemetry-Netflow-5                             [EXISTS]
  OpenNMS.Sink.Telemetry-Netflow-9                             [NOT CONFIGURED]
  OpenNMS.Sink.Telemetry-IPFIX                                 [EXISTS]
...
.Required Topics (RPC, Heartbeat, Twin)
[cols="1,3"]
|===
| Status | Meaning

| `[OK]`
| Topic exists - system will function correctly

| `[MISSING]`
| Topic does not exist - this is a problem that needs to be fixed
|===

.Optional Topics (Events, Syslog, Trap, DeviceConfig, Telemetry)
[cols="1,3"]
|===
| Status | Meaning

| `[EXISTS]`
| Topic exists - the feature is configured and ready

| `[NOT CONFIGURED]`
| Topic does not exist - this is normal if the feature is not enabled
|===
