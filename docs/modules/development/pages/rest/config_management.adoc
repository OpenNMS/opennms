...
| /cm
| Returns a list of all registered configuration names in JSON format (for example, `["provisiond", "org.opennms.features.datachoices"]`). +
200: Return: String Array

| /cm/schema
| Returns the full OpenAPI schema (for Swagger UI) for all registered configurations, in JSON/YAML format depending on the `Accept` header. +
...
Accepts an optional `replace` query parameter (`?replace=true` or `?replace=false`, default `false`).
When `replace=false`, the supplied JSON is merged into the existing configuration; when `replace=true`, the existing configuration is fully replaced by the supplied JSON. +
200: Return: nothing +
400: Return: `{"MESSAGE": "<ERROR_MESSAGE>"}`
