...
Get SNMP interfaces whose node is in the foreign source `Servers` with an `ifIndex` of `6`:

[source, bash]
----
curl -u admin:admin "http://localhost:8980/opennms/api/v2/snmpinterfaces?_s=node.foreignSource==Servers;ifIndex==6"
----
