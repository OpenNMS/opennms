...
| `/requisitions/\{name}/import?rescanExisting=false`                   | Similar to the above import command, but existing nodes will not be scanned until the next rescan interval, only newly added/removed nodes will be updated.
