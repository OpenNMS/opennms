...
| /requisitions/deployed/stats
| Get statistics for all deployed requisitions. Returns, for each deployed foreign source, its name, the date it was last imported, and the list of requisitioned foreign IDs.

| /requisitions/deployed/stats/\{foreignSource}
| Get statistics for the named deployed requisition: its name, the date it was last imported, and the list of requisitioned foreign IDs.

| /requisitions/repositoryStrategy
| Get the currently configured foreign source repository strategy (returns plaintext rather than XML or JSON).
