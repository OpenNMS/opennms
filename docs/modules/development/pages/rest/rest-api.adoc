...
.Navigate to the Rest API Explorer: Select "Endpoints"
...
The response status and response time are displayed and you can use the tabs to via the Json response, response headers, as well as a listing of the corresponding `curl` command.

For most `POST` and `PUT` commands, additional Json request data is required; the API Explorer will generally display the schema and some example Json for the particular request.

.Sending a Rest Request in API Explorer
