...
| Retrieve metadata describing all available filters that can be applied to measurement queries.

| /measurements/filters/\{name}
| Retrieve metadata for the filter with the given name.
Returns `404 Not Found` if no filter with that name exists.
|===
...
| relaxed
| When set to `true`, the query does not fail if one or more of the requested attributes cannot be found; missing attributes are returned as `NaN` values instead.
| false
|===
