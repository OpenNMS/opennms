...
| /foreignSources/count
| Get the total number of active foreign sources (returns plaintext, rather than XML or JSON).
...
POST accepts XML, JSON, or Atom XML using `application/xml`, `application/json`, or `application/atom+xml` as its `Content-Type`.
...
| /foreignSources/deployed/\{name}
| Delete the named deployed (active) foreign source.
