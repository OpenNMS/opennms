...
This API also supports the following query parameters: `limit`, `offset`, `orderBy`, `order`, `deviceName`, `ipAddress`, `ipInterfaceId`, `configType`, `status`, `createdAfter`, and `createdBefore`.
...
This API supports the following query parameters: `limit`, `offset`, `orderBy`, `order`, `search`, `status`, and `pageEnter`.
