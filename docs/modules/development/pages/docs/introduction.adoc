...
This is a great way to help other OpenNMS community users who might have similar questions.

* Contribute formal technical documentation related to the source code on https://github.com/OpenNMS/opennms[GitHub]. 
You must specify a reviewer for this content, at least one of whom must be on the technical writing team.  
...
Include images only to illustrate a concept that may be difficult to understand or something that is not easy to locate in the UI.  

[[gd-docs-file-structure]]
== File structure in the opennms repository 

Documentation is in the `docs\modules` directory. 
