...
The queue names used by {page-component-title} are prefixed with a constant value.
If many {page-component-title} are configured to use the same broker, then these queues would end up being shared amongst the instances, which is not desired.
