...
| `OPENNMS_BROKER_URL` | ActiveMQ broker URL. | `tcp://127.0.0.1:61616`
|===
...
| `POSTGRES_DB`       | PostgreSQL database name. | `opennms`
...
|===
| Variable | Description | Default value
| `SENTINEL_FLOWS_ENABLED` | Set to `true` to install the `sentinel-flows`, `sentinel-jsonstore-postgres`, and `sentinel-blobstore-noop` features at startup. | `false`
|===

Individual adapters are defined by overlaying one configuration file per adapter into `$\{SENTINEL_HOME}/etc/` (see <<sentinel-etc-overlay>>), named
`org.opennms.features.telemetry.adapters-<name>.cfg`, where `<name>` uniquely identifies the adapter. This is the same factory-PID convention used to
configure Minion listeners.

.Example: `org.opennms.features.telemetry.adapters-NetFlow-5.cfg`
[source, properties]
----
name = NetFlow-5
...
| `PROM_JMX_EXPORTER_JAR`     | Path to the JMX Prometheus agent jar inside the container. | `/opt/prom-jmx-exporter/jmx_prometheus_javaagent.jar`
|===
