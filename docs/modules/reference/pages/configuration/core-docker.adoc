...
TIP: By default, the JVM temp directory (`java.io.tmpdir`) is `$\{OPENNMS_HOME}/tmp` and the embedded ActiveMQ
broker's on-disk store (`activemq.data`) is `$\{OPENNMS_HOME}/var/activemq`. To use different paths, pass them
via `JAVA_OPTS`, e.g. `JAVA_OPTS="-Djava.io.tmpdir=/some/path -Dactivemq.data=/some/other/path"`.
...
| `OPENNMS_LIBRARY_JRRD2`            | JRRD2 library path                                                      | optional | `/usr/lib/jni/libjrrd2.so`
|===
...
| `OPENNMS_CASSANDRA_HOSTNAME`   | A comma-separated list with Cassandra hosts for Newts                     | optional | `hostname`
