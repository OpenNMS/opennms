...
When there are multiple locations, the partition count should be greater than the max number of Minions at a location.

* By default, Kafka RPC uses a buffer size of 900KB.
When set to a larger size, they will be split into 900kb chunks.
Configure max buffer size by setting `org.opennms.core.ipc.rpc.kafka.max.buffer.size` to the desired size in bytes.
