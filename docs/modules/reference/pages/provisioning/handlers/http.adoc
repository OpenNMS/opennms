...
[[http-handler]]
= HTTP(S) Handler
:description: Learn how the HTTP(S) handler in OpenNMS Horizon/Meridian can import a requisition definition from an XML document served by a remote web server.
