...
To configure an HTTP(S) requisition, follow the steps in xref:reference:provisioning/handlers/introduction.adoc#create-req-def[Create a requisition definition], but specify HTTP or HTTPS as the type, set the host, path, schedule, and rescan behavior.
