...
To configure a DNS requisition, follow the steps in xref:reference:provisioning/handlers/introduction.adoc#create-req-def[Create a requisition definition], but specify DNS as the type, and set the host, DNS zone, foreign source, schedule, and rescan behavior.
