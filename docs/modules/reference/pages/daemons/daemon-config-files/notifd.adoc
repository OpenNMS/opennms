...
Metadata expression can be used for the authenticateUser and authenticatePassword fields in order to reference credentials stored in the secure credentials vault, or an OAuth2 access token via `${token:<name>}` for providers that require XOAUTH2 (see xref:operation:deep-dive/admin/configuration/token-authentication.adoc[token authentication] for a worked Microsoft 365 example).
| no
| no
