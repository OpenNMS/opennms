...
| syslogd-configuration.xml (`syslog-tcp-port` and the `tcp-*` attributes)
| Configure the optional xref:operation:deep-dive/events/sources/syslog.adoc#_tcp[TCP listener], including RFC 6587 framing and TLS.
Unset by default, in which case only UDP is used.
| yes
| no
