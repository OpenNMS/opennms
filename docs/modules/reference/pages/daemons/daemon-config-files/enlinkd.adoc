...
Data Collection is Scheduled for each node and protocol and queued into a Priority Executor Running Queue.
The data collection is performed according with the Priority assigned.
...
|===
|*WARNING*
|*use-{protocol}-discovery*=*false* deletes *protocol* persisted data.
|===
