...
Availability:: The ability of a resource or service to perform its intended function when required.
...
Direct-discovered nodes:: Nodes that are added to {page-component-title} manually through the requisition UI.
...
Traps:: Event triggers collected from SNMP-capable devices on the network by the {page-component-title} Trapd service daemon.
