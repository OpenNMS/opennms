...
{page-component-title} typically shows availability statistics for the past 24 hours, and calculates this by subtracting the amount of outage time for a node/interface/service.
...
The Trapd and Syslogd daemons can be configured to generate this event when messages are received from nodes that do not exist in {page-component-title}'s database.
...
The Telemetry daemon in {page-component-title} accepts data sent from nodes in one of the supported protocol formats.
(See xref:operation:telemetryd/introduction.adoc[Telemetry Daemon].)
