...
Asset:: Any data, device, or other component that is monitored by {page-component-title}.
...
xref:operation:bsm/introduction.adoc[Business Service Monitoring (BSM)]:: A {page-component-title} feature that monitors high-level business Service Monitor contexts and quickly identifies the most critical problems affecting them.
...
They allow you to define a matrix for aggregating monitoring results.
...
https://docs.docker.com/[Docker]:: An open-source container virtualization service that allows you to deploy application-level services without worrying about operating system maintenance.
...
Drools:: A Business Rules Management System solution that supports the Java Rules Engine API standard.
It is used in the {page-component-title} Alarmd daemon to provide a more robust infrastructure for workflow and problem state management.
...
https://docs.opennms.com/helm/latest[OpenNMS HELM] is a Grafana plugin that allows users to create customized dashboards with data from {page-component-title}.
...
Management Information Base (MIB):: A database used for managing entities on a network.
Used in {page-component-title} to facilitate Transparent Bridge Discovery.
(See xref:operation:topology/enlinkd/layer-2/bridge-discovery.adoc[Bridge Discovery].)
...
xref:development:minion/introduction.adoc#minion[Minion]:: An instance of the Karaf OSGi service that enables OpenNMS to monitor devices and services in locations that OpenNMS cannot reach.
...
It can be caused by systems or communications failures, or it can be planned as part of routine maintenance.
