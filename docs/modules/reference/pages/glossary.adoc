...
IPLIKE:: IPLIKE is a stored procedure you can install with the PostgreSQL database that provides more flexible searching of IP addresses. (See xref:deployment: core/getting-started.adoc#iplike-install[Installing IPLIKE]).
...
Stored procedure:: A group of SQL statements stored in a database that can be reused.
