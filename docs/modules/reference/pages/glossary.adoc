...
Elasticsearch:: Elasticsearch is a NoSQL database (See https://azure.microsoft.com/en-us/overview/nosql-database/[What is NoSQL?]) with an integrated search engine based on the Lucene library.
...
Nodes can be manually assigned a location, or the location can be automatically determined using the GeoIP Provisioning Adapter (See xref:reference:provisioning/adapters/geoip.adoc[GeoIP]).
