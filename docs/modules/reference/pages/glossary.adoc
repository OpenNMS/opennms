...
(See https://en.wikipedia.org/wiki/Jira_(software)[Jira].)
...
Nodes can be manually assigned a location (See xref:operation:reference/provisioning/handlers/dns.adoc[Location]), or the location can be determined using the GeoIP Provisioning Adapter (See xref:operation:reference/provisioning/adapters/geoip.adoc[GeoIP]).
