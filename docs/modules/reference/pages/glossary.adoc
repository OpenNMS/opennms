...
Architecture for Learning Enabled Correlation (ALEC):: An OpenNMS framework that uses artificial intelligence and machine learning to provide alarm triage.
...
{page-component-title} typically shows availability statistics for the past 24 hours; this is calculated by subtracting the amount of outage time for a node, interface, or service.

Azure:: Microsoft Azure is a cloud hosting platform that allows users to host a variety of different services.

Bean Scripting Framework (BSF):: A framework that allows scripting to be used in Java code via a set of Java classes.
{page-component-title} uses BSF-compatible scripts to determine the status of a service.
(See xref:reference:provisioning/detectors/BsfDetector.adoc[BSFDetector].)
...
Daemon:: A {page-component-title} sub-service that performs a specific set of tasks.
(See xref:reference:daemons/introduction.adoc[Introduction to Daemons].)
...
Docker:: An open-source container virtualization service that allows you to deploy application-level services without worrying about operating system maintenance.
...
Grafana:: An open-source analysis and visualization web application.
...
HELM:: An OpenNMS application for creating flexible dashboards to display and customize fault, flow, and performance data from {page-component-title}.
...
Horizon:: An open-source solution from OpenNMS that helps users visualize and monitor everything on their local and remote networks.
...
Jira:: A project management platform that {page-component-title} uses to track software issues, feature requests, and IT requests.
...
Kubernetes:: An open-source container orchestration system for automating software deployments, scaling, and management.
Originally designed by Google, the Cloud Native Computing Foundation now maintains it.
(See https://kubernetes.io/[Kubernetes].)

Lightweight Directory Access Protocol (LDAP):: An industry standard protocol used by {page-component-title} to access and maintain distributed directory information services on an IP network.
...
Meridian:: A subscription-based, optimized, and stable version of the OpenNMS Horizon platform that includes only the most stable and secure Horizon features.
...
Northbounder:: An alarm generated by {page-component-title} that is sent to a higher-level component (service, resource, etc.).
...
Round Robin Database (RRD):: A special type of database designed for collection, visualization, and analysis of time-series data.
...
Service:: Software that performs a specified set of tasks (automation, listening for data requests, etc.).
...
Simple Network Management Protocol (SNMP):: An internet standard protocol used by {page-component-title} to collect, organize, and modify information about managed devices on an IP network.
...
Southbounder:: An alarm generated by {page-component-title} that is sent to a lower-level component (service, resource, etc.).
...
Topology:: A description of a network's elements (devices, services) and the connections between them.
(See xref:operation:topology/introduction.adoc[Introduction to Topology].)

Traps:: Event triggers generated by SNMP-capable devices on the network and sent to the {page-component-title} Trapd service daemon.
(See xref:operation:events/sources/snmp-traps.adoc[SNMP Traps].)