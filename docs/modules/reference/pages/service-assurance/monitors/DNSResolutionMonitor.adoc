...
You can use it in combination with the <<deep-dive/service-assurance/monitors/PageSequenceMonitor.adoc#pagesequencemonitor, PageSequenceMonitor>> to provide a hint if a website is not available for DNS reasons.

The <<deep-dive/service-assurance/monitors/DnsMonitor.adoc#poller-dns-monitor, DnsMonitor>> is a test against a specific DNS server.
