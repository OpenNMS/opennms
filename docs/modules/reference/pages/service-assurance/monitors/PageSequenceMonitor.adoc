...
<6> Assign the value of a regex match group to a session variable
...
<1> The relative URL to call in the request.
<2> Host port number to use when connecting.
<3> Name of the RRD data source (DS) name in which to store this service monitor’s response-time samples.
