...
If sending events directly from an external system such as via xref:operation:events/sources/rest.adoc[REST] or xref:operation:events/sources/kafka.adoc[Kafka Consumer], the event can be sent directly with these parameters for Eventd to process.
If you will be extracting passive status updates from traps or syslog messages, you can use the xref:operation:events/event-translator.adoc#ga-event-translator-psk[event translator] to generate the `uei.opennms.org/services/passiveServiceStatus` events.
...
If you will have multiple passive monitors on a node, using xref:operation:service-assurance/polling-packages.adoc#ga-pollerd-packages-patterns[service patterns] can make configuration easy.
