...
<service name="Script_Example" interval="300000" user-defined="true" status="on">
  <parameter key="script" value="/opt/opennms/contrib/Script_Example.sh"/> <1>
  <parameter key="timeout" value="5000"/> <2>
</service>

<monitor service="Script_Example" class-name="org.opennms.netmgt.poller.monitors.SystemExecuteMonitor"/> <3>
----
<1> The system call to run.
<2> Timeout for the *isReachable* method, in milliseconds.
<3> Required *monitor* section.
...
<service name="Script_Example" interval="300000" user-defined="true" status="on">
  <parameter key="script" value="/opt/opennms/contrib/Script_Example.sh"/> <1>
  <parameter key="banner" value="PASSED"/> <2>
  <parameter key="timeout" value="5000"/> <3>
</service>

<monitor service="Script_Example" class-name="org.opennms.netmgt.poller.monitors.SystemExecuteMonitor"/> <4>
----
<1> The system call to run.
<2> A string that is matched against the output of the system-call.
