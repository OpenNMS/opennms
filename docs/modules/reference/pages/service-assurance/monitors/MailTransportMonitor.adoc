...
| use-jmta
| Deprecated; the JMTA transport has been removed.
The attribute is still accepted (and still defaults to `true`), but the test always sends through the configured sendmail-host.
| true
