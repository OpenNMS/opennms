...
For example, if the monitor checks at 11:59, but the trigger time is 12:00 a.m., the monitor retrieves the configuration at 12:04 a.m., which is the next check interval.