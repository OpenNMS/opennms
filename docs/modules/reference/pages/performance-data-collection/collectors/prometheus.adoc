...
When `key-store` is set, HTTP redirects are followed only when the target stays on the host of the original URL, so that the client certificate is not presented to other hosts.
An endpoint that redirects scrapes to a different host will appear to return no metrics; point the `url` parameter at the final location instead.
