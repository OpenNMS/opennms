...
The extracted from out of a hex string data can be of various types. At the moment is the support for float data type
implemented, but it can be easily extended if necessary. Implemented float format have 120 as type field in ASN.1.
To define the opaque type in a data collection the attribute `type` of element `mibObj` must be set to `opaque`
