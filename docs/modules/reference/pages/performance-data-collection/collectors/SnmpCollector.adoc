...
Opaque data types are ASN.1(Abstract Syntax Notation One) binary encoded data which are additionally wrapped in a hex string.
OpenNMS interprets these encoded values and properly compiles them so that users do not need to edit the resulting compiled data collection definitions.
OpenNMS currently supports the float data type, but this can be extended, if necessary. 
