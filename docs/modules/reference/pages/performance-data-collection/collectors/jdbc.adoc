...
Once the definition is added to `$\{OPENNMS_HOME}/etc/jdbc-datacollection-config.xml`, you can test it using the `collect` command in the xref:operation:deep-dive/karaf-shell/karaf-shell.adoc[Karaf shell]:
