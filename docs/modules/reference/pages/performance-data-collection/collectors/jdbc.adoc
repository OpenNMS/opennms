...
JDBC collections are defined in `etc/jdbc-datacollection-config.xml`.
...
Once the definition is added to `etc/jdbc-datacollection-config.xml`, you can test it using the `collect` command in the Karaf Shell:
