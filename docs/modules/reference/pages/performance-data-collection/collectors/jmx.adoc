...
Once added to `etc/jmx-datacollection-config.xml` you can use the `collect` command available in the xref:reference:karaf-shell/karaf-shell.adoc[Karaf shell] to test it:
