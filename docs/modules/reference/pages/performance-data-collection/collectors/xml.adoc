...
To circumvent this, we allow to define mappings between input values and values to be persisted.
...
The following group configuration allows to persist the `input` values as integer values over time:
...
<3> A default value can be defined by omitting the `from` attribute in a `xml-mapping` definition.
