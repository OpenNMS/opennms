...
The BMP parser accepts BMP connections from router packets using a <<deep-dive/telemetryd/listeners/tcp.adoc#telemetryd-listener-tcp, TCP Listener>>.
...
The BMP telemetry adapter handles BMP statistics received and parsed by the <<deep-dive/telemetryd-bmp-parser, BMP parser>>.
...
It handles BMP Peer Up and Down messages that the <<deep-dive/telemetryd-bmp-parser, BMP Parser>> receives and parses, and converts to {page-component-title} events.
...
It handles BMP messages the <<deep-dive/telemetryd-bmp-parser, BMP Parser>> receives and parses, and creates OpenBMP-compatible messages, which are then passed to the OpenBMP Kafka cluster.
...
It handles BMP messages the <<deep-dive/telemetryd-bmp-parser, BMP Parser>> receives and parses, and creates OpenBMP-compatible messages, which are then
