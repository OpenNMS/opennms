...
| jira.status.open
| A comma-separated list of Jira status names under which a ticket should be considered 'Open.'

| jira.status.closed
| A comma-separated list of Jira status names under which a ticket should be considered 'Closed.'

| jira.status.cancelled
| A comma-separated list of Jira status names under which a ticket should be considered 'Canceled.'
...
Besides the common fields (creator, create date, description, and subject), you also may also need to set custom fields.
...
The project key and issue types can also be redefined in the `jira.properties` file.
...
| date
| Any date in the _YYYY-MM-DD_ format.

| datetime
| Any date-time value in the ISO 8601 format (_YYYY-MM-DDThh:mm:ss.sTZD_).
...
. Defined components are `core`, `service`, and `web`.
