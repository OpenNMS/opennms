...
=== Configuring a listener on a Minion
...
3+| *Optional*
| nodeCache.maximumSize      | 1000    | The maximum size of the cache.
| nodeCache.expireAfterWrite | 300     | Number of seconds until an entry in the node cache is evicted. Set to 0 to disable eviction.
| nodeCache.recordStats      | true    | Defines if cache statistics are exposed via JMX. Set to `false` to disable statistic recording.
|===
...
| Property                                   | Default | Description
3+| *Optional*
| cache.classificationFilter.enabled         | false   | Enables or disables the cache.
| cache.classificationFilter.maxSize         | 5000    | The maximum size of the cache.
| cache.classificationFilter.expireAfterRead | 300     | Number of seconds until an entry in the node cache is evicted.
                                                         Set to 0 to disable eviction.
                                                         The timer is reset every time an entry is read.
| nodeCache.recordStats                      | true    | Defines if cache statistics are exposed via JMX.
