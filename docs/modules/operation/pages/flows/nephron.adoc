...
Nephron provides a number of metrics that can be used to monitor its operation. In case Prometheus is used to monitor Nephron the link:https://ci.apache.org/projects/flink/flink-docs-master/docs/deployment/metric_reporters/#prometheus[Prometheus metric reporter] has to be enabled by adding the following lines to `flink-conf.yaml`:
...
A corresponding scrape configuration must be added to `prometheus.yml`:
