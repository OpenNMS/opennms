...
This is an asset management report that shows all assets with maintenance contracts that are expiring within a threshold period, in a range from today to the number of days specified in the Warning Threshold parameter.
...
The report includes the node label, manufacturer, maintenance contract, expiration date, and status.
...
You can customize this report with three company logos that are each approximately 195px by 50px.
One logo is in the report header, another in the page header, and a third in the page footer.
...
True will include contracts that have already expired along with assets with maintenance contracts that will expire within the number of days specified for the warning threshold.
| true
