...
| Status
| One-page summary of statistics on outages and availability for user-specified surveillance category during a user-specified number of days.
...
| Grafana
| Create Report from Grafana Dashboard; three options available.
...
| Asset Management
| Provides an overview of assets, showing how many within each age group have an active contract, no contract, or an expired contract; shows how many will expire in the next 12 months.
