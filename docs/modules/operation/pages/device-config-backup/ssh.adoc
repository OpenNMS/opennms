...
OpenNMS uses an SSH scripting engine to run the commands required to upload device configurations.
You must create a script for each type of device whose configuration you want to back up and save this script in the `etc/device-config` folder.
This script effectively simulates an interactive session with a network device that tells the device to create a backup of its configuration and send that backup to a TFTP server operated by the Minion or OpenNMS server.
...
OpenNMS treats configuration types as arbitrary strings, and uses when no configuration type is specified.
...
* `await: ...`: Waits for some response from the device that matches the given string
...
This sample script is for a Palo Alto firewall.
