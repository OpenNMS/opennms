...
NOTE: Some devices(ex: Juniper) may allow TFTP upload on non-default port. For such devices there is no need to change default port.
...
cannot be automatically determined. In this case IP Address need to be configured as below:
...
== Backup device configuration on demand

You can manually backup devices from OpenNMS UI under Info>Device Configs.
