...
Device Config backup shares resources with Poller, particularly poller threads.
...
As a rough estimate, start with number of devices you expect will ever be simultaneously backed up, multiply by the time (in minutes). Increase the existing thread pool size by the resulting figure.
For example: 50 devices each backup running 1 minute, should increase pool size by 50.
