...
====
Decreasing the significance of the reduction-key is a way to aggregate, for example, all nodes down in to a single alarm.
...
.Example of a `interfaceUp` event clearing a `interfaceDown` alarm
[source, xml]
----
   <event>
...
----
<1> The `interfaceDown` event sets a `reduction-key` that include enough info to identify a specific interface on a specific node.
