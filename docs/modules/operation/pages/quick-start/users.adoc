...
:description: Step 1 to set up {page-component-title}: usage statistics, creating a user, the default admin account.

As part of the installation process, we recommend signing in using the default `admin` account.
If you have not already done so, sign in for the first time and change the `admin` account password to a secure one (see xref:deployment:core/install.adoc#ga-data-choices[First-time sign in and usage statistics]).
...
WARNING: Do not delete the default `admin` user account.
