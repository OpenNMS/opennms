...
Before xref:operation:quick-start/notifications.adoc[setting up default notifications], you must configure notification information for the `admin` user profile:

. Click the *Gear* icon at the top-right of the screen.
. Choose menu:OpenNMS System[Configure Users, Groups, and On-Call Roles, Configure Users].
. Click *Modify* beside the `admin` user account.
. In the Notification Information section, add contact information for default notification routing (for example, an email address or mobile phone number).
