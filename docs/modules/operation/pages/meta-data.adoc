...
The special context `scv` provides access to values stored in the Secure Credential Vault.
...
Will resolve the `credentials` value specified in the requisitioning UI while using the node label as fallback value.
