...
| node:geohash           | A https://en.wikipedia.org/wiki/Geohash[Geohash] of the node's latitude/longitude
|===
...
The special context `scv` provides access to values stored in the xref:operation:deep-dive/admin/scv-configuration.adoc[Secure Credentials Vault].
