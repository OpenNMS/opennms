...
When it is enabled, alarms are indexed in Elasticsearch when they are created, deleted, or when any "interesting" fields on the alarm are updated.
...
** (Optional) To set the feature to start automatically upon future service starts, you can add `opennms-alarm-history-elastic` to `$\{OPENNMS_HOME}/etc/featuresBoot.d/alarm.boot`.
...
When alarms are deleted, a new document containing the alarm ID, reduction key, and deletion time is pushed to Elasticsearch.
...
| @update_time
| Time (in milliseconds) when the document was created.
...
| Time (in milliseconds) to wait before re-indexing an alarm if nothing "interesting" has changed.
| 3600000
