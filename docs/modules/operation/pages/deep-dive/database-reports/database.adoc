...
Every time a report is run, the Report Engine passes it a JDBC connection to the {page-component-title} database.
...
The node source descriptor references a node using either its foreign source and foreign ID, or it node ID.
