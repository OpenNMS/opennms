...
By default, the TFTP server uses non-privileged port `6969` on Minions and {page-component-title}.
Follow the steps below to configure the TFTP port:
...
== Back up and retrieve device configuration via Karaf
...
NOTE: In the shell, you can use `opennms:dcb-get --help` to view the command's help information.
...
NOTE: These commands will not persist configuration settings by default.
