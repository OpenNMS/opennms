...
WARNING: Do not delete the default `admin` user profile.

NOTE: Releases before 37 shipped an `rtc` system account that the start page used to receive availability data.
It is no longer needed and can be deleted after upgrading.
