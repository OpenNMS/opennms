...
If a flow threshold does not specify an 

To create a flow threshold that applies to all applications (for example, "if any application consumes more than X bandwidth"), use a `match-all` regex expression inside the filter like `.*` to match all applications

//not sure what was meant in the Jira by "But then you could just threshold on the interface statistics."

You cannot apply flows thresholds to the total bandwidth for a location.
