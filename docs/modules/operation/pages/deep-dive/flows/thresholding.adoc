...
Finally, define the thresholds under menu:Administration[Configure Thresholds], or over the xref:development:rest/thresholds-rest-api.adoc[Thresholds REST API].
Use `flowApp` for the `ds-type`.
The following example defines thresholds for HTTPS traffic exceeding 4096000 bytes/second.

.Example of thresholds for https traffic
[source, xml]
----
<?xml version="1.0"?>
<thresholding-config>
