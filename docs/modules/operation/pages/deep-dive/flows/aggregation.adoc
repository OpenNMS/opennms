...
To use this functionality you must enable the Kafka forwarder as described in <<deep-dive/flows/setup.adoc#kafka-forwarder-config, Configure Kafka forwarder>> and set up the streaming analytics tool to process flows and persist aggregates in Elasticsearch.
