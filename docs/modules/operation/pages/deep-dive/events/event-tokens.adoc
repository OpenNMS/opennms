...
== Event configuration elements (former `eventconf.xml`file)

The following elements in event definitions accept replacement tokens:
...
NOTE: `autoaction`, `operaction`, and `tticket` are legacy event fields that are not used by other services.
