...
Use the `mask` tag to match SNMP traps in your event definitions.
...
The order in which events are defined is extremely important.
...
To compare variables whose order is unknown, the OID can also be specified.
To do this, use the `vboid` element instead of the `vbnumber` element.

[source, xml]
----
<varbind>
    <vboid>.1.3.6.1.4.1.28458.1.26.3.1.1.13.0</vboid>
    <vbvalue>1</vbvalue>
</varbind>
----

This refers to the variable in the mask using the OID and compares its value, ignoring the variable order.

NOTE: Please note that specifying both `vbnumber` and `vboid` within a `varbind` element is not permitted.
