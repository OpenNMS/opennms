...
* Manually send the corresponding event to the {page-component-title} server and see how the event is handled (see xref:deep-dive/events/event-debugging.adoc#send-event[Manually send events]).
...
See xref:operation:deep-dive/events/event-definition.adoc#ga-events-anatomy-of-an-event[Anatomy of an event] for details on event structure.
...
The search will stop with the first event definition that matches the given event (see xref:operation:deep-dive/events/event-configuration.adoc[eventconf.xml]).
...
See xref:operation:deep-dive/events/event-tokens.adoc#parameter-tokens[Parameter tokens] for more information.
