...
The process for creating a custom resourceType definition is the same as configuring your data collection settings, with only a few small differences.
...
For the MIB-2 host resources storage table, OpenNMS uses the following definition:
...
This ensures that {page-component-title} knows what should be gathered for different types of SNMP agents.
