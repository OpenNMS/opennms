...
The forecast page surfaces a yellow banner above the graph, and draws no overlays, when the measurements query returns no data for the selected metric.
This commonly happens when the graph is new, has gaps, or the history window extends before collection started — or when the upstream `Chomp` and `Outlier` filters remove too many points.
