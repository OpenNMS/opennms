...
If the in-sample residuals are all zero (typical for a perfectly flat metric) sigma is zero and the bounds collapse onto the fit line, while the forecast itself still renders.

== Limitations and warnings

The forecast page surfaces a yellow banner above the graph when one of the following conditions is detected.
The historical data line still renders; the banner explains why the forecast overlay is missing or degenerate.

[cols="1,3"]
|===
| Condition | What it means

| Could not load data for the selected metric
| The measurements query returned no data.
This commonly happens when the graph is new, has gaps, or the history window extends before collection started.
Widen the *Training Start* window or pick a different metric.

| Forecast could not be produced
| The response has no `HWFit` column, usually because the selected training window does not have enough historical data.

| Forecast produced no valid values
| The filter pipeline ran but every fit sample is `NaN`.
Typical cause: the upstream `Chomp` and `Outlier` filters removed too many points.
A metric that reaches zero within its season is also reported here, because the multiplicative Holt-Winters model cannot forecast through a zero.
