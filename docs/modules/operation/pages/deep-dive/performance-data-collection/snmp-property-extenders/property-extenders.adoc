...
When the string property exists as a column in the same table that contains the numerical metrics, it's easy to associate the string to the correct resource by adding a `mibObj` with the same `instance` attribute and the `string` type.
