...
{page-component-title} uses resource types to group sets of performance data measurements for persisting, indexing, and display in the web UI.
...
There are two resource types that do not have a `resource-type` definition: `node` and `ifIndex`.
