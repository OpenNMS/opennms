...
. Select specific MBeans and composite data objects to be collected by {page-component-title}.
...
.JMX connection configuration dialog
image::operation/jmx-config-generator/01-webui-connection.png["JMX connection configuration dialog with example configuration settings"]
...
The MBeans Configuration dialog lets you assign MBeans and composite data attributes to RRD domain-specific data types and data source names.

.Select MBeans and composite data attributes for {page-component-title} data collection
image::operation/jmx-config-generator/02-webui-mbean-selection.png["MBeans Configuration dialog displaying a list of MBeans and composite data attributes. Some are selected and have been assigned aliases"]
...
image::operation/jmx-config-generator/03-webui-mbean-details.png["MBean Configuration dialog displaying an MBean's details and attributes. Both attributes are selected and have been assigned aliases."]

.Configure composite attributes for data collection
image::operation/jmx-config-generator/04-webui-composite-details.png["MBean Configuration dialog displaying a composite data object's details and members. All four members are selected and have been assigned aliases."]
...
The contents of the configuration files can be manually copied and pasted, or they can be downloaded as a `.zip` archive.

NOTE: If a configuration file exceeds 2,500 lines, it can be downloaded only as a `.zip` archive.
