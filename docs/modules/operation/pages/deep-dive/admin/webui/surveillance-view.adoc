...
For example, to create a view that can only be accessed by the user `drv4doe`, edit `$\{OPENNMS_HOME}/etc/surveillance-view.xml` and add a definition for the user:
...
The same can be done for group names.

NOTE: The definition should be added within the `<views>` tag, above the default view.
