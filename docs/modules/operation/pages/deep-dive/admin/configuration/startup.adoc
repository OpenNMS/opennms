...
broker's on-disk store (`activemq.data`) is `$\{OPENNMS_HOME}/var/activemq`. To use different paths -- for example,
to keep the pre-upgrade locations under `$\{OPENNMS_HOME}/data/tmp` -- set `ADDITIONAL_MANAGER_OPTIONS` in
`opennms.conf`:
+
[source, properties]
----
ADDITIONAL_MANAGER_OPTIONS="-Djava.io.tmpdir=${OPENNMS_HOME}/data/tmp -Dactivemq.data=${OPENNMS_HOME}/data/tmp/activemq"
----
