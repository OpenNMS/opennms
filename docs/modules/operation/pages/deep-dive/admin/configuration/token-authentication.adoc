...
For Microsoft 365, the client-credentials flow requires an Entra application with the `SMTP.SendAsApp` application permission, tenant admin consent, and a service principal registered in Exchange Online (see Microsoft's documentation on https://learn.microsoft.com/en-us/exchange/client-developer/legacy-protocols/how-to-authenticate-an-imap-pop-smtp-application-by-using-oauth[SMTP OAuth]).
...
The raw `mail.smtp.auth.mechanisms` line passes through to the mail session unchanged and restricts authentication to XOAUTH2; without it, the client may attempt other mechanisms first and fail.
