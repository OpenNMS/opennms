...
== Worked example: OAuth2 SMTP authentication (Microsoft 365)

Notification mail sent through providers that have disabled basic SMTP authentication (Microsoft 365, Gmail) can authenticate with an OAuth2 access token via the SASL XOAUTH2 mechanism.
A token-auth definition acquires and caches the token; the mail configuration references it with `${token:<name>}` in the password field and selects the XOAUTH2 mechanism.

For Microsoft 365, the client-credentials flow requires an Entra application with the `SMTP.SendAsApp` application permission, tenant admin consent, a service principal registered in Exchange Online, and mailbox permissions granted to that service principal (see Microsoft's documentation on https://learn.microsoft.com/en-us/exchange/client-developer/legacy-protocols/how-to-authenticate-an-imap-pop-smtp-application-by-using-oauth[SMTP OAuth]; a missing mailbox grant typically surfaces as a 535 error with an otherwise correct configuration).
The token request must use the scope `https://outlook.office365.com/.default`.

.token-auth-configuration.xml
[source, xml]
----
<token-auth name="m365-smtp">
  <url>https://login.microsoftonline.com/${scv:m365:tenant_id}/oauth2/v2.0/token</url>
  <method>POST</method>
  <content type="application/x-www-form-urlencoded">grant_type=client_credentials&amp;client_id=${scv:m365:client_id}&amp;client_secret=${scv:m365:client_secret}&amp;scope=https%3A%2F%2Foutlook.office365.com%2F.default</content>
  <token-from jsonpath="access_token"/>
  <ttl-seconds>3600</ttl-seconds>
</token-auth>
----

For notification mail (notifd), reference the token in `javamail-configuration.properties`:

[source, properties]
----
org.opennms.core.utils.mailHost=smtp.office365.com
org.opennms.core.utils.smtpport=587
org.opennms.core.utils.starttls.enable=true
org.opennms.core.utils.authenticate=true
org.opennms.core.utils.authenticateUser=notifications@example.com
org.opennms.core.utils.authenticatePassword=${token:m365-smtp}
mail.smtp.auth.mechanisms=XOAUTH2
----

The raw `mail.smtp.auth.mechanisms` line passes through to the mail session unchanged and is required: the mail client never attempts XOAUTH2 unless the mechanism is selected explicitly.
The token is fetched at connection time, so a cached token that has been refreshed is picked up automatically.

For mail paths driven by `javamail-configuration.xml` (reportd, the email northbounder), the equivalent configuration on a `sendmail-config` with `use-authentication="true"` is (element order matters; `javamail-property` precedes the host and auth elements):

[source, xml]
----
<javamail-property name="mail.smtp.auth.mechanisms" value="XOAUTH2"/>
...
<user-auth user-name="notifications@example.com" password="${token:m365-smtp}"/>
----
