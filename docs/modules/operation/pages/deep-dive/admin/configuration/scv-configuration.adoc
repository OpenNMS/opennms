...
By default, the Secure Credentials Vault in OpenNMS uses a `JCEKS` keystore with the file name `scv.jce`.
...
----
NOTE: If you change the keystore type to `PKCS12`, be sure to update the file extension to match the format (e.g., use `.pk12` instead of `.jce`).
