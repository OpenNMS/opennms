...
{page-component-title} already persists traps in an events form.
...
- If `paramValue` exists in eventconf, {page-component-title} converts the matching key into numeric values.
...
You need to create separate snmp-graph settings in the `$\{OPENNMS_HOME}/etc/snmp-graph.properties.d` directory.
