...
. Specify additional information, including the UUID, node ID, parameters, severity, and so on, then click *Send Event*.
...
The `host:port` defines the hostname and port to send the event; by default, this value is for the local host.
