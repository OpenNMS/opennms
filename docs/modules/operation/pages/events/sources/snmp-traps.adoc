...
Configure SNMP traps to customize the types of information you receive and the conditions under which {page-component-title} converts the trap into an event.
...
* Make changes to the xref:operation:admin/webui/mib.adoc[SNMP MIB compiler].
...
Thus the second paramater, the `sysName`, is printed using `%parm[#2]%`.
