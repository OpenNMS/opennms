...
It receives SNMP traps via the xref:reference:daemons/daemon-configfiles/alarmd.adoc[trapd service daemon], which is enabled by default.

IMPORTANT: Disabling trapd renders {page-component-title} incapable of receiving SNMP traps.

{page-component-title} transforms these traps into events according to default preconfigured rules and those created by users.
...
Configure SNMP traps to customize the types of trap information you receive and the conditions under which OpenNMS converts the trap into an event.
...
OpenNMS transforms these traps into events only when the conditions you define are met.
...
* Manually send the corresponding event to the OpenNMS server and see how OpenNMS handles it (see x-reference to the send-event.adoc file)
* Make changes to MIB files, if required (see xref:operation:events/mib.adoc)
...
** Filter on variable bindings for further granularity on the conditions for creating an event
...
See xref:operation/events:anatomy-events.adoc[Anatomy of an Event] for details on event structure.
...
The search will stop with the first event definition that matches the given event (see xref:operation:events/anatomy-events.adoc[eventconf.xml]).

Thus if the above code with the wildcard was listed before the more specific ciscoC3800SysAggregateStatusChange event, the latter event would never be generated.
...
In the <code>ciscoC3800SysAggregateStatusChange</code> event listed above, there are four of them, and they can be accessed using the <code>parm</code> replacement token.
...
We re-write our event as follows:
...
In our example, adding a mask with a varbind tag will match on the same id, generic and specific values, but also requires that the third parameter is equal to "3" (indicating a Cisco-determined trap severity of "major").
...
Because OpenNMS does not have a MibParser, we usually put this map (between the numerical encoded value and its meaning) into the event description.
...
The trap contains the following varbind:  `cHsrpGrpStandbyState`, whose possible values are from 1 to 6:
* initial(1)
* learn(2)
* listen(3)
* speak(4)
