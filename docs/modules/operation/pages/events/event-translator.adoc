...
This example uses a trap from an Adtran TA4303 to create a new passive status event.
...
This example is hard coding the label, but this can also be a lookup from the original event's parameters.
...
<5> Lookup the port description from our SQL table.
This new parameter can be used in the event and notifications  as `%parm[portdesc]%`.
