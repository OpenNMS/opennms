...
As with most OpenNMS daemons, the EventTranslator has a companion configuration "factory" class that marshals its configuration from an XML-formatted file.
...
This lets the user associate the traps with a node other than the SNMP manager (typically a passive node) making correlation much easier.
