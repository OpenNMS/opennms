...
This monitor uses the name resolver configuration from the poller configuration or from the operating system where {page-component-title} is running on.
...
The response from the A record lookup can be any address, it is not verified with the IP address on the {page-component-title} IP interface where the monitor is assigned to.
...
| `nameserver`      | The DNS server to query for the records. +
                      ...
                      hostname:port, or [ipv6address]:port.             | optional | Use name server from host system running {page-component-title} | Yes
|===
...
In {page-component-title} the DNS server is the node and the DnsMonitor will send a lookup request for a given A record to the DNS server IP address.
