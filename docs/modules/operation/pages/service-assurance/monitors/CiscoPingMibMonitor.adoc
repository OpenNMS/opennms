...
After the IOS device completes the requested ping operations, the poller monitor queries the IOS device to determine the results.
...
NOTE: Unlike most poller monitors, the CiscoPingMibMonitor does not interpret the timeout and retries parameters to determine when a poll attempt has timed out or whether to attempt it again.
...
* One or more Cisco devices running any 12.2 or later IOS image after 12.2.
...
.Monitor specific parameters for the CiscoPingMibMonitor
[options="header, autowidth"]

This monitor has no required parameters.
The following are optional parameters. 
|===
| *Parameter* | *Description* | *Default* 
| version                    | SNMP protocol version (1, 2c, or 3) to use for operations this service monitor performs. 
                                  Do not use without a very good reason to do so.                             | from snmp-config.xml
