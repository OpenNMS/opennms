...
* The _IOS_ devices that will perform the remote pings must be configured with an _SNMP write community_ string whose source address access-list includes the address of the {page-component-title} server and whose MIB view (if any) includes the OID of the _ciscoPingTable_.
...
While {page-component-title}' world-class service assurance is generally sufficient, Bar also wants to be notified any time a premise router at one of their locations unreachable from the perspective of an _IOS_ device in Bar's main data center.
