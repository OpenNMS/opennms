...
The monitor class DhcpMonitor is run by Pollerd and opens the background process listening for incoming DHCP responses.
...
You can modify the behavior for testing the DHCP server in `poller-configuration.xml`.
