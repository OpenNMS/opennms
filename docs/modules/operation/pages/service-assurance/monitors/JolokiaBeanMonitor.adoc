...
If it is required to run a method via JMX or poll an attribute via JMX, the JolokiaBeanMonitor can be used.
It requires a fully installed and configured Jolokia agent to be deployed in the JVM container.
If required it lets you provide attribute names, paths, and method parameters as additional arguments to the call.
