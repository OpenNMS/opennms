...
The Win32ServiceMonitor enables {page-component-title} to monitor the running state of any Windows service.
...
For this reason it is required the SNMP agent and {page-component-title} is correctly configured to allow queries against part of the _MIB_ tree.
...
== Troubleshooting
If you have created a _Win32ServiceMonitor_ poller and are having difficulties with it not being monitored properly on your hosts, chances are there is a difference in the name of the service you've created, and the actual name in the registry.
