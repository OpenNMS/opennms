...
To assign a polling package to nodes, use the the link:http://www.opennms.org/wiki/Filters[Rules/Filters] syntax.
...
<2> Base filter on IP address, categories or asset attributes of nodes based on link:http://www.opennms.org/wiki/Filters[Rules/Filters].
