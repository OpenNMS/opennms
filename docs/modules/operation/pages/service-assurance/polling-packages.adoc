...
To define more complex monitoring configuration you can group service configurations into polling packages.
...
This provides great flexibility for how the service levels will be determined for a given device.
...
Instead of the `include-range` you can add one or more specific IP interfaces:
...
<1> Polling interval for all services in this polling package is reflected in the step size of 300 seconds.
    All services in this package must be polled in a five-minute interval, otherwise response time measurements are not persisted correctly.
...
With the ability to specify multiple polling packages you can use the same service (like ICMP) multiple times.
The order that polling packages are defined in the `poller-configuration.xml` is important when IP interfaces match multiple polling packages with the same service configuration.
...
If a poller is matched using its pattern, the parts of the service name that match the capture groups of the pattern are available as parameters to the <<service-assurance/configuration.adoc#ga-pollerd-configuration-meta-data, Metadata DSL>> using the context `pattern` and the capture group name as key.
...
By specifying a distinct service name for each instance, the services are identifiable, but there is no need to add a poller definition per service.
