...
<4> We derive `cpmCPUTotalName` in the target group, telling the extender to use the pointer-like property's value as an index into the source table, and specifying that we want to "pull over" the source attribute `entPhysicalName`. <5>
