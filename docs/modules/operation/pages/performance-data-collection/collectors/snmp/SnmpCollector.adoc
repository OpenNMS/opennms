...
| Configuration Files | $OPENNMS_HOME/etc/datacollection-config.xml and $OPENNMS_HOME/etc/datacollection/*.xml
|===
...
After creating a new datacollection config file, you need to add it to `etc/datacollection-config.xml` for inclusion in polling.
...
If you want to collect different OIDs based on a categories filter, you must define separate Collectd packages.
