...
| Configuration Files   | $OPENNMS_HOME/etc/xml-datacollection-config.xml and $OPENNMS_HOME/etc/xml-datacollection.d/*.xml
|===
...
3+|  *Required*
| collection             | The name of the XML Collection to use.  | n/a
3+| *Optional*
| handler-class          | Class that performs the collection.     | org.opennms.protocols.xml.collector.DefaultXmlCollectionHandler
|===
...
|===
| Parameter             | Description                                                                          | Default
| timeout               | The connection and socket timeout in milliseconds                                   | n/a
| retries               | How often to repeat the request in case of an error.                                 | 0
| use-system-proxy      | Should the system-wide proxy settings be used? Configure the system proxy
                            settings
                            via <<system-properties/introduction.adoc#system-properties, system properties>>.  | false
|===
