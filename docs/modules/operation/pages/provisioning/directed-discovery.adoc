...
The `$\{OPENNMS_HOME}/bin/send-event.pl` script can be used to generate newSuspect events to trigger the Provisiond service to check a given IP address.
...
NOTE: This will directly add the discovered node into the database.
It is strongly recommended to use <<requisition-add-nodes, requisitions>> when adding individual IP addresses, as it will be easier to manage the nodes over time.
