...
public class EventConfRestService implements EventConfRestApi {
    ...
    public Response uploadEventConfFiles(final List<Attachment> attachments, final SecurityContext securityContext) {
        ...
        Map<String, Attachment> fileMap = attachments.stream().collect(Collectors.toMap(a ->
                        a.getContentDisposition().getParameter("filename"),
                        a -> a, (a1, a2) -> a1, LinkedHashMap::new));
        ...
        for (final String fileName : orderedFiles) {
            ...
            try {
                eventConfPersistenceService.persistEventConfFile(fileEvents, new EventConfSourceMetadataDto.Builder()
                        .filename(fileName)
                        .eventCount(fileEvents.getEvents().size())
                        .fileOrder(fileOrder++).username(username)
                        .now(now)
    ...
    private List<String> determineFileOrder(final Attachment eventconfXmlAttachment, final Set<String> uploadedFiles) {
        ...
        if (eventconfXmlAttachment != null) {
            try (InputStream stream = eventconfXmlAttachment.getObject(InputStream.class)) {
                ...
                List<String> fromXml = fromXmlRaw.stream().map(path -> path.contains("/") ?
                        path.substring(path.lastIndexOf("/") + 1) : path).toList();

                // Identify files not listed in eventconf.xml
                List<String> extraFiles = uploadedFiles.stream().filter(f -> !fromXml.contains(f))
    ...
    private Map<String, Object> buildSuccessResponse(String filename, Events events) {
        ...
        List<Map<String, ? extends Serializable>> eventSummaries = events.getEvents().stream().map(e ->
                Map.of("uei", e.getUei(), "label", e.getEventLabel(), "description", e.getEventLabel(), "enabled", true))
                .collect(Collectors.toList());
