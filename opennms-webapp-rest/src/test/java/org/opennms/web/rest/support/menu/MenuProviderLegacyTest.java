...
public class MenuProviderLegacyTest {
    final static String RESOURCE_PATH = "src/test/resources/dispatcher-servlet.xml";

    @Test
    public void testParseBeansXml() {
        MenuProviderLegacy provider = new MenuProviderLegacy(getResourcePath(), "");
        provider.setMenuRequestContext(new MenuProviderLegacyTest.TestMenuRequestContext(true));
        ...
        List<MenuEntry> topMenuEntries = null;
    ...
    @Test
    public void testParseMainMenuLegacy() {
        ...
        Optional<MenuEntry> infoMenuOpt = mainMenu.menus.stream().filter(m -> m.name.equals("Info")).findFirst();
        assertTrue(infoMenuOpt.isPresent());

        MenuEntry infoMenu = infoMenuOpt.get();
        ...
        assertThat(deviceConfigMenuOpt.get().roles, containsInAnyOrder(expectedDeviceConfigRoles));
    }

    @Test
    public void testParseMainMenuLegacyZenithConnectDisabled() {
        MainMenu mainMenu = null;

        try {
            MenuProviderLegacy provider = new MenuProviderLegacy(getResourcePath(), "");
            provider.setMenuRequestContext(new MenuProviderLegacyTest.TestMenuRequestContext(false));
            mainMenu = provider.getMainMenu();
        } catch (Exception e) {
            Assert.fail("Error in MenuProvider.getMainMenu: " + e.getMessage());
        }
