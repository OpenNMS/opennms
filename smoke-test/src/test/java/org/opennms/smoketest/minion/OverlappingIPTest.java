...
public class OverlappingIPTest {

    private static final Logger LOG = LoggerFactory.getLogger(OverlappingIPTest.class);
    ...
    public void setup() {
        ...
        final InetSocketAddress pgsql = environment.getServiceAddress(ContainerAlias.POSTGRES, 5432);
    ...
    public void test() throws Exception {
        ...
        final RestClient client = new RestClient(environment.getServiceAddress(ContainerAlias.OPENNMS, 8980));
        ...
        sendSyslog(ContainerAlias.MINION);
        ...
        sendSyslog(ContainerAlias.MINION_OTHER_LOCATION);
        await().atMost(1, MINUTES).pollInterval(5, SECONDS)
               .until(DaoUtils.countMatchingCallable(this.daoFactory.getDao(EventDaoHibernate.class),
                                                     new CriteriaBuilder(OnmsEvent.class)
                                                             .eq("eventUei", "uei.opennms.org/vendor/cisco/syslog/SEC-6-IPACCESSLOGP/aclDeniedIPTraffic")
    ...
    private static void sendSyslog(ContainerAlias container) throws Exception {
        final InetSocketAddress syslogAddr = environment.getServiceAddress(container, 1514, "udp");

        final byte[] message = ("<190>Mar 11 08:35:17 " + HOST + " 30128311: Mar 11 08:35:16.844 CST: %SEC-6-IPACCESSLOGP: list in110 denied tcp 192.168.10.100(63923) -> 192.168.11.128(1521), 1 packet\n").getBytes();
        final DatagramPacket packet = new DatagramPacket(message, message.length, syslogAddr.getAddress(), syslogAddr.getPort());
