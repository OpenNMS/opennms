...
public class AlecPluginIT {
    ...
    public void everythingHappy() throws Exception {
        karafShell.runCommand("config:edit org.opennms.features.flows.persistence.elastic\n" +
                "config:property-set elasticUrl http://localhost:9200\n" +
                "config:update");
        karafShell.runCommand("config:edit org.opennms.features.alarms.history.elastic\n" +
                "config:property-set elasticUrl http://localhost:9200\n" +
                "config:update");
        karafShell.checkFeature("alec-opennms-standalone", "Started", Duration.ofSeconds(30));
    }
}
