...
public class MinionContainer extends GenericContainer<MinionContainer> implements KarafContainer<MinionContainer>, TestLifecycleAware {
    ...
    private void applyProfileConfiguration(MinionProfile profile) {
        if (!Strings.isNullOrEmpty(profile.getDominionGrpcScvClientSecret())) {
            addEnv("SCV_PROVIDER", "dominion");
            addEnv("DOMINION_GRPC_CLIENT_SECRET", profile.getDominionGrpcScvClientSecret());
        }
        ...
        if (IpcStrategy.KAFKA.equals(model.getIpcStrategy())) {
            ...
            addEnv("GRPC_IPC_PORT", "8990");
        }

        if (model.isJaegerEnabled()) {
            addEnv("JAEGER_ENDPOINT", JaegerContainer.getThriftHttpURL());
            addEnv("SYSTEM_PROP_JAEGER_ENDPOINT", JaegerContainer.getThriftHttpURL());
        }
    }
