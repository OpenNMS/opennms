...
public class OnmsOSGiBridgeActivator implements RegistrationHook, ServiceListener, BundleActivator {
    private AtomicReference<BundleContext> m_bundleContext = new AtomicReference<BundleContext>();
    ...
    private void registerWithOnmsRegistry(ServiceReference<?> reference) {
        final BundleContext bundleContext = m_bundleContext.get();
        if (bundleContext == null) {
            System.err.println("No BundleContext found, skipping registration of services: " + reference);
            return;
        }
        ...
        try {
            ...
            System.err.println("OnmsOSGiBridgeActivator: registering...");
            ...
            System.err.println("OnmsOSGiBridgeActivator: registered provider "+provider+" for interfaces: "+ Arrays.toString(providerInterfaces) +" with properties: " + properties);
        } catch (final ClassNotFoundException e) {
            System.err.println("OnmsOSGiBridgeActivator: Unable to find class used by exported OSGi service");
            e.printStackTrace();
        }
    }
