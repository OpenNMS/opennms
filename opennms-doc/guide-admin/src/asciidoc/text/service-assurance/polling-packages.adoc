...
There can be multiple capure groups inside of a pattern, but each must have a unique name.
Please not, that the _RegEx_ must be escaped XML to make it a valide attribute.

If a poller is matched using its pattern, the parts of the service name which matches the capture groups of the pattern are available as parameters to the link:#ga-pollerd-configuration-meta-data[Meta-Data-DSL] using the context `pattern` and the capute group name as key.
