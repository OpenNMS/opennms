...
If a poller is matched using its pattern, the parts of the service name which matches the capture groups of the pattern are available as parameters to the link:#ga-pollerd-configuration-meta-data[Metadata DSL] using the context `pattern` and the capture group name as key.

Examples:

`<pattern><![CDATA[^HTTP-(?<vhost>.*)$]]></pattern>`::
Matches all services which names starts with `HTTP-` followed by a host name.
If the services is called `HTTP-www.example.com`, the Metadata DSL expression `${pattern:vhost}` will resolved to `www.example.com`.
