...
| `brokerURL`      | The ActiveMQ Broker URL to connect to.                                                             | required | `vm://localhost?create=false&broker.persistent=false`
...
| `clientID`       | The client ID to use when connecting to the ActiveMQ broker.                                       | optional | `-`
|===
...
[source, xml]
----
<parameter key="brokerURL" value="failover://auto+ssl://192.168.1.1:61616/"/>
