...
The monitor class _DhcpMonitor_ is executed by _Pollerd_ and opens the background _Dhcpd_ daemon listening for incoming DHCP responses.
...
The _Dhcpd_ background daemon is only started if the _DhcpMonitor_ is used.
...
._Dhcpd_ parameters in `poller-configuration.xml`.
[options="autowidth"]
|===
| Parameter          | Description                                                                                  | Required | Default value
