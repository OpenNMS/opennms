...
To overwrite the default response time datasource add a entry into _times_ named _response-time_
...
Depending on the _run-type_ the script has to provid its results in diverent ways.
For minimal scripts with very simple logic _run-type_ _eval_ is the simple option.
Scripts runing in _eval_ mode have to return a string matching one of the Response Codes.

Every time your script is more then a one liner _run-type_ _exec_ is required.
Scripts runing in _exex_ mode have not to return anything, but they have to add a "status" entry with a "response code" to the _results_ object.
