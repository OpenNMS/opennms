...
The entries start with a _String_ that names the datasource and number that represents the response time.
...
Copy _jython-2.5.3.jar_ into the opennms lib folder and restart OpenNMS.
That makes Jython available for the BSFMonitor.

Jython Example PollerConfig with _run-type_ _exec_
[source, xml]
----
...
We have to use _run-type_ _exec_ here because Jython chokes on the _import_ keyword in _eval_ mode. Also note the substitution of Python's _None_ value for Java's _null_ in the log call.
