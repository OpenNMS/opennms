...
=== SSLCertMonitor
This monitor is used to test if a SSL certificate is valid.
Additionally it is possible to configure a threshold for monitoring the date of expire of the certificate.
...
WARNING: Monitoring of SSL certificates on virtual hosts is currently unsupported.
There is currently no possibility to pass or configure a HTTP header with a _host name_.
