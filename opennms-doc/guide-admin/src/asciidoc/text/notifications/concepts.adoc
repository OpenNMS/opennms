...
_Notifications_ are how _{page-component-title}_ informs users about an event that happened in the network, without the users having to log in and look at the UI.
...
As discussed in the chapter on <<ga-events>>, events are central to the operation of _{page-component-title}_.
...
_Users_ are entities with login accounts in the _{page-component-title}_ system.
...
_On-Call Roles_ are an overlay on top of groups, designed to enable _{page-component-title}_ to target the appropriate user or users according to a calendar configuration.
...
NOTE: Use external binary notification commands sparingly to avoid fork-bombing your _{page-component-title}_ system.
