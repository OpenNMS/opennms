...
TIP: Use the `opennms-rpc:stress` _Karaf shell_ command to help evaluate and tune performance.
...
To interactively test RPC communication with a remote location use the `opennms-poller:poll` command from the _Karaf shell_:

[source]
----
opennms-poller:poll -l LOCATION IcmpMonitor 127.0.0.1
----
