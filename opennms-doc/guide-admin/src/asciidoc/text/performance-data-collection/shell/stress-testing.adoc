...
The `opennms-metrics:stress` _Karaf Shell_ command can be used to simulate load on the active persistence strategy, whether it be `RRDtool`, `JRobin`, or `Newts`.
...
[source]
----
opennms-metrics:stress -n 10 -i 15 -r 30
----
