...
In order to support wildcard (*) objectname, JMX collector supports generic resource types.
Two changes needed to jmx configuration for this to work.

* Include a custom resource type in `etc/resource-types.d/`
for ex: there is already a jmx-resource.xml which defines a custom resource for kafka lag
...
----
* Match the resourceType name as keyfield in mbean definition

[source, xml]
----
<mbean name="org.opennms.core.ipc.sink.kafka.heartbeat" keyfield="kafkaLag" objectname="org.opennms.core.ipc.sink.kafka:name=OpenNMS.Sink.*.Lag">
