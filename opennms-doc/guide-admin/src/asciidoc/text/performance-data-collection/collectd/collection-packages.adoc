...
Collector packages in the _collectd-configuration.xml_ file contain the information (IP interfaces #should this be address or interface?#, services, and connection parameters) that collectd needs to activate data collection. 
...
----
#TODO: what does "remote" do? why is it there?#
...
|`include-url`|Specify a file that contains a list of #IP addresses to include. TODO: we use address and interface interchangeably. Please clarify.#|`<include-url>file:/opt/OpenNMS/etc/include</include-url>``
|===
...
<5> Assign performance data collection metric groups named `default`. #TODO: What does this mean?#
...
#TODO: Why do you want to use meta-data-dsl? Why is it important? Why is it included here?#
