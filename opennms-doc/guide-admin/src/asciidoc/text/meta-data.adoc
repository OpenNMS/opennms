{opennms-product-name} supports the assignment of arbitrary metadata to nodes, interfaces and services, for dynamic configuration of service monitoring, performance data collection, and service detection.
...
The only restriction is that each context/key/value must be unique in the element with which they are associated.

To view the metadata currently assigned to nodes, interfaces and services, open the details page associated with the node, interface, or service.

A link:#ga-meta-data-dsl[simple domain-specific language] (DSL) allows users to access the metadata associated with the elements they are working on.

=== Special contexts

A context defines the environment and current state of an object.
...
There can be zero or more fallback context-key pairs in a placeholder that will follow the primary context-key pair.
...
Which scopes are available depends on the environment for which an expression is evaluated and is documented in the corresponding places.
