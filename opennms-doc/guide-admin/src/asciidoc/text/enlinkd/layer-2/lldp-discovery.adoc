...
The following OIDs are supported to discover and build the _LLDP_ network topology and are collected by the _LLDP Discovery Collector_.
...
_Lldp Topology Updater_ provide LLDP OnmsTopology consolidating lldp data collected by _LLDP Collector_ only full bidirectional connections between two Lldp supported devices become _Edges_.
_Node A_  and _Node B_ are connected by an _LLDP Edge_ if and only if there is an LLDP MIB port connection in _Node A_ to _Node B_ and viceversa.
