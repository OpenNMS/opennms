...
.Asset tokens
A node may have additional asset meta data stored for it. You can access that metadata using the `asset` replacement token, which takes the form:

`%asset[<token>]%`::
    The asset field <token>'s value, or "Unknown" if it does not exist.

.Hardware tokens
A node may have additional hardware meta data stored for it. You can access that metadata using the `hardware` replacement token, which takes the form:
