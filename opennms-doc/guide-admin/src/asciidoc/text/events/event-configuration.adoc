...
The `opennms-events:show-event-config` command can be used to render the event definition for one or more event UEIs (matching a substring) to _XML_.
...
...
admin@opennms()> opennms-events:show-event-config -u uei.opennms.org/alarms
----
