...
Kafka producer also allows performance data (collections) to be persisted in Kafka.
It can be enabled as follows.
[source]
----
echo 'collection.persister=true' >> "$OPENNMS_HOME/etc/org.opennms.features.kafka.producer.client.cfg"
----

It also needs opennms-kafka-producer feature to be enabled. <<Enabling the Kafka Producer>>

This allows collection data to be persisted in kafka as well along with either `RRD` or `Newts` whichever is enabled by default.

===== Standalone Kafka Persister =====

Kafka persister can also work as standalone persister as alternative to `RRD` or `Newts`.

For this enable kafka persister as above and also configure below system property in `opennms.properties` and restart _{opennms-product-name}_ instance.
[source]
----
