...
The _Kafka Producer_ listens for changes made to the current set of topologies (bridge, cdp, isis, lldp and ospf) and forwards the resulting messages to _Kafka_ topics. A topic is defined for Topology Vertex Update Message and another topic is defined for Topology Edge Update Message.
