...
Kafka producer also allows performance data to be persisted in Kafka. It needs opennms-kafka-producer feature to be enabled. <<Enabling the Kafka Producer>>

.There are two ways you can enable Kafka Persister.

. As a separate time series strategy (as an alternative to `RRD` or `Newts`). For this add below system property in `opennms.properties`and restart _{opennms-product-name}_ instance.
+
[source]
----
...
----
+
. Allow performance data to be persisted in Kafka along with RRD or Newts. For this add below system property in `opennms.properties` and restart _{opennms-product-name}_ instance.
+
[source]
----
org.opennms.timeseries.kafka.persister=true
----
+
