...
In environments with large volumes of flows (> 10,000 flows/sec) the queries used to render the flow statistics used in the dhas render the  the storage requirements to retain flows and compute requirements to generate the statistics

the flow and compute requirements to generate the statis
The flow query engine supports rendering
Setup
