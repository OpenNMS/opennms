...
The goal of _Sentinel_ is to scale out and distribute individual components from _{page-component-title}_.
...
It is assumed, that the _Sentinel_ container is running on a different system than the _{page-component-title}_ and _Minion_.
...
This is required in order to have _Sentinel_ connect to the PostgreSQL database _{page-component-title}_.
...
<3> url which points to {page-component-title} (required)
<4> url which points to the {page-component-title} Active MQ Broker (only required if using feature `sentinel-jms`, otherwise may be omitted)
...
By default the _Sentinel_ consumes messages from the _{page-component-title} ActiveMQ Broker_.
...
In order to process flows via _Sentinel_ ensure that _{page-component-title}_, _Minion_ and _Sentinel_ are all installed according
...
==== Configure OpenNMS
_{page-component-title}_ must expose its _ActiveMQ Broker_ to have a _Minion_ and _Sentinel_ connect to it.
