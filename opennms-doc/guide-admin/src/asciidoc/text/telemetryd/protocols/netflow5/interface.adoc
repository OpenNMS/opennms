...
...
admin@minion()> config:edit org.opennms.features.flows.config
...
| _cache.nodeinfo.maxSize_
| Defines the maximum size of _node info objects_ to cache.
  A _node info object_ consists of `foreignSource`, `foreignId` and the `node categories` and is added to each flow identified by source and destination ip address of the flow.
| optional
| `1000`

| _cache.nodeinfo.expireAfter_
| Defines the time in seconds until each cached _node info object_ expires.
| optional
| `300 seconds`
...
The configuration properties will be stored in `${OPENNMS_HOME}/etc/org.opennms.features.flows.config.cfg`.
