...
It handles _BMP_ messages the <<telemetryd-bmp-parser, BMP Parser>> receives and parses, and creates _OpenBMP_-compatible messages, which are then
...
For this `opennms-telemetry-bmp-stats` feature need to be installed on {opennms-product-name}.
...
In order to ensure that the feature continues to be installed as subsequent restarts, add `opennms-telemetry-bmp-stats` to the `featuresBoot` property in the `${OPENNMS_HOME}/etc/org.apache.karaf.features.cfg`.


====== Configuring BMP persistence for external sources.

Following parameters can be configured when connecting to external sources like RPKI or RouteInfo DB tables.
...
Above parameters can be configured as following.
...
BMP Persisting Adapter can also be run on Sentinel. , _Sentinel_ must start appropriate BMP adapter.
In _Sentinel_ adapters are configured by either be placing a `.cfg` file in `${SENTINEL_HOME}/etc` or via `config:edit` statement.
...
Following features need to be installed to start processing BMP messages on Sentinel.
[source]
----
