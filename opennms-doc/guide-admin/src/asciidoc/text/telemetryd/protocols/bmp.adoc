...
The integration in _{page-component-title}_ allows you to use these route views, status updates and statistics for monitoring and management.
...
The router must exist as a regular node in _{page-component-title}_.

{page-component-title} uses the IP address exporting BMP messages to associate a router with the particular _{page-component-title}_ node.
...
It handles _BMP_ Peer Up and Down messages that the <<telemetryd-bmp-parser, BMP Parser>> receives and parses, and converts to _{page-component-title}_ events.
_{page-component-title}_ associates the created events with the router sending the messages.
This router must exist as regular node in _{page-component-title}_.
