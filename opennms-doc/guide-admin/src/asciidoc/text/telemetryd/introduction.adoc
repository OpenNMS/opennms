...
The resolved hostname, if found, is attached to the address information and both will be send the the _Adapter_ for further processing.
...
...
admin@minion()> system:property -p org.opennms.features.telemetry.enable true
admin@minion()> system:property -p org.opennms.features.telemetry.primaryServer
admin@minion()> system:property -p org.opennms.features.telemetry.secondaryServer
----
