...
After activating the Timeseries Integration Layer an actual implementation needs to be started.
This is done via Karaf.
Here is an example on how to activate the im memory plugin:
...
It is also highly recommended that resources stored in the _Timeseries Integration Layer_ are referenced by their foreign source and foreign ID, as opposed to their database ID.
