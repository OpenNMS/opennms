...
=== Web based JMX Configuration Generator

Generating complex _JMX_ data collection configurations can be generated from the a web based tool.
...
. Connect with _JMX_ or _JMXMP_ against a _MBean Server_ provided from a _Java_ application
...
At the beginning the connection to an _MBean Server_ of an _Java_ application has to be configured.
...
* _Host_: IP address or _FQDN_ connecting to the _MBean Server_ and load _MBeans_ and _Composite Data_ into the generation tool
* _Port_: Port to connect to the _MBean Server_
* _authenticate_: Enable / Disable authentication for _JMX_ connection with username and password
...
In the left sidebar shows the tree with the _JMX Domain_, _MBeans_ and _Composite Data_ hierarchy retrieved from the _MBean Server_.
To select or deselect all attributes use _Mouse right click -> select/deselect_.

The right panel shows the _MBean Attributes_ with the _RRD_ specific mapping and allows select or deselect specific _MBean Attriubtes_ or _Composite Data Attributes_ for the data collection configuration.
