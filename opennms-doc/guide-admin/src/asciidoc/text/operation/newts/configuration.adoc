...
_{page-component-title}_ can be configured to use Newts by setting the following property in in `${OPENNMS_HOME}/etc/opennms.properties`:
...
With these set, _{page-component-title}_ will begin persisting metrics using the _Newts_ engine when restarted.
...
Metadata related to resources are cached in order to avoid writing redundant records in _Cassandra_.
...
Cache priming is used to help reduce the number of records that need to be indexed after restarting _{page-component-title}_.
