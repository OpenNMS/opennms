...
This section describes the metrics _{page-component-title}_ collects for monitoring the _Newts_ keyspace from `org.apache.cassandra.metrics.Keyspace` on an _Cassandra_ node.

TIP: JMX must be enabled on the _Cassandra_ nodes and made accessible from __{page-component-title}_ in order to collect these metrics.
