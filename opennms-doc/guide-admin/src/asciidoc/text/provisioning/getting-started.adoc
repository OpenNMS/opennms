...
the range will be split into two separate ranges and a specific entry wil
...
This is very similar to the way how entities were previously handled by the (now obsolete) Capsd daemon but with finer grained sense of control.
