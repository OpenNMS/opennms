...
| Expression | ((loadavg5 / 100) / CpuNumCpus)| Divides the five-minute CPU load average by the number of CPUs. 
...
| Value| 7 | Trigger an event when the five-minute CPU load average goes above 70%. 

| Re-arm | 5 | Re-arm the system when the five-minute CPU load average drops below 50%
...
NOTE: #TODO: To determine datasources for other protocols, refer to#
