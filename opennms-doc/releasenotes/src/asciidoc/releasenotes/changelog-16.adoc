...
[[release-16-changelog]]
=== Changelog

[[releasenotes-changelog-16.0.4]]
==== Release 16.0.4

Release 16.0.4 is the latest stable release of OpenNMS.
It contains a number of bugfixes, including a change to help mitigate remote attacks over RMI.

As of this release, RMI listens only on localhost by default.
This can be overridden by setting the property `opennms.poller.server.serverHost=<hostname or IP>` in `opennms.conf` or `opennms.properties`.
