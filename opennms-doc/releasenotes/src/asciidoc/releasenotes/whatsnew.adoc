...
* `opennms:stress-dns` - stress-test the new internal DNS resolver
* `opennms:kv-get-blob` - print a record from the blob store
* `opennms:kv-put-blob` - inserts a string into the blob store
* `opennms:kv-benchmark-blob` - benchmark the internal OpenNMS blob store
* `opennms:kv-get-json` - print a record from the JSON store
* `opennms:kv-put-blob` - inserts a record into the JSON store
* `opennms:show-measurements` - show measurements data
* `opennms:show-newts-samples` - show raw sample data from Newts
* `opennms:show-measurement-resources` - show/filter the resource tree
* `opennms:delete-measurement-resources` - delete the measurements and meta-data for a resource
...
* `opennms:threshold-enumerate` - list threshold states
* `opennms:threshold-details` - show the details of a specific threshold state
* `opennms:threshold-clear` - clear a specific threshold state
* `opennms:threshold-clear-all` - reset all threshold states
