...
The `exporters` endpoints also support:

|===
| name               | default   | comment
| limit              | 10        | Limits the number of exporters (for summary queries) or the number of interfaces (for detailed queries) returned.

                                   In a case where the elements are truncated, those with the most flows will be returned.
|===
