...
IMPORTANT: On systems where `storeByForeignSource` is enabled, you must *always* use the `nodeSource` style when referring to requisitioned nodes and `node` when referring to auto-provisioned nodes. The following tables summarize which style is permissible in which situation. Auto-provisioned nodes are ones that do not belong to any requisition. As these nodes lack a foreign-source name, they intrinsically cannot be represented using `nodeSource` style identifiers.
