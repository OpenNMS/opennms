...
The `node[]` style cannot take a foreign-source:foreign-ID tuple. For example, it is not valid to use `node[Routers:atledg04]`. As a fallback mechanism, however, using `nodeSource[175]` will be internally translated to `node[175]`. It is therefore always safe to use the `nodeSource[]` style, which will simplify many client implementations.
