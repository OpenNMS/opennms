...
| `/config/syslog-nbi/status` | Get the status of the Syslog NBI (returns either true or false).
| `/config/syslog-nbi/destinations` | Get the name of all the existing destinations.
| `/config/syslog-nbi/destinations/{name}` | Get the content of the destination named {name}
|===
