...
| `vertex-status-provider`  | no       | Graph        | `string`  | -         | Defines which <<gd-topology-graphml-vertex-status-provider>> should be used.
...
There are three implementations available which can be configured for each graph: `default`, `script` and `propagate`.
...
In order to have a status calculated a (OpenNMS) node must be associated with the Vertex.
...
The `propagate` status provider follows all edges from a node to its connected nodes.
