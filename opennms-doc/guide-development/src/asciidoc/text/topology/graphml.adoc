...
Afterwards this is send to the {page-component-title} REST API to create it:
...
A various set of GraphML attributes are supported and interpreted by {page-component-title} while reading the GraphML file.
...
In order to have a status calculated a ({page-component-title}) node must be associated with the Vertex.
