...
{opennms-product-name} provides and publishes container images on link:https://hub.docker.com/u/opennms[DockerHub].

==== Objectives

* Run and configure a Minion in, and connect it to the {opennms-product-name} instance using environment variables
...
* a basic knowledge of _Docker_, _Docker Compose_ with networking, persisting files and mounting directories
* {opennms-product-name} configured to accept connections via _ActiveMQ_ and a Minion user with _ROLE_MINION_ (see <<gi-minion-setup-communication, Minion communication>>)
* Minion can connect to {opennms-product-name} with port `61616/TCP` for _ActiveMQ_ and _REST_ on port `8980/TCP` (see <<gi-minion-setup-communication, Minion communication>>)
