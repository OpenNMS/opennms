...
public abstract class SnmpTrapForwarderHelper extends AbstractEventForwarder implements
	...
	public SnmpTrapForwarderHelper(String ip, int port, String community) {
		this.ip = ip;
		this.port=port;
		this.community=community;		
	}
	...
	public void sendV1AlarmTrap( Event event, boolean sync) throws UnknownHostException {
		...
		trap = buildAlarmTrap(event, sync, trap, snmpTrapHelper);
        try {
	...
	public void sendV1EventTrap( Event event) throws UnknownHostException {
		...
		trap = buildEventTrap(event, trap, snmpTrapHelper);
        try {
			trap.send(getIp(), getPort(), getCommunity());
		} catch (Exception e) {
			e.printStackTrace();
	...
	private SnmpTrapBuilder buildAlarmTrap(Event event, boolean sync, SnmpTrapBuilder trap, SnmpTrapHelper snmpTrapHelper) {
		try {
             ...
             if (event.getLogmsg().getContent() != null) {
	...
	private SnmpTrapBuilder buildEventTrap(Event event, SnmpTrapBuilder trap, SnmpTrapHelper snmpTrapHelper) {
		try {
			 Integer t_dbid = new Integer(event.getDbid());
             if (t_dbid.intValue() > 0)
                     snmpTrapHelper.addVarBinding(trap, ".1.3.6.1.4.1.5813.20.1.1.0", "OctetString", "text", t_dbid.toString());
