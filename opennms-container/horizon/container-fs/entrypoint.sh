...
initOrUpdate() {
  if [[ -f "${OPENNMS_HOME}"/etc/configured ]]; then
    ...
    ${JAVA_HOME}/bin/java -Djava.endorsed.dirs="${OPENNMS_HOME}/lib/endorsed" -Dopennms.home="${OPENNMS_HOME}" -Dlog4j.configurationFile="${OPENNMS_HOME}"/etc/log4j2-tools.xml -cp "${OPENNMS_HOME}/lib/opennms_bootstrap.jar" org.opennms.bootstrap.InstallerBootstrap "${@}" || exit ${E_INIT_CONFIG}
  fi
}

configTester() {
  echo "Run config tester to validate existing configuration files."
  ${JAVA_HOME}/bin/java -Dopennms.manager.class="org.opennms.netmgt.config.tester.ConfigTester" -Djava.endorsed.dirs="${OPENNMS_HOME}/lib/endorsed" -Dopennms.home="${OPENNMS_HOME}" -Dlog4j.configurationFile="${OPENNMS_HOME}"/etc/log4j2-tools.xml -jar ${OPENNMS_HOME}/lib/opennms_bootstrap.jar "${@}" || exit ${E_INIT_CONFIG}
}
...
start() {
  echo "ADDITIONAL_MANAGER_OPTIONS=-Dorg.apache.jasper.compiler.disablejsr199=true \
  -Dgwt.geocoder.email=bamboo@opennms.org
  -Djava.endorsed.dirs=${OPENNMS_HOME}/lib/endorsed
  ...
  -XX:+StartAttachListener" > /opt/opennms/etc/opennms.conf
  echo "RUNAS=opennms" >> /opt/opennms/etc/opennms.conf
  exec "${OPENNMS_HOME}/bin/opennms" -f start
}
