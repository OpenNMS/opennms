    ...
    public PollStatus poll(MonitoredService svc, Map<String, Object> parameters) {
        ...
        try {
            ...
            if(flagResults.size() == 0) {
                log().debug("SNMP poll failed: no results, addr=" + hostAddress + " oid=" + prTableErrorSnmpObject);
                return PollStatus.unavailable();
            }

            for (Map.Entry<SnmpInstId, SnmpValue> e : flagResults.entrySet()) { 
                log().debug("poll: SNMPwalk poll succeeded, addr=" + hostAddress + " oid=" + prTableErrorSnmpObject + " instance=" + e.getKey() + " value=" + e.getValue());
