...
public class HostResourceSwRunMonitor extends SnmpMonitorStrategy {
    ...
    public PollStatus poll(MonitoredService svc, Map<String, Object> parameters) {
        ...
        PollStatus status = PollStatus.unavailable("HostResourceSwRunMonitor service not found, addr=" + ipaddr.getHostAddress() + ", service-name=" + serviceName);
        ...
        try {
            ...
            if (serviceName == null) {
                status.setReason("HostResourceSwRunMonitor no service-name defined, addr=" + ipaddr.getHostAddress());
		...
		return status;
            ...
            if (minServices > 0 && maxServices > 0 && minServices >= maxServices) {
                ...
                status.setReason("HostResourceSwRunMonitor " + reason + ", addr=" + ipaddr.getHostAddress() + ", service-name=" + serviceName);
        ...
        if (!minOk && maxServices < 0) { // failed min-services only
            status = logDown(Level.DEBUG, "HostResourceSwRunMonitor poll failed: service-count(" + matches + ") >= min-services(" + minServices + "), addr=" + ipaddr.getHostAddress() + ", service-name=" + serviceName);
        }
        if (!maxOk && minServices < 0) { // failed max-services only
            status = logDown(Level.DEBUG, "HostResourceSwRunMonitor poll failed: service-count(" + matches + ") <= max-services(" + maxServices + "), addr=" + ipaddr.getHostAddress() + ", service-name=" + serviceName);
        }
        if ((!minOk || !maxOk) && minServices > 0 && maxServices > 0) { // failed both (bad range)
            status = logDown(Level.DEBUG, "HostResourceSwRunMonitor poll failed: min-services(" + minServices + ") >= service-count(" + matches + ") <= max-services(" + maxServices + "), addr=" + ipaddr.getHostAddress() + ", service-name=" + serviceName);
        }
