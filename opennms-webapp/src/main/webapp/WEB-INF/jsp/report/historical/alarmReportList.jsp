   ...
   <c:choose>
    ...
    <c:otherwise>
     <c:if test="${currentPage gt 2 }">
      <a href="report/historical/alarmReportList.htm?page=1"> First </a>
     </c:if>
    </c:otherwise>
   </c:choose>
   ...
   <c:if test="${currentPage eq 1}">
    ...
    <c:if test="${noOfPages >3}">
     <c:forEach begin="${currentPage+1}" end="${currentPage+2}" var="i">
      ...
      <a href="report/historical/alarmReportList.htm?page=${i}">${i}</a>
   ...
   <c:if test="${currentPage eq noOfPages}">
    ...
    <c:choose>
     <c:when test="${noOfPages >3}">
      ...
      <a href="report/historical/alarmReportList.htm?page=${noOfPages-2}">${noOfPages-2}</a>
      <a href="report/historical/alarmReportList.htm?page=${noOfPages-1}">${noOfPages-1}</a>
     </c:when>
 ...
 <c:choose>
  ...
  <c:otherwise>
   <c:if test="${currentPage lt noOfPages-1 }">
    <a href="report/historical/alarmReportList.htm?page=${noOfPages}">Last</a>
   </c:if>
  </c:otherwise>
 </c:choose>
</c:if>
...
<c:choose>
	...
	<c:otherwise>
		<table border="1">
			<thead>
				<tr>
					<td colspan="2" id="o-repository-title">Alarm Historical
						Reports</td>
				</tr>
				...
				<c:forEach var="mapEntry" items="${alarmList}">
					<tr>
						<td><c:out value="${mapEntry.key}">
							</c:out></td>
						<td><a
							href="report/historical/alarmReportDownload.htm?filepath=${mapEntry.value}&filename=${mapEntry.key}">Save</a>&nbsp;</td>
