...
-2.1 opennms/etc/collectd-configuration.xml 
	...
	- MBG-KPIs - Only collect those stats which are used to calculate the bundled KPIs/reports (both PGW/SGW stats). 
		- Select this if you want to just see the bundled reports. 
	
	- MBG-PGW - collect all PGW stats from the devices 
		- Select this option if you would like to add a lot more reports than what is available by default for GGSN/PGWs.
		- Do not select MBG-KPIs package if you select this(will cause duplicates)

	- MBG-SGW - collect all SGW stats from the devices 
		- Select this option if you would like to add a lot more reports than what is available by default for SGWs
		- Do not select MBG-KPIs package if you select this(will cause duplicates)
...
3. All other configurations should be in place 
	...
	These have the details of the stats to be collected for the respective packages described above. The resource labels and tables are also defined in these files.
...
5. Restart opennms by typing service opennms restart or restart Junos
