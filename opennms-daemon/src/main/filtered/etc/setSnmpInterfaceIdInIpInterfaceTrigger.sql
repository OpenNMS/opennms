...
BEGIN
  IF NEW.snmpInterfaceId IS NULL AND NEW.ifIndex IS NOT NULL AND NEW.ifIndex > 0 THEN
     SELECT snmpif.id INTO NEW.snmpInterfaceId
       ...
       IF NOT FOUND THEN
          RAISE EXCEPTION ''IpInterface Trigger Exception: No SnmpInterface found for... nodeid: %  ipaddr: %  ifindex: %'', NEW.nodeid, NEW.ipAddr, NEW.ifIndex;
       END IF;
  END IF;
  RETURN NEW;
END;
