...
public class JmxDatacollectionConfiggenerator {
    ...
    public JmxDatacollectionConfig generateJmxConfigModel(
            MBeanServerConnection mBeanServerConnection, String serviceName,
            Boolean runStandardVmBeans, Boolean runWritableMBeans) {
    ...
    private String createAndRegisterUniceAlias(String originalAlias) {
        ...
        if (!aliasMap.containsKey(originalAlias)) {
            ...
            uniceAlias = aliasMap.get(originalAlias).toString()
                    + originalAlias;
        }
        ...
        if (aliasList.contains(NameTools.trimByCamelCase(uniceAlias, 19))) {
            logger.error("ALIAS CRASH AT :" + uniceAlias + "\t as: "
                    + NameTools.trimByCamelCase(uniceAlias, 19));
