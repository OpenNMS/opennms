...
public class HealthTrackingRestClient implements RestClient {
    ...
    private final CachingHealthCheck cachingHealthCheck;

    public HealthTrackingRestClient(RestClient delegate, CachingHealthCheck cachingHealthCheck) {
        this.delegate = delegate;
        this.cachingHealthCheck = cachingHealthCheck;
    }

    private <T> T callAndInformCachingHealthCheck(Callable<T> callable) throws Exception {
        try {
            var v = callable.call();
            cachingHealthCheck.setResponse(Response.SUCCESS);
            return v;
        } catch (Exception e) {
            cachingHealthCheck.setResponse(new Response(e));
            throw e;
        }
