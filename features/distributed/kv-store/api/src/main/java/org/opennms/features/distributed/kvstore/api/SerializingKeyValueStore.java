...
public final class SerializingKeyValueStore<V> {
    ...
    public SerializingKeyValueStore(KeyValueStore keyValueStore, Serializer<V> serializer,
    ...
    public long put(String key, V value, String context) {
        return keyValueStore.put(key, serializer.serialize(value), context);
    }

    public long put(String key, V value, String context, Integer ttlInSeconds) {
        return keyValueStore.put(key, serializer.serialize(value), context, ttlInSeconds);
    }

    public Optional<V> get(String key, String context) {
        return keyValueStore.get(key, context).map(deserializer::deserialize);
    }

    public Optional<Optional<V>> getIfStale(String key, String context, long timestamp) {
        return keyValueStore.getIfStale(key, context, timestamp).map(o -> o.map(deserializer::deserialize));
    }

    public OptionalLong getLastUpdated(String key, String context) {
        return keyValueStore.getLastUpdated(key, context);
    }

    public CompletableFuture<Long> putAsync(String key, V value, String context) {
        return keyValueStore.putAsync(key, serializer.serialize(value), context);
    }

    public CompletableFuture<Long> putAsync(String key, V value, String context, Integer ttlInSeconds) {
        return keyValueStore.putAsync(key, serializer.serialize(value), context, ttlInSeconds);
    }

    public CompletableFuture<Optional<V>> getAsync(String key, String context) {
        return keyValueStore.getAsync(key, context).thenApply(o -> o.map(deserializer::deserialize));
    }

    public CompletableFuture<Optional<Optional<V>>> getIfStaleAsync(String key, String context, long timestamp) {
        return keyValueStore.getIfStaleAsync(key, context, timestamp).thenApply(o -> o.map(ov -> ov.map(deserializer::deserialize)));
    }

    public CompletableFuture<OptionalLong> getLastUpdatedAsync(String key, String context) {
        return keyValueStore.getLastUpdatedAsync(key, context);
    }
