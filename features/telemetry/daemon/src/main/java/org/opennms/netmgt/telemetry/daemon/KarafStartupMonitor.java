 ...
 * TODO: patrick move this class to better place
 */
@EventListener(name= KarafStartupMonitor.NAME, logPrefix= KarafStartupMonitor.LOG_PREFIX)
public class KarafStartupMonitor implements SpringServiceDaemon {
    ...
    private boolean isKarafOk() {
        Class serviceClass = ReportStoreService.class;
        try {
            final ReportStoreService service = SERVICE_LOOKUP.lookup(serviceClass, null);
            return service != null;
        } catch (Exception e) {
    ...
    @Override
    public synchronized void destroy() { LOG.info("{} is stopped.", NAME);
    }
    ...
    public void handleReloadEvent(Event e) {
        DaemonTools.handleReloadEvent(e, KarafStartupMonitor.NAME, (event) -> handleConfigurationChanged());
    }
