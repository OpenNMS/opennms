...
public class Udp implements Listener.Factory {
    ...
    public Parser.Creator parser(final ParserDefinition parserDefinition) {
        return dispatcher -> Beans.buildParser(SimpleUdpParser.Factory.class,
                SimpleUdpParser.Factory::createUdpParser,
    ...
    public Listener create(final String name,
        ...
        if (parsers.size() != 1) {
            throw new IllegalArgumentException("The simple UDP listener supports exactly one parser");
        }
        final SimpleUdpListener listener = new SimpleUdpListener(name,
                (SimpleUdpParser) parsers.iterator().next());
