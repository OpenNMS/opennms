...
public class TelemetryAdapterRegistryImpl implements TelemetryAdapterRegistry, ServiceLookup<String, Void> {
    ...
    public Adapter getAdapter(AdapterDefinition adapterDefinition) {
        final AdapterFactoryRegistration registration = delegate.lookup(adapterDefinition.getClassName(), null);
        if (registration != null) {
            final Adapter adapter = registration.getAdapterFactory().createAdapter(adapterDefinition);
