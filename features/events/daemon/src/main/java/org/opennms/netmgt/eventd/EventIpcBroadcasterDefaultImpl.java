...
public class EventIpcBroadcasterDefaultImpl extends DefaultDispatcher implements EventIpcBroadcaster {
     ...
     * @param synchronous Whether this call should wait for all listeners
    ...
    public void broadcastNow(Event event, boolean synchronous) {
        ...
        if (event.getLogmsg() != null && event.getLogmsg().getDest().equals("suppress")) {
            ...
            try (Context context = logBroadcastTimer.time()) {
                // TODO: Figure out how to make this synchronous based
                // on the value of 'synchronous'
                m_proxy.broadcastNow(event, synchronous);
