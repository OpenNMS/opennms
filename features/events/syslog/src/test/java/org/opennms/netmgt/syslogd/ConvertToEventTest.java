...
public class ConvertToEventTest {
	...
	public void testConvertToEvent() throws MarshalException,
		...
		InputStream stream = ConfigurationTestUtils.getInputStreamForResource(
				this, "/etc/syslogd-loadtest-configuration.xml");
		config = new SyslogdConfigFactory(stream);

		// Sample message which is embedded in packet and passed as parameter to
		// ConvertToEvent constructor
		byte[] bytes = "<34> 2010-08-19 localhost foo10000: load test 10000 on tty1"
				.getBytes();

		// Datagram packet which is passed as parameter for ConvertToEvent
		// constructor
		DatagramPacket pkt = new DatagramPacket(bytes, bytes.length,
				InetAddress.getLocalHost(), SyslogClient.PORT);
		...
		try {
			ConvertToEvent convert=new ConvertToEvent(
					pkt.getAddress(), pkt.getPort(), data, config);
