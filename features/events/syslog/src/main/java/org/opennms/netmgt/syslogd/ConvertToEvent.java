 ...
 * likely to be made.
 * 
 * TODO: This class is sloooow. It needs to be sped up significantly to handle
 * increased syslog volume.
 *
...
public class ConvertToEvent {
	...
	private static final Logger LOG = LoggerFactory
			.getLogger(ConvertToEvent.class);
	...
	private static final Map<String, Pattern> CACHED_PATTERNS = Collections
	 ...
	 */
	public ConvertToEvent(final DatagramPacket packet,
			final SyslogdConfig config) throws UnsupportedEncodingException,
			MessageDiscardedException {
		this(packet.getAddress(), packet.getPort(), new String(
				packet.getData(), 0, packet.getLength(), "US-ASCII"), config);
	}
	 ...
	 * @throws MessageDiscardedException
	 */
	public ConvertToEvent(final InetAddress addr, final int port,
			final String data, final SyslogdConfig config)
		...
		SyslogParser parser = SyslogParser.getParserInstance(config,
				syslogString);
		if (!parser.find()) {
		...
		try {
			LOG.info("Parse method start call");
			message = parser.parse();
			LOG.info("Parse method calling end");
		...
		if (message == null) {
			throw new MessageDiscardedException(String.format(
					"Unable to parse '%s'", syslogString));
		}
		...
		EventBuilder bldr = new EventBuilder("uei.opennms.org/syslogd/"
				+ facilityTxt + "/" + priorityTxt, "syslogd");
		...
		if (hostAddress != null && hostAddress.length() > 0) {
			// Set nodeId
			long nodeId = SyslogdIPMgrJDBCImpl.getInstance().getNodeId(
					hostAddress);
			if (nodeId != -1) {
		...
		/*
		 * We matched on a regexp for host/message pair. This can be a forwarded
		 * message as in BSD Style or syslog-ng. We assume that the host is
		 * given to us as an IP/Hostname and that the resolver on the ONMS host
		 * actually can resolve the node to match against nodeId.
		 */
		...
		final List<UeiMatch> ueiMatch = ueiList == null ? null : ueiList
				.getUeiMatchCollection();
		if (ueiMatch == null) {
			LOG.warn("No ueiList configured.");
		} else {
			for (final UeiMatch uei : ueiMatch) {
				final boolean otherStuffMatches = matchFacility(
						uei.getFacilityCollection(), facilityTxt)
						&& matchSeverity(uei.getSeverityCollection(),
								priorityTxt)
						&& matchProcess(uei.getProcessMatch(),
								message.getProcessName())
						&& matchHostname(uei.getHostnameMatch(),
								message.getHostName())
						&& matchHostAddr(uei.getHostaddrMatch(),
								message.getHostAddress());
		...
		final List<HideMatch> hideMatch = hideMessage == null
				? null
		...
		if (hideMatch == null) {
			...
			for (final HideMatch hide : hideMatch) {
				if (hide.getMatch().getType().equals("substr")) {
					...
					try {
						msgPat = Pattern.compile(hide.getMatch()
								.getExpression(), Pattern.MULTILINE);
						msgMat = msgPat.matcher(fullText);
					} catch (PatternSyntaxException pse) {
						LOG.warn("Failed to compile regex pattern '{}'", hide
								.getMatch().getExpression(), pse);
						msgMat = null;
					}
	...
	private static boolean matchFind(final String expression,
			final String input, final String context) {
		final Pattern pat = getPattern(expression);
		if (pat == null) {
			LOG.debug(
					"Unable to get pattern for expression '{}' in {} context",
					expression, context);
			return false;
		}
	...
	private static boolean matchHostAddr(final HostaddrMatch hostaddrMatch,
		...
		if (matchFind(expression, hostAddress, "hostaddr-match")) {
			LOG.trace(
					"Successful regex hostaddr-match for input '{}' against expression '{}'",
					hostAddress, expression);
			return true;
		}
		return false;
	}

	private static boolean matchHostname(final HostnameMatch hostnameMatch,
		...
		if (matchFind(expression, hostName, "hostname-match")) {
			LOG.trace(
					"Successful regex hostname-match for input '{}' against expression '{}'",
					hostName, expression);
			return true;
		}
		return false;
	...
	private static boolean matchProcess(final ProcessMatch processMatch,
		...
		if (matchFind(expression, processName, "process-match")) {
			LOG.trace(
					"Successful regex process-match for input '{}' against expression '{}'",
	...
	private static boolean matchSeverity(List<String> severities,
			String priorityTxt) {
		if (severities.size() == 0)
			return true;
		for (String severity : severities) {
			if (severity.equalsIgnoreCase(priorityTxt))
				return true;
		}
		return false;
	}

	private static boolean matchFacility(List<String> facilities,
			String facilityTxt) {
		if (facilities.size() == 0)
			return true;
		for (String facility : facilities) {
			if (facility.equalsIgnoreCase(facilityTxt))
				return true;
		}
		return false;
	}
	...
	private static Pattern getPattern(final String expression) {
		...
		if (msgPat == null) {
			try {
				final Pattern newPat = Pattern.compile(expression,
						Pattern.MULTILINE);
				CACHED_PATTERNS.put(expression, newPat);
				return newPat;
			} catch (final PatternSyntaxException pse) {
				LOG.warn("Failed to compile regex pattern '{}'", expression,
						pse);
			}
		}
		return msgPat;
	...
	private static boolean matchSubstring(final String discardUei,
			final EventBuilder bldr, String message, final UeiMatch uei)
			throws MessageDiscardedException {
		...
		if (message.contains(uei.getMatch().getExpression())) {
			if (discardUei.equals(uei.getUei())) {
				if (traceEnabled)
					LOG.trace(
							"Specified UEI '{}' is same as discard-uei, discarding this message.",
							uei.getUei());
				...
				if (traceEnabled)
					LOG.trace(
							"Changed the UEI of a Syslogd event, based on substring match, to : {}",
							uei.getUei());
			...
			if (traceEnabled)
				LOG.trace(
						"No substring match for text of a Syslogd event to : {}",
						uei.getMatch().getExpression());
		}
		return doIMatch;
	}

	private static boolean matchRegex(final SyslogMessage message,
			final UeiMatch uei, final EventBuilder bldr, final String discardUei)
			throws MessageDiscardedException {
		boolean traceEnabled = LOG.isTraceEnabled();
		...
		if (msgPat == null) {
			LOG.debug("Unable to create pattern for expression '{}'",
					expression);
			return false;
		} else {
			final String text;
			...
			if (msgMat.groupCount() > 0) {
				if (uei.getMatch().isDefaultParameterMapping()) {
					...
					for (int groupNum = 1; groupNum <= msgMat.groupCount(); groupNum++) {
						...
						bldr.addParam("group" + groupNum,
								msgMat.group(groupNum));
					}
				}
				if (uei.getParameterAssignmentCount() > 0) {
					...
					for (ParameterAssignment assignment : uei
						...
						String parmValue = msgMat.group(assignment
								.getMatchingGroup());
		...
		if (traceEnabled)
			LOG.trace("Message '{}' did not regex-match pattern '{}'",
					message.getMessage(), expression);
		return false;
	}
	...
	public String toString() {
		return new ToStringBuilder(this).append("Event", m_event).toString();
	}
}
