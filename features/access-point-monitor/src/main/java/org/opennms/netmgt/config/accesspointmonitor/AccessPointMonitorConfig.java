...
/**
 * <p>AccessPointMonitorConfig class.</p>
 *
...
public class AccessPointMonitorConfig implements Serializable, Comparable<AccessPointMonitorConfig> {
	...
	public int hashCode() {
		...
		int result = 1;
		result = prime * result
				+ ((m_monitors == null) ? 0 : m_monitors.hashCode());
		result = prime
				* result
				+ (int) (m_packageScanInterval ^ (m_packageScanInterval >>> 32));
		result = prime * result
				+ ((m_packages == null) ? 0 : m_packages.hashCode());
		result = prime
				* result
				+ ((m_serviceTemplates == null) ? 0 : m_serviceTemplates
						.hashCode());
    ...
    public void updateServiceTemplates() {
		...
		for(Package p : m_packages) {
			...
			if( StringUtils.isNotBlank(s.getTemplateName()) ) {
				// The template name is set, try and associate it to the service
				if(serviceTemplateMap.containsKey(s.getTemplateName())) {
					s.setTemplate(serviceTemplateMap.get(s.getTemplateName()));
				}
			} else {
				// The template name is not set, try and find one with the service's name
