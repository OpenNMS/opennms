...
/**
 * Instance strategy for polling access-points:
 * 1) Walks the configured OID and uses the instance variable as the AP MAC address.
 * 2) Verifies the returned value against the configured criteria to determine if the AP is ONLINE or NOT.
 *
	...
	public OnmsAccessPointCollection call() throws IOException {
    	...
    	try {
            ...
            for (Map.Entry<SnmpInstId, SnmpValue> entry : map.entrySet()) {
                ...
                // If the criteria is met, find the AP and add it to the list of online APs
                if(isUp) {
