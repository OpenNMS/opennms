...
public class Package implements Serializable, Comparable<Package> {
    ...
    public Service getEffectiveService() {
    	...
    	// Inherit the attributes form the template, if these are not set in the template, then use default values
    	...
    	if( template == null ) return service;
    	
    	// Add parameters from the template to the service if they don't already exist
    ...
    public AccessPointPoller getPoller(List<Monitor> monitors)
    {
        for(Monitor monitor : monitors) {
            if(monitor.getService().compareToIgnoreCase(m_service.getName()) != 0) continue;
	...
	public int hashCode() {
		...
		int result = 1;
		result = prime * result
				+ ((m_excludeRanges == null) ? 0 : m_excludeRanges.hashCode());
		result = prime * result
				+ ((m_filter == null) ? 0 : m_filter.hashCode());
		result = prime * result
				+ ((m_includeRanges == null) ? 0 : m_includeRanges.hashCode());
		result = prime * result + (m_isDynamic ? 1231 : 1237);
		result = prime * result + ((m_name == null) ? 0 : m_name.hashCode());
		result = prime * result
				+ ((m_service == null) ? 0 : m_service.hashCode());
		result = prime * result
				+ ((m_specifics == null) ? 0 : m_specifics.hashCode());
		return result;
	}
