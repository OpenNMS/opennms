...
package org.opennms.netmgt.collection.persistence.timescale;
 ...
 * and persist these via the {@link org.opennms.netmgt.timescale.TimescaleWriter} on {@link #commit()}.
 *
 * @author jwhite
 */
public class TimescalePersistOperationBuilder implements PersistOperationBuilder {
    private static final Logger LOG = LoggerFactory.getLogger(TimescalePersistOperationBuilder.class);

    private final TimescaleWriter m_newtsWriter;
    ...
    public TimescalePersistOperationBuilder(TimescaleWriter newtsWriter, Context context, RrdRepository repository, ResourceIdentifier resource, String name) {
    ...
    public List<Sample> getSamplesToInsert() {
        ...
        TimescaleUtils.addIndicesToAttributes(path, m_metaData);
        Resource resource = new Resource(TimescaleUtils.toResourceId(path), Optional.of(m_metaData));
    ...
    public List<Sample> getSamplesToIndex() {
        ...
        for (Entry<ResourcePath, Map<String, String>> entry : m_stringAttributesByPath.entrySet()) {
            ...
            samples.add(TimescaleUtils.createSampleForIndexingStrings(m_context, resource));
        }
        return samples;
    }
