...
public class EventCountDaoHibernate extends AbstractDaoHibernate<OnmsEvent, Integer> implements EventCountDao {
    ...
    public Set<CountedObject<String>> getUeiCounts(final Integer limit) {
        ...
        Query queryObject = sessionFactory.getCurrentSession().createQuery("SELECT event.eventUei, COUNT(event.eventUei) FROM OnmsEvent event GROUP BY event.eventUei ORDER BY COUNT(event.eventUei) desc");
