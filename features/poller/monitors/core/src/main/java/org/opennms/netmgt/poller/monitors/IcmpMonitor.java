...
public class IcmpMonitor extends AbstractServiceMonitor {
    ...
    public PollStatus poll(MonitoredService svc, Map<String, PollerParameter> parameters) {
        ...
        try {
            ...
            // get parameters
            //
            int retries = getKeyedInteger(parameters, "retry", PingConstants.DEFAULT_RETRIES);
            long timeout = getKeyedLong(parameters, "timeout", (long) PingConstants.DEFAULT_TIMEOUT);
            int packetSize = getKeyedInteger(parameters, "packet-size", PingConstants.DEFAULT_PACKET_SIZE);
