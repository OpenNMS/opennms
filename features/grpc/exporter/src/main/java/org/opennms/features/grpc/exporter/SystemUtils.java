...
public class SystemUtils {
    public static final Logger LOG = LoggerFactory.getLogger(SystemUtils.class);
    ...
    public static Properties getONMSSystemProperties() {
        Properties systemProperties = new Properties();
        try {
            Properties properties = new Properties();
            properties.load(SystemUtils.class.getResourceAsStream("/grpc-filtered.properties"));
            String opennmsHome = properties.getProperty("install.dir");
            systemProperties = loadSystemProperties(opennmsHome);
    ...
    public static Properties loadSystemProperties(String opennmsHome) {
    ...
    private static void loadProperties(String path, Properties onmsProperties) {
        ...
        if (fileOrDir.isFile() && path.endsWith(".properties")) {
            ...
            Optional.ofNullable(fileOrDir.listFiles((dir, name) -> name.endsWith(".properties")))
                    .map(Arrays::stream)
                    .orElse(Stream.empty())
                    .forEach(file -> SystemUtils.loadSingleFile(file, onmsProperties));
        } else {
