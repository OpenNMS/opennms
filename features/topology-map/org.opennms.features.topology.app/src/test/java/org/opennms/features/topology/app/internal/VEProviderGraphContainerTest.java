...
//			in a GraphProviderAccessor like so:
//
//			VEProviderGraphContainer { GraphProviderAccessor { VertexHopGraphProvider } } }
//
//			But for the VertexHopProvider to calculate the SZL correctly, it needs to be aware of all
//			edges, including those provided by the GraphProviderAccessor. So we should rearrange things
//			so that they are laid out like:
//
//			VEProviderGraphContainer { VertexHopGraphProvider { GraphProviderAccessor } } }
//
//			We should decouple the GraphProviderAccessor from the VEProviderGraphContainer and then just
...
//		);
//		assertEquals(
//			ArrayUtils.toString(m_graphContainer.getTopologyServiceClient().getVertices(new TestCriteria1())),
//			3,
//			m_graphContainer.getTopologyServiceClient().getVertices(new TestCriteria1()).size()
//		);
//
//		expectVertex("nodes", "v1", "vertex");
//		expectVertex("nodes", "v3", "vertex");
