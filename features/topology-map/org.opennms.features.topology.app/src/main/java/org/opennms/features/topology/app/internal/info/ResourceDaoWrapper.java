...
/**
 * A read-only wrapper around {@link ResourceDao} designed to be placed into
 * the Jinjava template context used by {@link GenericInfoPanelItemProvider}.
 *
 * <p>Only query/lookup methods are delegated. The mutating method
 * {@code deleteResourceById} is deliberately <b>not</b> exposed.</p>
 *
...
public class ResourceDaoWrapper {
     ...
     *
     * @param id the resource identifier (e.g. {@code node[1].nodeSnmp[]})
    ...
    /**
     * Retrieve the resource tree rooted at the given node.
     *
     * @param node the node whose resources to retrieve
     * @return the node-level {@link OnmsResource} (children populated)
     */
