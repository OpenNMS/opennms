...
public class CdpTopologyProvider extends AbstractTopologyProvider implements GraphProvider {

    private static Logger LOG = LoggerFactory.getLogger(CdpTopologyProvider.class);
    ...
    public CdpTopologyProvider(MetricRegistry registry) {
        ...
        m_loadFullTimer = registry.timer(MetricRegistry.name("cdp", "load", "full"));
        m_loadCdpLinksTimer = registry.timer(MetricRegistry.name("cdp", "load", "links", "cdp"));
        m_loadVerticesTimer = registry.timer(MetricRegistry.name("cdp", "load", "vertices", "none"));
        m_loadEdgesTimer = registry.timer(MetricRegistry.name("cdp", "load", "edges", "none"));
    }
    ...
    private void loadTopology() {
        ...
        Timer.Context context = m_loadCdpLinksTimer.time();
        try{
            loadTopology(ProtocolSupported.CDP);
            LOG.info("loadEdges: CdpTopology loaded");
        }  catch (Exception e){
