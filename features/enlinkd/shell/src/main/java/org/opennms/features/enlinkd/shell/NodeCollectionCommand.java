...
public class NodeCollectionCommand implements Action {
    ...
    @Override
    public Void execute() {
        boolean runned = reloadableTopologyDaemon.runSingleSnmpCollection(node, protocol);
        if (runned) {
            System.out.printf("protocol %s, node %s found and collection is runned", protocol, node);
        } else {
            System.out.printf("protocol %s , or node %s not found and collection is not runned", protocol, node);
        }
        return null;
    }
