...
public class GPath<O> {
    ...
    public GPath<O> field(String name, String altName) {
        ...
        return new GPath<O>(je -> {
            if (je.isJsonObject()) {
                ...
                val j = jo.get(name);
                val j2 = j != null ? j : jo.get(altName);
                if (j2 != null) {
                    return eval(j2);
                }
            }
    ...
    public GPath<List<O>> array(String name) {
        return new GPath<List<O>>(je -> {
            if (je.isJsonObject()) {
                val array = je.getAsJsonObject().getAsJsonArray(name);
                if (array != null) {
                    val list = new ArrayList<O>();
                    je.getAsJsonObject().getAsJsonArray(name).forEach(j -> list.add(eval(j)));
                    return list;
                }
            }
            return null;
