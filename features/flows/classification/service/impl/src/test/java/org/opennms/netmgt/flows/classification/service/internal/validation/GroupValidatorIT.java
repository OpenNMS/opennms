...
public class GroupValidatorIT {
    ...
    public void verifyValidation() {
        ...
        ValidatorTestUtils.verify(() -> groupValidator.validate(userGroup, null));
        ValidatorTestUtils.verify(() -> groupValidator.validate(systemGroup, null));

        // Group with a rule should succeed
        final Rule rule = createDummyRule();
        ValidatorTestUtils.verify(() -> groupValidator.validate(userGroup, rule));
        userGroup.addRule(rule);

        // Same rule different group, should still succeed
        final Rule rule2 = createDummyRule();
        ValidatorTestUtils.verify(() -> groupValidator.validate(systemGroup, rule2));
        systemGroup.addRule(rule2);

        // Adding the same rule (even with different name) should fail
        Rule rule3 = createDummyRule();
        rule3.setName("xxx");
        ValidatorTestUtils.verify(() -> groupValidator.validate(userGroup, rule3), Errors.GROUP_DUPLICATE_RULE);
    }
