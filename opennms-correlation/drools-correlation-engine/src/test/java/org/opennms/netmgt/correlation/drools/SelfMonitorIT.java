...
public class SelfMonitorIT {

    @Autowired
    private MockEventIpcManager eventIpcMgr;

    @Autowired
    private CorrelationEngineRegistrar mockCorrelator;

    @Test(timeout=120000)
    public void canSendSystemHealthSummaryEvents()  {
        CorrelationEngine engine = mockCorrelator.findEngineByName("selfMonitorRules");
        assertNotNull("engine must exist", engine);
    ...
    private void anticipate(Event event) {
        eventIpcMgr.getEventAnticipator().anticipateEvent(event);
    }

    public Callable<Boolean> allAnticipatedEventsWereReceived() {
        return () -> eventIpcMgr.getEventAnticipator().getAnticipatedEvents().isEmpty();
    }
