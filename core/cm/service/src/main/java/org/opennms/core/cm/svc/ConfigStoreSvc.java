...
public class ConfigStoreSvc implements ConfigStore {
    private final SchemaManager schemaManagerSvc;
    private final Map<String,Object> objectStore = new ConcurrentSkipListMap<>();

    public ConfigStoreSvc(SchemaManager schemaManagerSvc) {
        this.schemaManagerSvc = Objects.requireNonNull(schemaManagerSvc);
    }

    @Override
    public <T> Optional<T> getModel(String service, Class<T> clazz) {
        if (!schemaManagerSvc.hasSchemaForService(service)) {
