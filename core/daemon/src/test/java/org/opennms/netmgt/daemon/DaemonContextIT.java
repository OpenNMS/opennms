...
public class DaemonContextIT {
	...
	*/
	...
	public void canUseEmbeddedActiveMQBroker() throws Throwable {
	    final QueueConnection connection = activeMQConnectionFactory.createQueueConnection();
	    final QueueSession session = connection.createQueueSession(false, QueueSession.AUTO_ACKNOWLEDGE );
	    final TextMessage message = session.createTextMessage();
	    message.setText("ping");
	    final QueueSender sender = session.createSender(session.createQueue("pong"));
	    sender.send(message);
	}
