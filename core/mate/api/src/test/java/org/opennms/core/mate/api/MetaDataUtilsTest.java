...
public class MetaDataUtilsTest {
    ...
    public void testJexl() {
        final Interpolator.Result result = Interpolator.interpolate("$(3*mate(\"${ctx7:key1}\"))", new MapScope(Scope.ScopeName.NODE, this.metaData));
        assertEquals("126", result.output);
    }

    @Test
    public void testToString() throws MateParserException {
        final String expression = "foo-${aaa}-bar-${ctx1:key1|down}-bla-${ctx2:key4|down}-blupp-${bbb}+$(3*mate(\"${ctx7:key1}\"))";
        assertEquals("foo-${aaa}-bar-${ctx1:key1|down}-bla-${ctx2:key4|down}-blupp-${bbb}+$(3*mate(\"${ctx7:key1}\"))", MateExpression.of(expression).toString());
    }
}
