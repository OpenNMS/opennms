...
public class MockSnmpValueFactory implements SnmpValueFactory {
	...
	SnmpValue parseMibValue(String mibVal) {
	    ...
	    if (mibVal.startsWith("Wrong Type")) {
	        ...
	        LogUtils.errorf(this, "Bad Mib walk has value: '%s' using '%s'", mibVal, newVal);
	        mibVal = newVal;
	    }
	    ...
	    if (mibVal.startsWith("OID:")) {
	        ...
	        return getIpAddress(addr(mibVal.substring("IpAddress:".length()).trim()));
	        ...
	        if (trimmed.matches("^.*[ :].*$")) {
	            ...
	            if (trimmed.length() % 2 != 0) {
	                LogUtils.warnf(this, "Hex-STRING %s does not have ' ' or ':' separators, but it is an uneven number of characters.", trimmed);
	            }
