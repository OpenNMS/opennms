...
public class PingThreadTest {
    ...
    public void testMultiThreadSocketUse() throws Exception {
        ...
        listener.ping((Inet4Address)InetAddress.getByName("127.0.0.1"), 1000, 0, 1234, pingCount, 1000);
    ...
    private Thread pingThead(final AbstractPinger<Inet4Address> listener, final int id, final int count) {
        return new Thread() {
            public void run() {
                try {
                    Thread.sleep(id/10);
                    listener.ping((Inet4Address)InetAddress.getByName("127.0.0.1"), id, 0, 1234, count, 1000);
                } catch(Throwable e) {
                    e.printStackTrace();
                }
            }
