...
{
	/**
	 * The queue where runnable objects are
	 * added.
	 */
	private SizingFifoQueue	m_delegateQ;

	/**
	 * The list of running fibers in the pool. The
	 * list allows the size of the pool to vary.
	 */
	...
	/**
	 * The thread group that all pool threads
	 * belong to.
	 */
	private ThreadGroup	m_tGroup;

	/**
	 * The log4j prefix used when starting
	 * up a new fiber!
	 */
	...
	/**
	 * The set of listeners to call when a
	 * Runnable completes successfully.
	 */
	private List		m_completedListeners;

	/**
	 * The set of listeners to call when a
	 * Runnable fails to complete successfully.
	 */
	private List		m_errorListeners;

	/**
	 * <p>This class is used to create a queue that auto adjust
	 * the number of threads in the pool. Each time an addition
	 * or removal of queue elements occur the pool will be
	 * adjusted. This will cause some synchronization overhead, 
	 * but it should be correct implementation.</p>
	 *
	 * <p>To avoid the condition of toggleing a single thread the
	...
	{
		/** 
		 * Determines if the queue is open or closed. If the
		 * queue is closed then an exception is thrown on 
		 * queue additions. Also, queue removals will cause
		 * a queue exception if the queue is empty.
		 */
		private volatile boolean m_isClosed = false;

		/**
		 * Adjust the size of the thread pool based on the
		 * ratio of queue elements to threads. The thread
		 * pool is adjusted by the lo and hi water marks which
		 * are a ratio of elements to threads.
		 */
		private void adjust()
		{
			int e = size();
		...
		/**
		 * Closes a currently open queue. When a queue is 
		 * closed is should still allow elements already in
		 * the queue to be removed, but new elements should
		 * not be added.
		 *
		 * @exception org.opennms.core.queue.FifoQueueException Thrown
		 *	if an error occurs closing the queue.
		 */
		public void close()
		...
		/**
		 * Ensures that the queue is open and new
		 * elements can be added to the queue.
		 *
		 * @exception org.opennms.core.queue.FifoQueueException Thrown
		 *	if an error occurs opening the queue.
		 */
		public void open()
			throws FifoQueueException
		{
	...
	/**
	 * This class implements the {@link org.opennms.core.fiber.Fiber Fiber} interface
	 * on top of a Java {@link java.lang.Thread Thread} instance. These fibers are
	 * the basic unit of work in the pool structure. Each fiber reads from the 
			   ...
			   Runnable
	{
		...
		/**
		 * if set true then the thread should exist as soon
		 * as possible.
		 */
		...
		/**
		 * Constructs a new fiber thread and adds the instance
		 * to the pool.
		 *
	...
	{
		/** 
		 * This method is invoked when a {@link java.lang.Runnable
		 * Runnable} complete.
		 *
		 * @param r The completed Runnable.
		 */
		public void onRunnableCompletion(Runnable r);
	}

	/**
	 * This interface is used to define a listerer for the thread pool
	 * that is notified if an error occurs processing a runnable. The
	 * instance is passed as an object just incase the actually object
	...
	public static interface RunnableErrorListener
	{
		/** 
		 * This method is invoked when a {@link java.lang.Runnable
		 * Runnable} fails with an exception.
		 *
		 * @param r The Runnable in error.
		 * @param t The caught error.
		 */
	 ...
	 * A living fiber is any fiber that is not currently in a 
	 * Stopped state. If the fiber is stopped then its entry
	 * will be removed from the internal array to allow for
	 * garbage collection.
	 *
	...
	/**
	 * <p>Constructs a new instance of the thread pool class. The thread
	 * pool consumes {@link java.lang.Runnable Runnable} instances from the
	 * input queue, calling the <code>run</code> method as they are consumed.</p>
	 *
	 * <p>The size of the thread pool is controlled by the low and high water
	 ...
	 * of threads are reduced.</p>
	 *
	 * @param name		The name of the thread pool.
	 * @param loMark	The lower ratio used to mark thread reduction.
	 * @param hiMark	The high ration used to mark thread production.
	 * @param max		The maximum number of threads in the pool.
	 *
	 * @exception java.lang.IllegalArgumentException Thrown if the low or high
	 * 	marks are invalid, or the maximum number of threads is invalid.
	 */
	...
	/**
	 * Starts the thread pool. The first thread is
	 * delayed until at least one element is added to
	 * the input queue.
	 */
	public void start()
	{
	 ...
	 * The status is set to stop pending and each thread
	 * is notified of the new status.
	 */
	public void stop()
	{
		synchronized(m_fibers)
	...
	/**
	 * Adds a listener that is notified upon completion
	 * of each runnable.
	 *
	 * @param listener The listener notified on runnable completion.
	 */
	...
	/**
	 * Adds a listener that is notified of error from
	 * any runnable.
	 *
	 * @param listener The listener notified on runnable completion.
	 */
