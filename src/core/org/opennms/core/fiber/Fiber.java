 ...
 * {@link java.lang.Thread Thread} class. The <code>Fiber</code> class is
 * used to define a working <em>context</em>, which is the basic feature 
 * of a <code>Thread</code>. The differences end there since the <code>Thread</code>
 * class is part of the core language and can only be extended since it is
 * a concrete class.</p>
 *
 * <p>The <code>Fiber</code> concept is used to represent an implementation 
 ...
 * classes.</p>
 *
 * <p>For example, the <code>Fiber</code> interface could be used to represent
 * a one-to-one mapping of a Java <code>Thread</code>. It could be used to 
...
{
	/**
	 * The string names that correspond to the states
	 * of the fiber.
	 */
	...
	/**
	 * This is the initial <code>Fiber</code> state. When
	 * the <code>Fiber</code> begins it startup process it
	 * will transition to the <code>STARTING</code> 
	 * state. A <code>Fiber</code> in a start pending state
	 * has not begun any of the initilization process.
	 */
	public static final int START_PENDING	= 0;

	/**
	 * This state is used to define when a <code>Fiber</code>
	 * has begun the initilization process. Once the initilization
	 * process is completed the <code>Fiber</code> will transition
	 * to a <code>RUNNING</code> status.
	 */
	public static final int STARTING	= 1;

	/**
	 * This state is used to define the normal runtime condition
	 * of a <code>Fiber</code>. When a <code>Fiber</code> is in
	 * this state then it is processing normally.
	 */
	public static final int RUNNING		= 2;

	/**
	 * This state is used to denote when the <code>Fiber</code>
	 * is terminating processing. This state is always followed
	 * by the state <code>ST0PPED</code>.
	 */
	public static final int STOP_PENDING	= 3;

	/**
	 * This state represents the final resting state of a <code>Fiber</code>.
	 * Depending on the implementation it may be possible to 
	 * resurect the <code>Fiber</code> from this state.
	 */
	...
	/**
	 * This method is used to return the name of the <code>Fiber</code>.
	 ...
	 * of the <code>Fiber</code> interface, or from one of the derived 
	 * interfaces.
	 *
