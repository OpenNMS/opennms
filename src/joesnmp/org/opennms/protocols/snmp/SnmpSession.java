...
public class SnmpSession extends Object
{
	...
	/**
	 * This is the saved fatal excetion that 
	...
	/**
	 * Adds an outstanding request to the session. The
	 * access to the list is synchronized on the actual
	 * list of request. This is done to allow synchronization
	 * between addRequest(), removeRequest(), and findRequest()
	 *
	 * @param req The request reference to add (not cloned)
	 */
	...
	/**
	 * Removes an outstanding request from the session. The
	 * method uses the Object.equals() to find a matching request.
	 * If the SnmpRequest object does not override the equals() 
	 * method the a "by reference" equality is used.
	 *
	 * @param req The request to remove. All matching request are removed.
	 */
	...
	/**
	 * Finds the first matching request in the list of outstanding
	 * request and returns it to the caller. The matching is done
	 * by means using the SNMP Protocol Data Unit's request id. If
	 * no match is found then a null is returned
	 *
	 * @param pdu The source pdu for the search.
	 * 
	 * @return Returns a SnmpRequest if a match is found. Otherwise a
	 *	null is returned.
	 *
	 */
		...
		synchronized(m_requests)
		{
			if(m_requests.size() > 0)
			{
				...
				while(iter.hasNext())
				{
					SnmpRequest req = (SnmpRequest) iter.next();
					if(!req.m_expired  
					   && req.m_pdu instanceof SnmpPduPacket  
