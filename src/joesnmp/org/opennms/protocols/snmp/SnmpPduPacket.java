 ...
 * The class defines methods to handle most v1 and v2 implementation
 * of SNMP with only minor work needed by the derived class.
 *
 * @see		SnmpPduRequest
 * @see		SnmpPduBulk
 *
 ...
 *
 */
public abstract class SnmpPduPacket extends Object
	implements SnmpSyntax, Cloneable
{
	/**
	 * The static variable for the class. This variable
	 * is used to get a "unique" sequence number for
	 * each PDU
	 */
	private static int	sm_seq = 0;

	/**
	 * The SNMP command for the pdu. See
	 * the list of command later on in the module
	 ...
	 * PDU. The list may be quite large so long
	 * as the packet can be received by the remote
	 * appliction
	 */
	private ArrayList	m_variables;		// from pdu

	/**
	 * The error status in a normal pdu, is
	 * is used as the non-repeaters in the getbulk.
	 * 
	 * @see SnmpPduRequest
	 * @see SnmpPduBulk
	 */
	...
	/**
	 * The error index in a normal pdu, it is
	 * used as the maximum repititions in 
	 * the get bulk pdu.
	 *
	 * @see SnmpPduRequest
	 * @see SnmpPduBulk
	...
	/**
	 * Default class constructor. Initialzies all
	 * primitive members to zero, and allocates a 
	 * new array list for the variables.
	 *
	 */
	...
	/**
	 * Class copy constructor. Constructs the object with all
	 * the same values as the passed packet. The variables are
	 * duplicated into a new array so that changes to the
	 * source pdu will not affect the newly create pdu.
	 *
	 * @param second The source pdu to copy values from.
	 *
	 */
	...
	/**
	 * creates a new pdu with the command set to the
	 * passed value.
	 *
	 * @param command	The type of pdu packet.
	 *
	 */
	...
	/**
	 * Creates a new pdu with the spcified command and the 
	 * list of variables.
	 *
	 * @param command	The type of pdu packet.
	 * @param vars		The variable list for the pdu.
	 *
	 */
	...
	/**
	 * Use to sequence the all pdu request across the
	 * entire library. If the sequence id is equal to
	 * zero then a random number generator is created and
	 * is used to seed the sequence.
	 *
	...
	/**
	 * Defines a SNMPv1 PDU Set Request message. The set
	 * request uses the write-only string from the
	 * session. All others use the read-only community
	 * string.
	 */
	...
	//
	/**
	 * No error occured in the request. Also known as
	 ...
	 * The variable is added at the end of the list
	 *
	 * @param vb The new variable to add 
	 */
	 ...
	 *
	 * @param ndx The index of the variable
	 * @param vb  The new variable.
	 *
	 */
	...
	/**
	 * Sets the specific variable at the requested 
	 * location.
	 *
	 * @param ndx	The location to set
	 * @param vb	The new variable
	 *
	 */
	 ...
	 *
	 * @param ndx	The index of the variable to remove
	 *
	...
	/**
	 * Returns a list of all the variables managed by
	 * this protocol data unit.
	 *
	 ...
	 * of self. This method not defined.
	 *
	 */
