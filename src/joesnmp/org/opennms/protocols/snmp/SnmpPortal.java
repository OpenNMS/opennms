	...
	extends Object
{
	 ...
	 * method in order to keep from blocking indefinitely on a socket I/O call.  This value
	 * is configurable at runtime via the system property
	 * "org.opennms.joeSNMP.vmhacks.socketSoTimeoutRequired".
	 * If this property is set to 'no', the bSocketSoTimeoutRequired variable will be set to
	 * false and the SNMP trap socket timeout will not be set.
	 * If this property is set to 'yes' or the property does not exist, the bSocketSoTimeoutRequired
	 * variable will be set to true. and the socket timeout will be set.
		...
		throws SocketException
	{
		...
		// method to set the socket timeout value thereby mimic'ing non-blocking socket I/O.
		// On platforms whose system close() is not preemptive it is necessary to use the socket timeout
		// to keep from blocking indefinitely on any socket call that performs I/O.
		//
		bSocketSoTimeoutRequired = true; // Default is to use set the socket timeout
	...
	private class Receiver implements Runnable
	{
		/**
	 	 ...
	 	 * to 64k. If it fails then the default buffer size
	...
	/**
	 * Returns true if this portal has had it's <CODE>close</CODE>
	 * method called.
	 *
	 */
	boolean isClosed()
