 ...
 * is received from an agent. In addition, if an internal 
 * error occurs or an agent fails to respond then the
 * object must handle those error conditions.<P>
 *
 * <P>For error conditions the pdu is recast to an SnmpSyntax
 * object. This is mainly due to the fact that the SnmpPduTrap
 * is not derived from SnmpPduPacket. Implementations of the 
...
public interface SnmpHandler
{
	 ...
	 *
	 */
	void snmpReceivedPdu(SnmpSession	session,
			     int		command,
			     SnmpPduPacket	pdu);

	/**
	 * <P>This method is invoked when an internal error occurs
	 * for the session. To determine the exact error the
	 * err parameter should be compared with all the error
	 * conditions defined in the SnmpSession class.</P>
	 *
	 ...
	 *
	 */
	void snmpInternalError(SnmpSession	session,
			       int		err,
			       SnmpSyntax	pdu);


	/**
	 * <P>This method is invoked when an agent fails to respond
