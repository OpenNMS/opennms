...
public class BerEncoder implements AsnEncoder
{
	...
	protected static void copy(byte[]	src, 
				   ...
				   byte[]	dest, 
				   int		destOff, 
				   int		bytesToCopy) throws ArrayIndexOutOfBoundsException
	{
		if((dest.length - destOff) < bytesToCopy ||
		   (src.length - srcOff)   < bytesToCopy)
	...
	protected static void copy(int[]	src, 
			           ...
			           int[]	dest, 
			           int		destOff, 
			           int		intsToCopy) throws ArrayIndexOutOfBoundsException
	{
		if((dest.length - destOff) < intsToCopy ||
		   (src.length - srcOff)   < intsToCopy)
			throw new ArrayIndexOutOfBoundsException("Destination or source buffer is insufficent");
	...
	protected static void rotate(byte[]	buf,
				     int	begin,
				     int	pivot,
				     int	end) throws ArrayIndexOutOfBoundsException
	{
		int dist    = end - pivot;
	...
	public Object[] parseLength(byte[] buf, int startOffset) throws AsnDecodingException
	{
		...
		else
		{
			...
			else if(numBytes == 2)
			{
				...
				int val = byteToInt(buf[startOffset++]) << 8 |
					  byteToInt(buf[startOffset++]);
