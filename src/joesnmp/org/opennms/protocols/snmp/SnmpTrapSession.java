...
// Tab Size = 8
//
// SnmpTrapSession.java,v 1.1.1.1 2001/11/11 17:27:22 ben Exp
//
 ...
 * SNMPv1 & v2 trap messages. The messages are received
 * on the configured port, or the default(162) port and
 * then decoded using the set ASN.1 codec. When messages
 * are sent they are encoded using the passed SnmpParameters
 * object that is part of the SnmpPeer object.</P>
 *
 * <P>A trap message handler must be bound to the session
 * in order to send or receive messages.</P>
 *
...
public final class SnmpTrapSession extends Object
{
	 ...
	 * snmpTrapSessionError method is invoked in the trap
	 ...
	 * when appropiate to the SnmpTrapHandler registered
	 * with the session.</P>
	 *
	 */
		...
		implements SnmpPacketHandler
	{
		...
		/**
		 * <P>Creates a in internal trap handler to be
		 * the intermediary for the interface between
		 * the SnmpPortal and the TrapSession.</P>
		 *
		 * @param sess The trap session reference.
		 *
		 */
					       ...
					       SnmpPduPacket	pdu) throws SnmpPduEncodingException
		{
			...
			try
			{
				m_handler.snmpReceivedTrap(m_forWhom,
							   agent,
							   port,
							   community,
							   pdu);
		...
		public void processException(Exception e)
		{
			try
			{
				m_handler.snmpTrapSessionError(m_forWhom,
							       ERROR_EXCEPTION,
							       e);
			}
			catch(Exception e1)
		...
		throws SocketException
	{
		...
		m_why		  = null;
		m_handler	  = handler;
		m_portal	  = new SnmpPortal(new TrapHandler(this), 
						   m_encoder, 
						   m_port);
	}
	...
	/**
	 * The default SnmpTrapSession constructor that takes a packet
	 * handler as parameter. Also changes the default port to listen on
	 *
	 * @exception java.net.SocketException If thrown it is from the creation
	 *		of a DatagramSocket.
	 */
	public SnmpTrapSession(SnmpTrapHandler handler, int port) 
		throws SocketException
	{
		m_port		= port;
		m_encoder	= (new SnmpParameters()).getEncoder();
	 ...
	 *
	 * @throws java.lang.IllegalStateException Thrown if the session was already closed.
	 */
	public void close()
	{
	 ...
	 * defined by the SnmpParameters. Once the packet is encoded it 
	 * is transmitted to the agent defined by SnmpPeer. If an error 
	 * occurs an appropiate exception is generated.
	 *
	 * @param peer	The remote peer to send to.
	 * @param trap	The SnmpPduTrap to transmit
	 *
	 ...
	 *
	 */
	public void send(SnmpPeer peer, SnmpPduTrap trap)
