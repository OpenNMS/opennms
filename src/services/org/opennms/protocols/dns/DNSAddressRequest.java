 ...
 * a host - provides for transmitting and receiving the response for this
 * lookup.
 *
 * NOTES: A DNS request and response has the following fileds
 *        header
 *	  questions
 *	  answers
 *	  authorities
 ...
 *	  id - unique id sent by the client and returned by the server 
 *             in its response
 *        16 bits of flags -
 *           Query(0)/response(1) flag
 *           opcode - that has type of query
 *           AA - set if the response is an authoritative answer
 *	     TC - set if response is truncated
 *	     RD - set if recursion is desired
 *	     RA - set if recursion is available
 * 	     Z  - reserved bits
 * 	     RCODE  - response code
 *
...
{
	/**
	 * <P>Defines the class internet in the domain name
	 * system.</P>
	 */
  	public final int CLASS_IN 		= 1;   // internet
	
	/**
	 * <P>Defines the address type.</P>
	 */
    	public final int TYPE_ADDR		= 1;   // address

	/**
	 * <P>The offset of the query bit in the header.</P>
	 */
	public final int SHIFT_QUERY 		= 15;
	
	/**
	 * <P>The offset of the opcode bits in the header.</P>
	 */
	public final int SHIFT_OPCODE 		= 11;
	
	/**
	 * <P>The offset of the authoritative bit in the header.</P>
	 */
	public final int SHIFT_AUTHORITATIVE 	= 10;
	
	/**
	 * <P>The offset of the truncated bit in the header.</P>
	 */
	public final int SHIFT_TRUNCATED 	= 9;
	
	/**
	 * <P>The offset of the recurse req bit in the header.</P>
	 */
	public final int SHIFT_RECURSE_PLEASE 	= 8;
	
	/**
	 * <P>The offset of the requrse avail bit in the header.</P>
	 */
	public final int SHIFT_RECURSE_AVAILABLE= 7;
	
	/**
	 * <P>The offset of the reserved bits in the header.</P>
	 */
	public final int SHIFT_RESERVED		= 4;
	
	/**
	 * <P>The offset of the response code bits in the header.</P>
	 */
	public final int SHIFT_RESPONSE_CODE 	= 0;
	
	/**
	 * <P>The op code for a query in the header.</P>
	 */
    	public final int OPCODE_QUERY 		= 0;

	/**
	 * <P>The host to request information from.
	 * This would be the nameserver if it supports
	 * DNS.</P>
	 */
	public String 	m_reqHost;
	
	/**
	 * <P>The id used to seralize the request. This allows the
	 * client (us) and the server (host) to match exchanges.</P>
	 */
	public int	m_reqID;

	/**
	 * <P>True if the answer is authoratitve.</P>
	 */
	public boolean 	m_authoritative;
	
	/**
	 * <P>True if the message is truncated.</P>
	 */
	public boolean  m_truncated;
	
	/**
	 * <P>True if the message is recursive.
	 */
	public boolean	m_recursive;

	/**
	 * <P>The list of answers.</P>
	 */
	public List	m_answers;

	/**
	 * <P>The global id, used to get the request id.</P>
	 */
	private static int globalID = 1;

	/**
	 * <P>Decodes the integer to get the flags - refer header for more
	 * info on the flags.</P>
	 */
		...
		throws IOException 
	{
		...
		if (code != 0) 
		{
			if (code == 2) // Throw exception if error code indicates 'Server Failure'
	...
	/**
	 * <P>Constructs a DNSAddressRequest for ths hostname passed.
	 * The host string that is passed to the address string should
	 * be a hostname in "x.y.z" where x, y, and z are strings. This
	 * is not suppose to be a dotted decimal address.</P>
	 *
	 * @param host		hostname for which address is to be constructed
	 */
	...
	/**
	 * <P>Builds the address request.</P>  
	 *
	...
	/**
	 * <P>Extracts the response from the bytearray.</P>
	 *
	 * @param data 		The byte array containing the response.
	 * @param length 	The length of the byte array.
	 *
	 * @exception IOException Thrown if there is an error 
	 *	while reading the recieved packet
	 */
	...
	/**
	 * <P>This method only goes so far as to decode the flags in
	 * the response byte array to verify that a DNS server sent
	 * the response.</P>
	 * 
	 * <P>NOTE:  This is really a hack to get around the fact 
	 * that the receiveResponse() method is not robust enough to 
	 * handle all possible DNS server responses.
	 *
	 * @param data 		The byte array containing the response.
	 * @param length 	The length of the byte array.
	 *
	 ...
	 *	while reading the recieved packet
	 */
	public void verifyResponse (byte[] data, int length) 
		throws IOException 
	{
		/*
		 * Decode the input stream.
	...
	/**
	 * <P>Return an enumeration of the recieved answers.</P>
	 *
	...
	/**
	 * <P>The request id for this particular instance.</P>
	 */
	...
	/**
	 * <P>The hostname that will be request from the DNS box.</P>
	 */
	...
	/**
	 * <P>Returns true if the answer is truncated.</P>
	 */
	...
	/**
	 * <P>Returns true if the answer is recursive.</P>
	 */
	...
	/**
	 * <P>Returns true if the answer is authoritative.</P>
	 */
	...
	/**
	 * <P>Returns the code string for the error code recieved.</P>
	 *
