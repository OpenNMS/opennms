 ...
 * the user of the class to set any of the values. Nor can a default
 * object be constructed. A copy of an existing header can be created or
 * one can be loaded from a collection of bytes. 
 *
 * For more information on the IP header see the book
	...
	extends Object
{
	...
	public static class RouteOption extends Option
	{
		 ...
		 * encapsulated addresses. The class IPv4Address is used
	...
	/**
	 * Constructs a new IP Header object that is identical 
	 * to the passed IPHeader. The new object is a complete
	 * duplicate including the option data that is copied
	 ...
	 * The data is gathered from the buffer starting at the location
	 * marked by offset. If there is not sufficent data in the buffer
	 * then an exception is thrown.
	 *
	 * @param header	The buffer containing the header
	 * @param offset	The offset into the buffer where
	 *			the IP header is located.
	 *
	 * @exception java.lang.IndexOutOfBoundsException This 
	 *		exception is thrown if the minimum number
	 *		of bytes are not present to represent an IPHeader object.
	 * @exception UnknownIPVersionException Thrown if the format of the version is unknown.
	 *
	 */
	public IPHeader(byte[] header, int offset)
	{
		...
		if((length - offset) < (m_hdrlen * 4))
			throw new IndexOutOfBoundsException("Insufficient data: buffer size = "
							   + (length - offset) 
							   + " and header length = "
							   + (m_hdrlen * 4));
		...
		// in bytes.
		//
		m_length= (short)(byteToShort(header[offset+ndx]) << 8 |
				  byteToShort(header[offset+ndx+1]));
		ndx += 2;

		//
		// Next get the 16-bit identification field
		//
		m_identity= (short)(byteToShort(header[offset+ndx]) << 8 |
				    byteToShort(header[offset+ndx+1]));
		ndx += 2;
		...
		// fragment offset!
		//
		m_fragOffset	= (short)(byteToShort(header[offset+ndx]) << 8 |
					  byteToShort(header[offset+ndx+1]));
		m_flags		= (byte)(m_fragOffset >>> 13);		// get the upper 3-bits
		m_fragOffset	= (short)(m_fragOffset & 0x1fff);	// mask off the upper 3-bits
		ndx += 2;
		...
		//
		m_checksum = (short)(byteToShort(header[offset+ndx]) << 8 |
				     byteToShort(header[offset+ndx+1]));
		ndx += 2;
		...
		//
		m_srcAddr = byteToInt(header[offset+ndx])   << 24 |
			    byteToInt(header[offset+ndx+1]) << 16 |
			    byteToInt(header[offset+ndx+2]) << 8  | 
			    byteToInt(header[offset+ndx+3]);
		ndx += 4;
		...
		//
		m_dstAddr = byteToInt(header[offset+ndx])   << 24 |
			    byteToInt(header[offset+ndx+1]) << 16 |
			    byteToInt(header[offset+ndx+2]) << 8  | 
			    byteToInt(header[offset+ndx+3]);
		ndx += 4;
	...
	/**
	 * Gets the 16-bit ones compliment checksum for the 
	 * IP header.
	 *
	 ...
	 *
	 * @param sum	The IP header checksum.
	 *
	 */
	 ...
	 * @param offset	The offset to start storing information.
	 *
