...
/**
 * This class defines Internet Control Message Protocol
 * header. The header defines the first 8 bytes of any
 * ICMP message. Some ICMP messages may need to override
 * the format of the second 4 bytes, the first 4 bytes
...
public class ICMPHeader extends Object
{
	 ...
	 * This number is generated application wide.
	 *
	 */
	private static short sm_seq = 0;

	/**
	 * Returns the next 16-bit sequence identifier
	 * for the class. The method is synchronized to
	 * prevent duplicate identifiers from being issued.
	 * depending on the number of classes and how often
	 * the method is called it will eventually wrap. When
	 * the value wraps back to zero, a random number is 
	 * generated and may cause a collision with an existing
	 * identifier. The probability is low, but possible.
	 *
	...
	/**
	 * Initializes the header with the specified
	 * type and code values.
	 *
	 * @param type The type value for the header
	 * @param code The code value for the header
	 *
	 */
	...
	/**
	 * Constructs an ICMP header with the specified
	 * header fields.
	 *
	 * @param type		The 8-bit ICMP type.
	 * @param code		The 8-bit ICMP code.
	 * @param checksum	The 16-bit checksum header.
	 * @param identity	The 16-bit identity (user).
	 * @param sequence	The 16-bit sequence id.
	 *
	 */
	public ICMPHeader(byte	type,
			  byte	code,
	...
	/**
	 * Constructs a duplicate ICMP header that is
	 * identical to the passed ICMPHeader object.
	 *
	 * @param second	The object to duplicate.
	 *
	 */
	...
	/**
	 * Returns the sequence identifier for the
	 * ICMP header.
	 *
	 ...
	 * and sets the value in the object. In
	 * addition the new sequence identifier
	 * is returned.
	 *
	...
	/**
	 * Sets the header's 16-bit user defined
	 * identity value.
	 *
	...
	/**
	 * Used to retrieve the current checksum for the
	 * header. This is the last checksum computed for
	 * the header (or it's derived classes). To compute
	 * the new checksum a call to the method computeChecksum()
	 * must be called.
	 *
	...
	/**
	 * Provides the default checksum implementation for the
	 * ICMP header. It MUST be overriden by any derived classes
	 * to ensure that the checksum includes the dervived class 
	 * data. Once the checksum is calculated, a call to the 
	 ...
	 * The process involves setting the checksum to zero
	 * and then performing the checksum over the various 
	 * values.
	 *
	 * @param summer The checksum builder object.
	 */
	...
	/**
	 * Writes the ICMP header out to the specified
	...
	/**
	 * Reads the ICMP header from the specified buffer and
	 * sets the internal fields equal to the data. If the 
	 * buffer does not have sufficent data to restore the
	 * header then an IndexOutOfBoundsExceptoin is thrown
	 * by the method.
	 *
	 ...
	 *
	 * @exception IndexOutOfBoundsException is thrown if there
	...
	protected int loadFromBuffer(byte[] buf, int offset)
	{
		...
		m_checksum =	(short)(byteToShort(buf[offset++]) << 8 |
				       byteToShort(buf[offset++]));

		m_ident =	(short)(byteToShort(buf[offset++]) << 8 |
				       byteToShort(buf[offset++]));

		m_sequence =	(short)(byteToShort(buf[offset++]) << 8 |
				       byteToShort(buf[offset++]));
	...
	/**
	 * Returns the number of bytes required to read/write
	 * an icmp header. A header is composed of a fixed size
	 * number of byte. This is not expected to change, but 
	 * this method allows derived classes to query the number
	 * of bytes for reading/writing an icmp header. Thus should
	 * the standard ever change, the derived class should be able
	 * to dynamically handle the change.
	 *
	...
	/**
	 * Used to test to see if the header is an echo
	 * reply message. If it is an echo reply message
	...
	/**
	 * Used to test to see if the header is an echo
	 * request message. If it is an echo request message
	 * then true is returned.
	 *
