 ...
 * for an event and loading info from the eventconf match to the event.
 * This class is also responsible for the event parm expansion</P>
 *
 * <P>Notes on event parm expansion:</P>
 * <P>The list of elements that can have a %element% or %parms[*]% in their
 * value are :
 * descr, logmsg, operinstr, autoaction, operaction(/menu), tticket</P>
 *
 * <P>The list of elements that can occur as a %element% are :
 * uei, source, nodeid, time, host, interface, snmphost, service, snmp, id, idtext,
 * version, specific, generic, community, severity, operinstr, mouseovertext,
 * parm[values-all], parm[names-all], parm[all], parm[<name>], parm[##], parm[#<num>]</P>
 *
 * <pre>
...
public final class EventExpander
{
	 ...
	 * generic types in a SNMPv1 trap. These are not relevant to a SNMPv2c 
	 * trap.
	 */
	 ...
	 * not have a mask and the information from the configuration
	 * object is copied.
	 *
	...
	/**
	 * This method is used to transform a log message event configuration instance
	 * into a log message event instance. This is used when the incomming event does
	 * not have any log message information and the information from the configuration
	 * object is copied.
	 *
	 * @param src	The configuration source to transform.
	 *
	...
	/**
	 * This method is used to transform a correlation event configuration instance
	 * into a correlation event instance. This is used when the incomming event does
	 * not have any correlation information and the information from the configuration
	 * object is copied.
	 *
	 * @param src	The configuration source to transform.
	 *
	...
	/**
	 * This method is used to transform an auto action event configuration instance
	 * into an auto action event instance. This is used when the incomming event does
	 * not have any auto action information and the information from the configuration
	 * object is copied.
	 *
	 * @param src	The configuration source to transform.
	 *
	...
	/**
	 * This method is used to transform an operator action event configuration instance
	 * into an operator action event instance. This is used when the incomming event does
	 * not have any operator action information and the information from the configuration
	 * object is copied.
	 *
	 * @param src	The configuration source to transform.
	 *
	...
	/**
	 * This method is used to transform an auto acknowledgement event configuration instance
	 * into an auto acknowledgement event instance. This is used when the incomming event does
	 * not have any auto acknowledgement information and the information from the configuration
	 * object is copied.
	 *
	 * @param src	The configuration source to transform.
	 *
	...
	/**
	 * This method is used to transform a trouble ticket event configuration instance
	 * into a trouble ticket event instance. This is used when the incomming event does
	 * not have any trouble ticket information and the information from the configuration
	 * object is copied.
	 *
	 * @param src	The configuration source to transform.
	 *
	 ...
	 * not have any forward information and the information from the configuration
	 * object is copied.
	 *
	 * @param src	The configuration source to transform.
	 *
	 ...
	 * is compared as is, if that doesn't match then the enterprise prefix
	 * is stripped and the lookup is preformed again.</p>
	 *
	 * <p>If the enterprise identifier is not found, but the event has
	 * generic and specific information then the appropriate generic
	 * UEI is used to find the appropriate record.</p>
	 *
	 *
	 * @param snmpInfo	The SNMP event information.
	 *
	 * @return The matching event configuration object if found. A null
	 * 	reference is returned if no match can be found.
	 *
	 * @exception java.lang.NullPointerException Thrown if the snmpInfo parameter
	...
	/**
	 * <p>This method is used to lookup the event configuration object
	 * based upon information in the passed information. The
	 * {@link EventConfigurationManager EventConfigurationManager} instance
	 * is consulted to find a matching configured event. The lookup 
	 * algorithm favors SNMP information if avaialable, and then defaults
	 * to the event's Universal Event Identfier.</p>
	 *
	 * @param event		The event to find a configuration for.
	 *
	 * @return The matching configuration event, if any.
	 *
	 * @exception java.lang.NullPointerException Thrown if the event parameter
	 *	that was passed is null.
	 *
	 */
	...
	/**
	 * Expand the element values if they have parms in one of the following formats
	 *
	...
	/**
	 * <p>This method is invoked to check and configure a received event. The
	 * event configuration manager is consulted to find the appropriate configuration
	 * that is used to expand the event. In addition, the security parameters
	 * from the configuration manager is consulted to ensure that secure files are
	 * cleared out if necessary.</p>
	 *
	 * <p>Any secure fields that exists in the incomming event are cleared 
	 * during expansion.</p>
	 *
	 * @param e	The event to expand if necesary.
	 *
	 */
