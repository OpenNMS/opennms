 ...
 * evaluated once at construction and reused(if new values are added or
...
public class EventMaskValueList extends ArrayList
{
	...
	/**
	 * <p>Handling the mask values ending with '%' is a pain since the
	 * hashcodes will need to work in reverse!
	 *
	 * <p>For e.g. consider mask values '.1.3.6.1.4.1.9%' and '.1.3.6.1.4.1.%'
	 * normal hashcodes will mean '.1.3.6.1.4.1.9%' will have a larger
	 * hashcode. However when we try a match in the eventconf(which uses
	 * a treemap of eventkeys to get the event keys in an ordered fashion),
	 * we want '.1.3.6.1.4.1.9%' to be a better match than '.1.3.6.1.4.1.%'
	 * - i.e.we want '.1.3.6.1.4.1.9%' to have a lesser hashcode than 
	 * '.1.3.6.1.4.1.%' - hence this method. Only a lot of testing will
	 * reveal conflicts etc. though</p>
	 */
