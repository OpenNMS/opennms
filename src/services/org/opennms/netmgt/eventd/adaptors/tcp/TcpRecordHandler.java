...
/**
 * This class is used to do the initial read of data from the
 * input stream and break it up into records. Each record is
 * written to a piped writer. This means that the reader never
...
{
	/**
	 * When set the runnable should exit as fast as 
	 * possible.
	 */
	...
	/**
	 * This class is use to model the set of states, the 
	 * attached TcpRecordHandler, and the transition
	 * actions.
	 */
	private static class StateManager
	{
		 ...
		 *
		 * @param level		The level of the state manager
		 * @param handler	The handler to fire events at.
		 */
		...
		/**
		 * handle the next character, returns the
		 * next level
		 */
		int next(char ch)
			throws IOException
		...
		/**
		 * Handle the transtion from character
		 * to character.
		 */
	...
	/**
	 * forwards the characters to the 
	 * attached pipe.
	 */
	...
	TcpRecordHandler(Socket s, List xchange)
	{
		...
		//
		m_tokenizer = new StateManager[] {
			new StateManager(0, this) {
			...
			//
			new StateManager(8, this) { // gobbles up white space after record
				int next(char ch) throws IOException {
					if(ch == '<') {
						onTransition(ch);
						return 1;
