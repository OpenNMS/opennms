 ...
 * client's event XML document. When a new stream handler is created and assigned
 * to an execution context it will unmarshall the remote document. The events
 * from the remote document are then passed to the registered event handlers. 
	...
	implements Runnable
{
	...
	/**
	 * The socket connection to receive and process events
	 * from. The successfully processed events will be 
	 * acknowledged in an event-recipt document.
	 */
	...
	/**
	 * Constructs a new TCP/IP stream handler to process the
	 * remote document.
	 *
	 * @param parent	The parent fiber
	 * @param sock		The socket connection
	 * @param handlers	The list of event handlers.
	 * @param number	The number of event records to process
	 */
	 ...
	 * Once the document is processed and an event receipt is returned
	 * to the client the thread will exit. 
	 */
	public void run()
	{
		...
		Thread tchunker = new Thread(chunker, "TCPRecord Chuncker["
		...
		MAINLOOP:
		while(!m_stop
		      && m_parent.getStatus() != Fiber.STOP_PENDING
		      && m_parent.getStatus() != Fiber.STOPPED
		      && m_recsPerConn != 0)
		{
			...
			if(events != null && events.length != 0)
			{
				...
				// lots of cycles doing synchronization when it should not normally
				// be necesary.
				//
