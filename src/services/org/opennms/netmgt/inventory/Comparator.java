...
public class Comparator {
	...
	private void visit(Item currItem, List path, List list, Map itemMap){
		...
		private String compareDataItem(List itemPath){
			...
			Dataitem oldDataItem = oldItem.getDataitem();
			Dataitem newDataItem = newItem.getDataitem();
			String oldDataItemStr ="";
			for(int i=0; i<oldDataItem.getLineCount();i++){
				oldDataItemStr+=oldDataItem.getLine(i);
			}
			String newDataItemStr ="";
			for(int i=0; i<newDataItem.getLineCount();i++){
				newDataItemStr+=newDataItem.getLine(i);
			}
			String oldDataItemStrNotFormatted=oldDataItemStr.replaceAll("\\s","");
			String newDataItemStrNotFormatted=newDataItemStr.replaceAll("\\s","");
			if(!oldDataItemStrNotFormatted.equals(newDataItemStrNotFormatted))
				retStr+=itemPath+" is changed. Old value: '"+oldDataItemStr+"', New value: '"+newDataItemStr+"'.\n<br>";
			return retStr;
		}
