...
/**
 * <P>A proxy to a PollableGroup which acts as a surrogate for rescheduling purposes.
 *    This was created as a temporary fix to allow us to schedule a PollableGroup
 ...
 *    necessary.</P>
 *
 * <P>In order to use this class, create an instance of it with the desired PollableGroup
 ...
 *    PollableGroup it contains.  Once it catches up to the originally scheduled time
 *    for its PollableGroup, it will politely quit rescheduling itself.  Assuming that
    ...
    implements ReadyRunnable
{
     ...
     *
     * @param psvc       The PollableGroup to be proxied.
     * @param runAt      The timestamp after which this proxy should run.
     */
    PollableGroupProxy(PollableGroup psvc, long runAt)
    {
        this._group = psvc;
     ...
     *
     * <P>It passes a run() call through to the PollableGroup that it
    ...
    public void run()
    {
        ...
        if (System.currentTimeMillis() > this._group.getScheduledRuntime())
        {
        ...
        try 
        {
            ...
            this._group.run(false);

            // Calculate the interval and the next scheduled runtime
            long interval = this._group.recalculateInterval();
             ...
             * the scheduler.
             */
            if (this._scheduledRuntime < this._group.getScheduledRuntime())
            {
                log.debug("run: Proxy rescheduling itself at " + interval + " ms");
                Inventory.getInstance().getScheduler().schedule(this, interval);
            }
            else
            {
        ...
        catch (LockUnavailableException e)
        {
            ...
            Inventory.getInstance().getScheduler().schedule(this, 10000);
        }
