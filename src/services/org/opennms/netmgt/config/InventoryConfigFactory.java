...
import org.opennms.netmgt.ExtendedConfigFileConstants;
 ...
 * the OpenNMS Inventory service from the inventory-configuration.xml.</p>
 *
 ...
 */
public final class InventoryConfigFactory
{
	/**
	 * The singleton instance of this factory
	 */
	private static InventoryConfigFactory	m_singleton=null;
	 ...
	 */
	private InventoryConfiguration		m_config;
	...
	/**
	 * Go through the inventory configuration and build a mapping of each configured
	...
	private void createUrlIpMap()
	{
		...
		{
			org.opennms.netmgt.config.inventory.Package pkg = (org.opennms.netmgt.config.inventory.Package)pkgEnum.nextElement();
	 ...
	 */
	private InventoryConfigFactory(String configFile)
		throws 	IOException,
			MarshalException, 
			ValidationException
	{
		...
		m_config = (InventoryConfiguration) Unmarshaller.unmarshal(InventoryConfiguration.class, new InputStreamReader(cfgIn));
		cfgIn.close();
			...
			ValidationException
	{
		...
		File cfgFile = ExtendedConfigFileConstants.getFile(ExtendedConfigFileConstants.INVENTORY_CONFIG_FILE_NAME); 

		ThreadCategory.getInstance(InventoryConfigFactory.class).debug("init: config file path: " + cfgFile.getPath());
		
		m_singleton = new InventoryConfigFactory(cfgFile.getPath());
	 ...
	 */
	public static synchronized InventoryConfigFactory getInstance()
	{
		if(!m_loaded)
	...
	/** 
	 * <p>Return the inventory configuration object</p>
	 */
	public synchronized InventoryConfiguration getConfiguration()
	{
		return m_config;
	}
	 ...
	 *	false otherwise.
	 */
	public synchronized boolean interfaceInPackage(String iface, org.opennms.netmgt.config.inventory.Package pkg)
	{
	 ...
	 */
	public synchronized boolean groupInPackageAndEnabled(String svcName, org.opennms.netmgt.config.inventory.Package pkg)
	{
		Category log = ThreadCategory.getInstance(this.getClass());
		...
		Enumeration esvcs = pkg.enumerateGroup();
		while(result == false && esvcs.hasMoreElements())
		{
			Group tsvc = (Group)esvcs.nextElement();
	 ...
	 * @param svcName	The service name to lookup.
	 */
	public synchronized boolean groupMonitored(String svcName)
	{
		boolean result = false;
		...
		while(monitorEnum.hasMoreElements())
		{
			...
			if (monitor.getGroup().equals(svcName))
			{
				result = true;
				break;
			}
	 ...
	 */
	public synchronized org.opennms.netmgt.config.inventory.Package getFirstPackageMatch(String ipaddr)
	{
		Enumeration pkgEnum = m_config.enumeratePackage();
		while(pkgEnum.hasMoreElements())
		{
	 ...
	 */
	public synchronized boolean isPolled(String svcName, org.opennms.netmgt.config.inventory.Package pkg)
	{
		// Check if the service is enabled for this package and
		// if there is a monitor for that service
		//
		boolean svcInPkg = groupInPackageAndEnabled(svcName, pkg);
		if (svcInPkg)
		{
			return groupMonitored(svcName);
		}
