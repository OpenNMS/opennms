...
public final class DataCollectionConfigFactory
{
	 ...
	 */
	private DataCollectionConfigFactory(String configFile)
		throws 	IOException,
			MarshalException, 
			ValidationException
	{
	 ...
	 * particular system object id, IP address, and ifType for the specified 
	 * collection.    
	 *
	 * @param cName		name of the data collection from which to retrieve
	 *			oid information.
	 * @param aSysoid	system object id to look up in the collection
	 * @param anAddress	IP address to look up in the collection
	 * @param ifType 	Interface type (-1 indicates that only 
	 * 				node-level objects should be retrieved.
	 *
	...
	public List getMibObjectList(String cName, String aSysoid, String anAddress, int ifType)
	{
		...
		if (log.isDebugEnabled()) 
			log.debug("getMibObjectList: collection: " + cName + 
					" sysoid: " + aSysoid + 
					" address: " + anAddress + 
					" ifType: " + ifType);
	 ...
	 * This method takes a group name and a list of MibObject objects
	 * as arguments and adds all of the MibObjects associated with
	 * the group to the object list.  If the passed group consists
	 * of any additional sub-groups, then this method will be 
	 * called recursively for each sub-group until the entire
		log.debug("processGroupName: adding MIB objects from group: " + groupName);
	 * group is processed.  
	 * 
	...
	private void processGroupName(String cName, String groupName, int ifType, List mibObjectList) 
	{
		...
		while(i.hasNext())
		{
			processGroupName(cName, (String)i.next(), ifType, mibObjectList); // Recursive call to process sub-groups
		}
			...
			else
			{
				...
				else
				{
					...
					while (tmpIndex != -1)
					{
						...
						if (nextComma == -1) // No comma, this is last type value
						{
	...
	/**
	 * Takes a list of castor generated MibObj objects iterates over them creating
	 * corresponding MibObject objects and adding them to the supplied
	 * MibObject list.
	 *
	...
	/**
	 * Retrieves the configured value for the maximum number
	 * of variables (oids) which can be encoded into a single 
	 * outgoing SNMP PDU request..
	 * 
	 * @param cName Name of the data collection
	 * 
