...
public class Manager implements ManagerMBean {
    ...
    public static Attribute getAttribute(org.opennms.netmgt.config.service.Attribute attrib) throws Exception {
                ...
                for (int j = 0; todo != null && j < todo.length; j++) {
                    if (todo[j].getPass() == pass && (todo[j].getAt() != null && todo[j].getAt().equals("status"))) {
                        ...
                        try {
                            ...
                            Object result = server.invoke(mbeans[i].getObjectName(), todo[j].getMethod(), parms, sig);
