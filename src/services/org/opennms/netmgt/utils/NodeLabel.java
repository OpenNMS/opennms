...
/**
 * <P> This class contains convenience functions for retrieving and
 * modifying the label associated with a managed node.  The 'node' table
 * contains a 'nodelabel' and 'nodelabelsource' field.  The 'nodelabel'
 * is a user-friendly name associated with the node.  This name can
 * be user-defined (via the WEB UI) or can be auto-generated based on
 * what OpenNMS knows about the node and its interfaces.  The 'nodelabelsource'
 * field is a single character flag which indicates what the source for
 * the node label was.
 * </P>
 * <PRE>
 * Valid values for node label source are:
 * 	'U' 	User defined
 * 	'H'	Primary interface's IP host name
 * 	'S'	Node's MIB-II sysName
 * 	'A'	Primary interface's IP address
 * </PRE>
 *
 * 
...
{	
	...
	/**
	 * The SQL statement to retrieve the current node label and node label source values associated 
	 * with a node.
	 */
	...
	/**
	 * Primary interface selection method MIN.  Using this selection
	 * method the interface with the smallest numeric IP address
	 * is considered the primary interface.
	 */
	private final static String SELECT_METHOD_MIN = "min";
	
	/**
	 * Primary interface selection method MAX. Using this selection
	 * method the interface with the greatest numeric IP address
	 * is considered the primary interface.
	 */
	 ...
	 * @param nodeLabelSource	Flag indicating source of node label
	 */
	 ...
	 *
	 * WARNING:  A properly instantiated and initlaized Vault class object
	 *           is required prior to calling this method. This method
	 ...
	 * the 'node' table for the specified nodeID.  A database connection
	 * is retrieved from the Vault.  
	 *
	 * WARNING:  A properly instantiated and initlaized Vault class object
	 *           is required prior to calling this method. This method
	 *           will initially only be called from the WEB UI. 
	...
	/**
	 * This method determines what label should be associated with a 
	 * particular node.  A database connection is retrieved from the Vault.  
	 *
	 * WARNING:  A properly instantiated and initlaized Vault class object
	 *           is required prior to calling this method. This method
	 *           will initially only be called from the WEB UI. 
	 ...
	 * particular node.   
	 * 
	 * Algorithm for determining a node's label is as follows:
	 * 1) If node has a NetBIOS name associated with it, the NetBIOS name
	 *    is used as the node's label.
	 * 2) If no NetBIOS name available, retrieve all the 'ipinterface' 
	      table entries associated with
	 *    the node with an 'isManaged' field value of 'M'
	 * 3) Find the primary interface where "primary" is defined as the
	 *    managed interface with the smallest IP address (each IP address is
	 *    converted to an integer value -- the IP address with the 
	 *    smallest integer value wins).
	 * 4) IF the primary interface's IP host name is known it becomes
	 *    	the node's label.
	 *    ELSE IF the node's MIB-II sysName value is known it becomes
	 *    	the node's label
	 *    ELSE the primary interface's IP address becomes the node's label.
	 *
	 * NOTE:  If for some reason a node has no "managed" interfaces 
	 * null is returned for the NodeLabel.
	 * 
	 * @param nodeID		Unique identifier of the node to be updated.
	 * @param dbConnection	SQL database connection
	 * 
	 * @return NodeLabel	Object containing label and source values or 
	 *                        null if node does not have a primary interface.
	 */
	public static NodeLabel computeLabel(int nodeID, Connection dbConnection)
		throws SQLException
	{
		Category log = ThreadCategory.getInstance(NodeLabel.class);
		...
		if (primaryAddr == null)
		{
			...
			// Issue SQL query to retrieve all non-managed interface IP addresses from 'ipinterface' table 
