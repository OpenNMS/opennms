...
final class LinkdEventProcessor implements EventListener {
     ...
     * @param event
     */
    private void handleNodeDeleted(Event event) throws InsufficientInformationException, UnknownHostException {
        Category log = ThreadCategory.getInstance(getClass());

        EventUtils.checkNodeId(event);

        // Remove the deleted node from the scheduler if it's an SNMP node
        getLinkd().unscheduleNode((int)event.getNodeid(),DbWriter.ACTION_DELETE);
     ...
     * @param event
     */
    private void handleNodeGainedService(Event event) throws InsufficientInformationException, UnknownHostException {
        Category log = ThreadCategory.getInstance(getClass());
     ...
     * @param event
     */
    private void handleNodeLostService(Event event) throws InsufficientInformationException, UnknownHostException {
        Category log = ThreadCategory.getInstance(getClass());

        EventUtils.checkNodeId(event);

        // Remove the deleted node from the scheduler
        getLinkd().unscheduleNode((int)event.getNodeid(),DbWriter.ACTION_UPDATE);
     ...
     * @param event
     */
    private void handleRegainedService(Event event) throws InsufficientInformationException, UnknownHostException {
        Category log = ThreadCategory.getInstance(getClass());
    ...
    public void onEvent(Event event) {
        ...
        try {
            ...
            if (log.isDebugEnabled()) {
                log.debug("Received event: " + eventUei + "; from host: " + eventHost + "; service id " + eventService);
            }
            ...
            log.info("LinkdEventProcessor: insufficient information in event, discarding it: " + ex.getMessage());
            notifyEventError(event, "Invalid parameters: ", ex);
        } catch (UnknownHostException ex) {
            log.error("LinkdEventProcessor: operation failed for event: " + event.getUei() + ", exception: " + ex.getMessage());
