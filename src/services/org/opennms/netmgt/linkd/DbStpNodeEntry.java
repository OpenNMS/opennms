		...
		*/
		...
		private void insert(Connection c) throws SQLException {
			...
			if (log.isDebugEnabled())
				log.debug("StpNodeEntry.insert: SQL insert statment = "
					+ names.toString());
			...
			if (log.isDebugEnabled())
				log.debug("StpNodeEntry.insert: SQL insert statement = "
					+ stmt.toString());
			...
			if (log.isDebugEnabled())
				log.debug("StpNodeEntry.insert: SQL update result = " + rc);
			stmt.close();
		...
		private void update(Connection c) throws SQLException {
			...
			if (log.isDebugEnabled())
				log.debug("StpNodeEntry.update: SQL update statment = "
					+ sqlText.toString());
			...
			if (log.isDebugEnabled())
				log.debug("StpNodeEntry.update: update SQL = " + stmt.toString());

			// Run the insert
			//
			int rc = stmt.executeUpdate();
			if (log.isDebugEnabled())
				log.debug("StpNodeEntry.update: update result = " + rc);
			stmt.close();
		...
		private boolean load(Connection c) throws SQLException {
			...
			if (log.isDebugEnabled())
				log.debug("StpNodeEntry.load: SQL load statement = "
					+ stmt.toString());
