	...
	implements SnmpHandler
{
	 ...
	 */
	private static String	STP_PORT_PRIORITY = ".1.3.6.1.2.1.17.2.15.1.2";

	/**
	 * <P>The list of collected dot1dStpPortTableEntries built from
	 ...
	 * variable listed within Dot1dStpPortTableEntry.  By keeping these
	 ...
	 *
	 * @see #Dot1dStpPortTable(SnmpSession, Signaler)
	 *
	 */
	private Dot1dStpPortTable( ) throws UnsupportedOperationException
	{
		throw new UnsupportedOperationException("Default constructor not supported!");
	}
	
	/**
	 * <P>Constructs an Dot1dStpPortTable object that is used to collect
	 ...
	 *
	 * @see Dot1dStpPortTableEntry
	 */
	public Dot1dStpPortTable(SnmpSession session, Signaler signaler)
	{
		m_signal  = signaler;
		m_entries = new ArrayList();
		m_error = false;
		m_version = SnmpSMI.SNMPV1;
		m_stopAt = Dot1dStpPortTableEntry.stop_oid();
		m_snmpVarBindList = new ArrayList();
		
		SnmpPduPacket pdu = Dot1dStpPortTableEntry.getNextPdu(m_version);
	 ...
	 * @see Dot1dStpPortTableEntry
	 */
	public Dot1dStpPortTable(SnmpSession session, Signaler signaler, int version)
	{
		m_signal  = signaler;
		m_entries = new ArrayList();
		m_error = false;
		...
		SnmpPduPacket pdu = Dot1dStpPortTableEntry.getNextPdu(version);
		ThreadCategory.getInstance(getClass()).debug("Dot1dStpPortTable: initial pdu request id: " + pdu.getRequestId());
		session.send(pdu, this);
	}
	 ...
	 *
	 * @return The list of Dot1dStpPortTableEntry maps.
	 */
	...
	{	
		...
		else
		{
			...
			else
			{
				...
				if (m_version == SnmpSMI.SNMPV2) 
				{
					...
					if (!m_error)
					{
						...
						else 
						{
							...
							int numAddresses = 0;
							SnmpObjectId AddrIndex = new SnmpObjectId(STP_PORT_PRIORITY);
							...
							//
							int numEntries = Dot1dStpPortTableEntry.getElementListSize();
							...
							for (int addr_index = 0; addr_index < numAddresses; addr_index++) 
							{
								...
								Dot1dStpPortTableEntry ent = new Dot1dStpPortTableEntry(vblist);
								m_entries.add(ent);
							}
						}
					} 
				...
				{	
					// Create a new map of the Stp Port Table entry
					//
					// The last varbind will be the first one to walk off the end 
					// of the StpPortTable. So verify that the last entry in the 
					// received pdu is still within the scope of the dot1dStpPortTable...
					// if it is then create a new entry.
					//
					if (Dot1dStpPortTableEntry.ROOT.isRootOf(pdu.getVarBindAt(pdu.getLength()-1).getName()))
					{
						if(log.isDebugEnabled())
						log.debug("snmpReceivedPdu: got SNMPv1 response and still within StpPortTable, creating new entry.");
