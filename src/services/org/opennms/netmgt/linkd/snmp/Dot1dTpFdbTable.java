	...
	implements SnmpHandler
{
	...
	/**
	 * The Stp Port Priority, used to get the number of port entries.
	 */
	private static String	FDB_PORT = ".1.3.6.1.2.1.17.4.3.1.2";

	/**
	 * <P>The list of collected dot1dBaseEntries built from
	 ...
	 *
	 * @see #Dot1dTpFdbTable(SnmpSession, Signaler)
	 *
	 */
	private Dot1dTpFdbTable( ) throws UnsupportedOperationException
	{
		throw new UnsupportedOperationException("Default constructor not supported!");
	}
	
	/**
	 * <P>Constructs an Dot1dTfFdbTable object that is used to collect
	 * the mac address to bridg port elements from the remote agent. Once all
	 ...
	 *
	 * @see Dot1dTfFdbTableEntry
	 */
	public Dot1dTpFdbTable(SnmpSession session, Signaler signaler)
	{
		m_signal  = signaler;
		m_entries = new ArrayList();
		m_error = false;
		m_version = SnmpSMI.SNMPV1;
		m_stopAt = Dot1dTpFdbTableEntry.stop_oid();
		m_snmpVarBindList = new ArrayList();
		
		SnmpPduPacket pdu = Dot1dTpFdbTableEntry.getNextPdu(m_version);
		
		session.send(pdu, this);
	}
	
	/**
	 * <P>Constructs an Dot1dTpFdbTable object that is used to collect
	 * the mac address to bridge port elements from the remote agent. Once all
	 * the elements are collected, or there is an error in
	 * the collection the signaler object is <EM>notified</EM>
	 * to inform other threads.</P>
	 *
	 ...
	 * @see Dot1dTfFdbTableEntry
	 */
	public Dot1dTpFdbTable(SnmpSession session, Signaler signaler, int version)
	{
		m_signal  = signaler;
		m_entries = new ArrayList();
		m_error = false;
		...
		SnmpPduPacket pdu = Dot1dTpFdbTableEntry.getNextPdu(version);
		ThreadCategory.getInstance(getClass()).debug("Dot1dTpFdbTable: initial pdu request id: " + pdu.getRequestId());
		session.send(pdu, this);
	}
	 ...
	 * information about the transparent bridge forwarding table.</P>
	 *
	 * @return The list of Dot1dTpFbTableEntry maps.
	 */
		...
		else
		{
			...
			else
			{
				...
				if (m_version == SnmpSMI.SNMPV2) 
				{
					...
					if (!m_error)
					{
						...
						else 
						{
							...
							int numForwardedAddresses = 0;
							SnmpObjectId tpFdbPort = new SnmpObjectId(FDB_PORT);
					    
							while (tpFdbPort.compare(tempStorage[numForwardedAddresses].getName()) > 0) 
								numForwardedAddresses++;						
							
							//store the dot1Dbase Table data for each port into a map.
							//
							int numEntries = Dot1dTpFdbTableEntry.getElementListSize();

							for (int addr_index = 0; addr_index < numForwardedAddresses; addr_index++) 
							{
								SnmpVarBind[] vblist = new SnmpVarBind[numEntries];
								for (int vb_index = 0;  vb_index < numEntries; vb_index++)
								{
									vblist[vb_index] = tempStorage[addr_index + (vb_index*numForwardedAddresses)];
								}
								Dot1dTpFdbTableEntry ent = new Dot1dTpFdbTableEntry(vblist);
								m_entries.add(ent);
							}
						}
					} 
				...
				{	
					...
					// of the Bridge Forward Table. So verify that the last entry in the 
					// received pdu is still within the scope of the IpAddrTable...
					// if it is then create a new entry.
					//
					if (Dot1dTpFdbTableEntry.ROOT.isRootOf(pdu.getVarBindAt(pdu.getLength()-1).getName()))
					{
						if(log.isDebugEnabled())
