	...
	implements SnmpHandler
{
	 ...
	 */
	private static String	IP_NET_TO_MEDIA_PHYSADDR = ".1.3.6.1.2.1.4.22.1.2";
	 ...
	 *
	 * @see #IpNetToMediaTable(SnmpSession, Signaler)
	 *
	 */
	private IpNetToMediaTable( ) throws UnsupportedOperationException
	{
	 ...
	 */
	public IpNetToMediaTable(SnmpSession session, Signaler signaler)
	{
		m_signal  = signaler;
		m_entries = new ArrayList();
		m_error = false;
		m_version = SnmpSMI.SNMPV1;
		m_stopAt = IpNetToMediaTableEntry.stop_oid();
		m_snmpVarBindList = new ArrayList();
		
		SnmpPduPacket pdu = IpNetToMediaTableEntry.getNextPdu(m_version);
	 ...
	 * @see IpNetToMediaTableEntry
	 */
	public IpNetToMediaTable(SnmpSession session, Signaler signaler, int version)
	{
		m_signal  = signaler;
		m_entries = new ArrayList();
		m_error = false;
		...
		SnmpPduPacket pdu = IpNetToMediaTableEntry.getNextPdu(version);
		ThreadCategory.getInstance(getClass()).debug("IpNetToMediaTable: initial pdu request id: " + pdu.getRequestId());
		session.send(pdu, this);
	}
	 ...
	 * information about the ipNetToMedia table.</P>
	 *
	 * @return The list of NetToMediaTableEntry maps.
	 */
		...
		else
		{
			...
			else
			{
				...
				if (m_version == SnmpSMI.SNMPV2) 
				{
					...
					if (!m_error)
					{
						...
						else 
						{
							...
							//counting the number of values found for the ipNetToMediaIfIndex variable,
							//we'll have the number of entries in.  Each Mac address
							//will have a value for each MIB variable listed
							//
							int numAddresses = 0;
							SnmpObjectId AddrIndex = new SnmpObjectId(IP_NET_TO_MEDIA_PHYSADDR);
					    
							while (AddrIndex.compare(tempStorage[numAddresses].getName()) > 0) 
								numAddresses++;
					
							//store the IP Address Table data for each mac address into a map.
							//
							int numEntries = IpNetToMediaTableEntry.getElementListSize();
							for (int addr_index = 0; addr_index < numAddresses; addr_index++) 
							{
								...
								IpNetToMediaTableEntry ent = new IpNetToMediaTableEntry(vblist);
								m_entries.add(ent);
							}
						}
					} 
				...
				{	
					...
					//
					if (IpNetToMediaTableEntry.ROOT.isRootOf(pdu.getVarBindAt(pdu.getLength()-1).getName()))
					{
						if(log.isDebugEnabled())
						log.debug("snmpReceivedPdu: got SNMPv1 response and still within IpNetToMediaTable, creating new entry.");
