 ...
 */
public final class Dot1dStpPortTableEntry extends java.util.TreeMap {
	 ...
	 * this class.</P>
	 */
	private static NamedSnmpVar[] stpport_elemList = null;
	 ...
	 * dot1dStpPortTable tree. It is created when the class is 
	 ...
	 */
	public Dot1dStpPortTableEntry(SnmpVarBind[] vars) {
		this();
		update(vars);
	}
	 ...
	 *
	 * @param vars	The variables in the stp port row.
	 *
	 */
	public void update(SnmpVarBind[] vars) {
		...
		//
		for (int x = 0; x < stpport_elemList.length; x++) {
			SnmpObjectId id = new SnmpObjectId(stpport_elemList[x].getOid());
			...
			for (int y = 0; y < vars.length; y++) {
				if (id.isRootOf(vars[y].getName())) {
					try {
						...
						//
						Class classObj = stpport_elemList[x].getTypeClass();
						...
						if (classObj == null
							...
							if (log.isDebugEnabled()) {
								log.debug("update: Types match!  SNMP Alias: "
										+ stpport_elemList[x].getAlias()
										+ "  Vars[y]: " + vars[y].toString());
							}
							put(stpport_elemList[x].getAlias(), vars[y].getValue());
							put(stpport_elemList[x].getOid(), vars[y].getValue());
						} else {
							if (log.isDebugEnabled()) {
								log.debug("update: variable '"
										+ vars[y].toString()
										+ "' does NOT match expected type '"
										+ stpport_elemList[x].getType() + "'");
							}
							put(stpport_elemList[x].getAlias(), null);
							put(stpport_elemList[x].getOid(), null);
						}
					} catch (ClassNotFoundException e) {
						log.error(
								"Failed to retreive SNMP type class for element: "
										+ stpport_elemList[x].getAlias(), e);
	...
	public static SnmpPduPacket getNextPdu(int version) {
		...
		if (version == SnmpSMI.SNMPV2) {
			...
			pdu.setRequestId(SnmpPduPacket.nextSequence());
			for (int x = 0; x < stpport_elemList.length; x++) {
				SnmpObjectId oid = new SnmpObjectId(stpport_elemList[x].getOid());
				pdu.addVarBind(new SnmpVarBind(oid));
			}
		}
	...
	public static SnmpObjectId stop_oid() {
		Integer endindex = new Integer(stpport_elemList.length + 1);
	...
	public static int getElementListSize() {
		return stpport_elemList.length;
	}
