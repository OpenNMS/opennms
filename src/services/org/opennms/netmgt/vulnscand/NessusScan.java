...
* Nessus security scanner daemon, nessusd. It has been
* designed to:</p>
* <ul>
...
*	</ul>
*	</li>
* 	<li>Communicate with the daemon using username and
*		and password, instead of SSL key exchange</li>
* </ul>
*
* <p>Other functions (arbitrary plugin selection, SSL support)
	...
	implements Runnable
{
	 ...
	 */
	private static final String	SELECT_ALL_VULNERABILITIES =
		"SELECT vulnerabilityid FROM vulnerabilities " +
		"WHERE ipaddr = ? AND resolvedtime IS NULL";

	/**
	 * This query retrieves the name and summary of a plugin out
	 * of the database so that we can construct the logmsg of the
	 * vulnerability with the fields.
	 */
	private static final String	SELECT_PLUGIN_INFO =
		"SELECT name, summary FROM vulnplugins " +
		"WHERE pluginid = ? AND pluginsubid = ?";

	/**
	 * Insert a new vulnerability into the "vulnerabilities" table
	 */
	private static final String	INSERT_NEW_VULNERABILITY =
		"INSERT INTO vulnerabilities " +
			"(vulnerabilityid, nodeid, ipaddr, serviceid, creationtime, lastattempttime, lastscantime, " +
			"severity, pluginid, pluginsubid, logmsg, descr, port, protocol, cveentry) " +
		"VALUES (?,?,?,?, ?,?,?, ?, ?,?, ?,?, ?,?, ?)";

	/**
	 * Find an open vulnerability ID in the database. The combination of
	 * ipaddr, port, protocol, pluginID, and pluginSubID is used as the
	 * key for vulnerability uniqueness.
	 */
	private static final String	SELECT_OPEN_VULNERABILITY =
		"SELECT vulnerabilityid FROM vulnerabilities " +
		"WHERE ipaddr = ? AND port = ? AND protocol = ? AND pluginid = ? AND pluginsubid = ? AND resolvedtime IS NULL";

	/**
	 * Update the timestamps in an open vulnerability that was rescanned
	 * and still exists.
	 */
	private static final String	VULNERABILITY_SCANNED =
		"UPDATE vulnerabilities SET lastattempttime = ?, lastscantime = ? " +
	 ...
	 * failed.
	 */
	private static final String	VULNERABILITY_SCAN_ATTEMPTED =
		"UPDATE vulnerabilities SET lastattempttime = ? " +
		"WHERE vulnerabilityid = ?";

	/**
	 * Resolve a given vulnerability by its unique ID number.
	 */
	private static final String	RESOLVE_VULNERABILITY =
		"UPDATE vulnerabilities SET lastattempttime = ?, resolvedtime = ? " +
		"WHERE vulnerabilityid = ?";
	...
	/**
	 * Integer of the ordinal of the last plugin that was executed against
	 * the target. This number is used to ensure that if the scan is
	 * terminated prematurely because of an unreachable host in the list,
	 * the vulnerabilities are not marked resolved.
	 */
	private int lastPlugin;

	/**
	 * Counter of the total number of plugins that will be executed against
	 * this host.
	 */
	 ...
	 * configuration and insert the results of the scan into the
	 * database.
	 */
	...
	public void run()
	{
		...
		// We'll use this list to resolve vulnerabilities that are not redetected.
		try
		{
		...
		// Perform a Nessus scan of the target IP address. As each vulnerability is
		// found, a new entry is put into the database or the existing entry is
		// updated.
		Socket nessusSocket = null;
		try
		{
			nessusSocket = NessusConnectionFactory.getConnection(
			...
			out.write((NTP_CLIENT_ENTITY + NTP_SEP + "NEW_ATTACK" + NTP_SEP +
				config.targetAddress.toString() + NTP_SEP +
				NTP_CLIENT_ENTITY + "\n").getBytes());
			...
			while (
				(returnCode == SCAN_SUCCESS) ||
				(returnCode == SCAN_NON_FATAL_ERROR)
			)
			{
				while (lines.size() > 0)
				{
					...
					if (
						(line.indexOf("the server killed it") == -1)
					)
					{
						// This processing will update existing vulnerabilities in the database
						// and add new vulnerability entries as the vulnerabilities are detected
			...
			// If there were open vulnerabilities that were not reconfirmed during this
			...
			if (openVulnerabilities.size() > 0)
			{
				...
				try
				{
					...
					while (vuln.hasNext())
					{
						...
						if (
							(returnCode == SCAN_COMPLETE) &&
							(lastPlugin == totalPlugins)
						)
						{
		...
		finally
		{
			...
			if (nessusSocket != null) NessusConnectionFactory.releaseConnection(nessusSocket);
		}
	...
	{
		return (NTP_CLIENT_ENTITY + NTP_SEP + "STOP_WHOLE_TEST" +
			NTP_SEP + NTP_CLIENT_ENTITY + "\n");
	}

	private String buildStopScanString()
	{
		return (NTP_CLIENT_ENTITY + NTP_SEP + "STOP_ATTACK" +
			config.targetAddress.toString() + NTP_SEP +
			NTP_CLIENT_ENTITY + "\n");
	}

	/**
	 * Build the preferences string with the appropriate plugins and
	 * safe_checks settings from the config file.
	 */
	private String buildPreferencesString()
	{
		...
		if (safeChecks[config.scanLevel]) retval += "yes";
		else retval += "no";
	...
	private int processScanMessage(String message)
	{
		...
		if (message.startsWith((NTP_SERVER_ENTITY + NTP_SEP).trim()))
		{
			...
			if (next.equals("INFO") || next.equals("HOLE"))
			{
				...
				{
					log.error("Could not parse the port and protocol information out of this string: " +
						portString
					);
...
//
				...
				try
				{
					...
					// port
					if (portvals.port > 0) stmt.setInt(2, portvals.port);
					else stmt.setNull(2, Types.INTEGER);

					// protocol
					if (portvals.protocol != null) stmt.setString(3, portvals.protocol);
...
//						stmt.setInt(4, serviceId);
						...
						if (plugRS.next())
						{
							...
							{
								if(!pluginLogmsg.equals("")) pluginLogmsg += ": ";
								pluginLogmsg += plugRS.getString("summary");
							}
						}
						plugRS.close(); plugRS = null;

						// If the logmsg could not be populated from the database...
...
// Punt this for now; we will pre-populate the DB
							if (portvals.port >= 0)
								pluginLogmsg = "A vulnerability was detected on port " + portvals.port +
									". See the description for more information.";
							else
								pluginLogmsg = "A vulnerability was detected. See the description for " +
									"more information.";
						}
						...
						if (portvals.port >= 0) stmt.setInt(13, portvals.port);
						else stmt.setNull(13, Types.INTEGER);

						stmt.setString(14, portvals.protocol);

						if (descrvals.cveEntry != null) stmt.setString(15, descrvals.cveEntry);
...
//						stmt.setInt(4, serviceId);
						...
						stmt.setInt(8, Constants.SEV_NORMAL);
