 ...
 *
 * <P>The RTC initializes its data from the database when it comes up. It
 * then subscribes to the Events subsystem to receive events of interest to
 * keep the data up-to-date</P>
 *
 * <P>Availability data is sent out to listeners who indicate that they are
 * listening by sending an RTC 'subscribe' event. The subscribe event has
 * an URL and user/passwd info. so RTC can post data to the URL</P>
 * 
 * <P>The RTC has two timers(a low threshold and a high threshold) and a
 * counter that can run upto a configurable max number of events - these
 * are used to determine when availability information is to be sent out
 * when event streams are coming in at normal rates. When no events are
 * received, a timer configured with a user configured time(defaulting to
 * one minute) decides the interval at which data is sent</P>
 *
		...
		implements PausableFiber
{
	...
	/**
	 * The RunnableConsumerThreadPool that runs updaters
	 * that interpret and update the data
	 */
	...
	/**
	 * The timer scheduled task that runs and informs the RTCManager
