 ...
 *
 * When the RTCManager's timers go off, the DataSender is prompted to
 * send data, which it does by maintaining a 'SendRequest' runnable queue
 * so as to not block the RTCManager
 *
...
final class DataSender implements Fiber
{
	...
	/**
	 * The listeners like the webui that send a URL to which the data
	 * is to be sent
	 */
	...
	/**
	 * Inner class to send data to all the categories - this runnable
	 * prevents the RTCManager from having to block
	 * until data is computed, converted to XML and sent out
	 */
	...
	/**
	 * Set the current thread's priority to the passed value and return the
	 * old priority
	 */
	...
	/**
	 * <P>Shutdown the data sender thread pool
	 */
	...
	/**
	 * Subscribe - Add the recd. URL and related info to the category->Urls
	 * map so the sendData() can send out to appropriate Urls for each
	...
	/**
	 * Unsubscribe - remove the recd. URL and related info from
	 * the category->Urls map so the sendData() will know when it sends
	 * data out
	 */
