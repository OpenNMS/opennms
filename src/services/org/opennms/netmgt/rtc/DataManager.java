...
public class DataManager extends Object {
     ...
     */
    private void addNodeIpSvcToCategory(long nodeid, String ip, String svcname, RTCCategory cat, HashMap knownIPs, PreparedStatement outagesGetStmt) {
        Category log = ThreadCategory.getInstance(DataManager.class);
        ...
        try {
            ...
            while (outRS.next()) {
                ...
                if (log.isDebugEnabled()) {
                    log.debug("lost time for nodeid/ip/svc: " + nodeid + "/" + ip + "/" + svcname + ": " + lostTimeTS + "/" + lostTime);

                    log.debug("regained time for nodeid/ip/svc: " + nodeid + "/" + ip + "/" + svcname + ": " + regainedTimeTS + "/" + regainedTime);
                }
     ...
     * 
     * @param ipRTCs
     *            the list of RTCNodes related to a particular IP
     * @param cat
     *            the category to which the list is to be added
     */
    private void addIpToCategory(List ipRTCs, RTCCategory cat) {
        ...
        while (rtcIter.hasNext()) {
            ...
            if (!cat.containsService(svcName)) {
                if (log.isDebugEnabled())
                    log.debug("service " + svcName + " not in category service list of cat: " + cat.getLabel() + " - skipping " + rtcN.getNodeID() + "\t" + rtcN.getIP() + "\t" + svcName);
    ...
    private void populateNodesFromDB(Connection dbConn) throws SQLException, FilterParseException, RTCException {
        // create a hashtable of IP->RTCNodes list for startup to save on
        // database access
     ...
     */
    public synchronized long getServiceID(String svcname) throws SQLException {
