 ...
 * and sends notifications to the external XMLRPC server via XMLRPC
 * protocol.
 *
	...
	implements Runnable, PausableFiber
{
	 ...
	 * The constructor
	 */
	EventQueueProcessor(FifoQueue eventQ, XmlrpcServer[] rpcServers, int retries, int elapseTime, 
                        boolean verifyServer, String localServer, int maxQSize)
	{
		m_eventQ = eventQ;
                m_maxQSize = maxQSize;
                ...
                if (parms != null)
                {
                        ...
                        while(parmEnum.hasMoreElements())
                        {
                                ...
                                else if (parmName.equals(EventConstants.PARM_SOURCE_EVENT_STATUS))
                                {
                                        ...
                                        {
                                                log.warn("Parameter " 
                                                        + EventConstants.PARM_SOURCE_EVENT_STATUS 
                                                        + " cannot be non-numberic", nfe);
                                                status = -1;
                                        }
                                }
	...
	/**
	 * Push the event back to the event queue if OpenNMS failed to 
	 * send message to the external XMLRPC server, so that it could 
         * be send to the server again later.
	 */
