	...
	implements Runnable, Fiber
{
	...
	/**
	 * The remote DHCP server we sent the request to has the option 
	 * of either unicasting the response directly back to us or 
	 * broadcasting the response to port 68 on the local subnet.
	 * The Receiver class handles the broadcast
	 * scenario and this class will take care of the unicast scenario.
	 */
	public final class UnicastListener extends Thread
	{
		/**
		 * Udp connection over which unicast DHCP response will
		 * be received.
		 */
		 ...
		 * Constructor
		 * 
		 * @param incoming	UDP socket over which the DHCP request was sent
		 * @param client	The client to which any unicasted responses are 
		 *				to be forwarded
		 */
		...
		while(isOk && m_status == RUNNING)
		{
			try
			{
				...
				else
				{
					...
					DatagramPacket pkt = new DatagramPacket(dhcp,
										dhcp.length,
										msg.getAddress(),
										DHCP_TARGET_PORT);
					try
