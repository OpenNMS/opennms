	...
	implements EventListener
{
	 ...
	 * is taken.
	 *
	 * @param thresholdableServices List of all the ThresholdableService objects 
	 * 			  scheduled for thresholding.
	 *
	 */
	...
	/**
	 * This method may be invoked by the garbage thresholding. Once
	 ...
	 * Process the event.  
	 *
	 * This event is generated when a managed node which supports SNMP
	 * gains a new interface.  In this situation the ThresholdableService
	 * object representing the primary SNMP interface of the node must
	 * be reinitialized.
	 * 
	 * The ThresholdableService object associated
	 * with the primary SNMP interface for the node will be marked
	 * for reinitialization.  Reinitializing the ThresholdableService
	 * object consists of calling the ServiceThresholder.release() method 
	 * followed by the ServiceThresholder.initialize() method which will refresh
	 * attributes such as the interface key list and number of
	 * interfaces (both of which most likely have changed).
	 *
		...
		synchronized(m_thresholdableServices)
		{
			...
			while (iter.hasNext())
			{
				...
				if (addr.getHostAddress().equals(event.getInterface()))
				{
					synchronized(tSvc)
					{
						// Got a match!  Retrieve the ThresholderUpdates object associated
						...
						if (log.isDebugEnabled())
							log.debug("reinitializePrimarySnmpInterfaceHandler: marking " + 
								event.getInterface() + " for reinitialization for service SNMP.");
					}
				}
			}
		}
	...
	/**
	 * Process the event, construct a new ThresholdableService object representing
	 * the node/interface combination, and schedule the interface
	 * for thresholding. 
	 * 
	 ...
	 * event parms.  Any ThresholdableService objects located in the collectable 
	...
	private void primarySnmpInterfaceChangedHandler(Event event)
	{
		...
		// Extract the old and new primary SNMP interface adddresses from the event parms.
		//
		...
		{
			// Mark the service for deletion so that it will not be rescheduled for 
			// thresholding.
			//
			// Iterate over the ThresholdableService objects in the service updates map 
				...
				while (liter.hasNext())
				{
					...
					if (addr.getHostAddress().equals(oldPrimaryIfAddr))
					{
						synchronized(tSvc)
						{
							...
							if (log.isDebugEnabled())
								log.debug("primarySnmpInterfaceChangedHandler: marking " + oldPrimaryIfAddr + 
	 ...
	 * associated with it.  All ThresholdableService objects in the service 
	 ...
	 * for thresholding all of the RRDs associated with the old nodeId are 
	...
	private void serviceDeletedHandler(Event event)
	{
		...
		synchronized(m_thresholdableServices)
		{
			...
			while (liter.hasNext())
			{
				...
				if ( !(tSvc.getNodeId() == nodeId && 
						addr.getHostName().equals(ipAddr)) &&
						tSvc.getServiceName().equals(svcName) )
					continue;
