	...
	implements ServiceThresholder
{
	...
	/**
	 * Interface attribute key used to store a map of node level 
	...
	/**
	 * We must maintain a map of interface level ThresholdEntity objects
	 * on a per interface basis in order to maintain separate exceeded
	 * counts and the like for each of a node's interfaces.  This 
	 * interface attribute key used to store a map of interface level 
	 * ThresholdEntity object maps keyed by ifLabel.  So it wil refer
	 * to a map of maps indexed by ifLabel.
	 */
	...
	public void initialize(NetworkInterface iface, Map parameters)
	{
		...
		// Add node and interface thresholding maps as attributes of the interface 
		...
		try
		{
			...
			if (log.isDebugEnabled())
				log.debug("initialize: db retrieval info: nodeid = " + nodeId + ", address = " + ipAddr.getHostAddress()
					  + ", ifIndex = " + primaryIfIndex + ", isSnmpPrimary = " + isSnmpPrimary);
	...
	public int check(NetworkInterface iface, EventProxy eproxy, Map parameters)
	{
		...
		if (log.isDebugEnabled())
			log.debug("check: service= " + SERVICE_NAME + " address= " + primary.getHostAddress() + 
				" thresholding-group=" + groupName + " interval=" + interval + "ms");
	 ...
	 * 
	 * @param path			RRD repository directory
				...
				java.util.Date  date)
	{
		...
		if (log.isDebugEnabled()) 
		{
			log.debug("createEvent: nodeId=" + nodeId + " primaryAddr=" + primary + 
				" ds=" + threshold.getDsName() + " uei=" + uei);
				
			if (ifDataMap != null)
			{
				log.debug("createEvent: specific interface data:" + 
					" ifAddr=" + (String)ifDataMap.get("ipaddr") +
					" macAddr=" + (String)ifDataMap.get("snmpphysaddr") +
					" ifName=" + (String)ifDataMap.get("snmpifname") +
					" ifDescr=" + (String)ifDataMap.get("snmpifdescr") +
					" ifIndex=" + (String)ifDataMap.get("snmpifindex") +
					" ifLabel=" + (String)ifDataMap.get("iflabel"));
			}
		}
