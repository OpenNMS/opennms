...
/**
 * <P>The ThresholdableService class ...</P>
 *
	...
	implements ReadyRunnable
{
	 ...
	 * that are passed to the thresholder.
	 */
	 ...
	 * threshold check again then a value of true is returned. If the
	...
	/**
	 * Checks the package information for the thresholdable service and determines
	 ...
	 * 
	 * @return false if no outage found (indicating thresholding may be performed)
	 * or true if applicable outage is found (indicating thresholding should be skipped).
	 */
	...
	private boolean processUpdates()
	{
		...
		synchronized(this)
		{
			...
			if (m_updates.isDeletionFlagSet())
			{
				...
				if(log.isDebugEnabled())
					log.debug("Collector for  " + m_address.getHostAddress() + 
						" is marked for deletion...skipping thresholding, will not reschedule.");
			...
			if (m_updates.isReinitializationFlagSet())
			{
				...
				{
					log.warn("Unable to reschedule " + m_address.getHostAddress() + 
						" for " + m_service.getName() + " thresholding, reason: " + rE.getMessage());
				}
				catch(Throwable t)
				{
					log.error("Uncaught exception, failed to reschedule interface " + m_address.getHostAddress() + 
							" for " + m_service.getName() + " thresholding", t);
				}
			}
