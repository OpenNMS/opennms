...
/**
 * This class is used as a thread for launching and executing
 * actions as they are discovered by the action daemon. The
 * actions are read from an execution queue and the processes
 * are created by the fiber. Each created process is added to
 * garbage collection list that is periodically polled and
 * culled based upon the status of the process or how long the
 * process is run. If the process has run long than allocated
	...
	implements Runnable, PausableFiber
{
	...
	/**
	 * The worker thread that executes the 
	...
	/**
	 * This class is designed to encapsulated a process and its
	 * start time. The start time is based upon the system clock
	 * and the runtime is the difference between the current time
	...
	private static final class DatedProc
	{
		...
		/**
		 * Returns the command being run by the dated
		 * process.
		 */
	...
	/**
	 * This class encapsules a singular run method that is used
	 * to <em>garbage collect</em> expired processes. If a process
	 * has exceeded its maximum runtime then it is killed and 
	...
	{
		/**
		 * The reaper execution enviroment. This method
		 * scans the process array and removes expired and
		 * completed commands from the array on a periodic
		 * basis. In that respect it is a garbage collection
		 * thread for processes.
		 *
		 */
		public void run()
		{
	...
	/**
	 * <p>Converts a single command to an array that can be passed to the
	 * {@link java.lang.Runtime#exec(java.lang.String[]) exec} system
	 * call. The element at index zero of the array is the name of 
	 * the executable to run. Indexs [1..length) are the arguments
	 * passed to the executable command.</p>
	 *
	 * <p>The input command has is white space trimmed before processing.
	 * The basic processing is to split on spaces, except when a double
	 * quote or single quote is encountered. Also backspaces(\) should also
	 * be handled correctly both in and out of the quotes. Shell escapes
	 * with <em>$</em> are not supported.</p>
	 *
	 * @param cmd	The command to split into an array.
	 *
	 ...
	 * The constructor takes two arguments that define the
	 * source of commands to be executed and the maximum time
	 * that a command may run.
	 *
	 * @param execQ		The execution queue
	 * @param maxRunTime	The maximum runtime of a process.
	 *
	 */
	...
	/**
	 * Stops a currently running fiber. If the fiber has already been
	 * stopped then the command is silently ignored. If the fiber was
	 * never started then an exception is generated.
	 *
	 * @throws java.lang.IllegalStateException Thrown if the fiber was never started.
	 *
	 */
	...
	/**
	 * Pauses a currently running fiber. If the fiber was not in
	 * a running or resuming state then the command is silently
	 * discarded. If the fiber is not running or has terminated
