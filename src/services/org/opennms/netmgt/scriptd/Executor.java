...
final class Executor implements Runnable, PausableFiber
{
	 ...
	 * @param config	The <em>Scriptd</em> configuration.
	 */
	 ...
	 * encapsulated thread to read events from the execution queue
	 * and to execute any configured scripts, allowing these scripts 
	 * to react to the received event. If the thread is interrupted
	 * or the status changes to <code>STOP_PENDING</code> then the method
