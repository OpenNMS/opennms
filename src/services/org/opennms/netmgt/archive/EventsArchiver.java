...
{
	/**
	 * The SQL statement to select events that have their
	 * eventCreateTime earlier than a specified age 
	 */
	...
	private void archiveEvents()
	{
		...
		try
		{
			...
			while(eventsRS.next())
			{
				...
				// log = N, display = Y, delete event only if event has been acknowledged
				...
				// log = Y, display = Y, log and delete event only if event has been acknowledged
				else
				{
					if (eventAckUser != null)
					{
	 ...
	 *
	 * @exception SQLException thrown if there is an error getting column values from the result set
	 */
	...
	/**
	 * Close the database statements and the connection and close
	 * log4j Appenders and categories
	 */
	private void close()
	{
		//
	 ...
	 * the database connection and the prepared statements to select and
	 * delete events
	 */
