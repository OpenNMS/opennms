...
final class SnmpCollector implements ServiceCollector {
     ...
     * node.
     */
    private static final String SQL_GET_SNMPIFALIASES =
        "SELECT snmpifindex, snmpifalias "
            + "FROM snmpinterface "
            + "WHERE nodeid=? "
            + "AND snmpifalias != ''";
     ...
     */
    private static final String SQL_GET_LATEST_FORCED_RESCAN_EVENTID =
        "SELECT eventid "
            + "FROM events "
     ...
     * node.
     */
    private static final String SQL_GET_LATEST_RESCAN_COMPLETED_EVENTID =
        "SELECT eventid "
            + "FROM events "
            + "WHERE nodeid=? "
    ...
    private static final String SQL_GET_NODEID =
        "SELECT nodeid,ifindex,issnmpprimary "
            + "FROM ipinterface "
            + "WHERE ipaddr=? "
            + "AND ismanaged!='D'";

    /**
     * SQL statement to retrieve interface's 'issnmpprimary' table
     * information.
     */
    private static final String SQL_GET_ISSNMPPRIMARY =
        "SELECT ifindex,issnmpprimary "
            + "FROM ipinterface "
            + "WHERE nodeid=?";
     ...
     */
    private static final String SQL_GET_NODESYSOID =
        "SELECT nodesysoid "
            + "FROM node "
            + "WHERE nodeid=? "
            + "AND nodetype!='D'";
     ...
     */
    private static final String SQL_CHECK_SNMPV2 =
        "SELECT ifservices.serviceid "
    ...
    private static final String SQL_GET_SNMP_INFO =
        "SELECT DISTINCT snmpifindex, snmpiftype, snmpifname, "
     ...
     * datacollection-config XML file. "primary" = only primary SNMP interface
     * should be collected and stored "all" = all primary SNMP interfaces
     ...
     * permitted to pack into a single outgoing PDU. This value is
     * intentionally kept relatively small in order to communicate
     * successfully with the largest possible number of agents.
     */
    private static int DEFAULT_MAX_VARS_PER_PDU = 30;

    /**
     * Max number of variables permitted in a single outgoing SNMP PDU
     * request.
     */
    ...
    /**
     * Interface attribute key used to store the number of interfaces
    ...
    /**
     * Interface attribute key used to store a NodeInfo object which holds
    ...
    public void initialize(NetworkInterface iface, Map parameters) {
        ...
        /*
         * Determine if data to be collected for all interfaces or only for
         * the primary SNMP interface.
         */
        String storageFlag = DataCollectionConfigFactory.getInstance().getSnmpStorageFlag(
                                                                                          collectionName);
        if (storageFlag == null) {
            if (log.isEnabledFor(Priority.WARN)) {
                log.warn("initialize: Configuration error, failed to "
        ...
        int maxVarsPerPdu = DataCollectionConfigFactory.getInstance().getMaxVarsPerPdu(
                                                                                       collectionName);
        if (maxVarsPerPdu == -1) {
            if (log.isEnabledFor(Priority.WARN)) {
                log.warn("initialize: Configuration error, failed to "
             ...
             * Integer.MAX_VALUE. This is a lot easier than building in
             * special logic to handle a value of zero. Doubt anyone will
             * attempt to collect over 2 billion oids.
             */
            maxVarsPerPdu = Integer.MAX_VALUE;
        }

        // Add max vars per pdu value as an attribute of the interface
        iface.setAttribute(MAX_VARS_PER_PDU_STORAGE_KEY,
                           new Integer(maxVarsPerPdu));
        if (log.isDebugEnabled()) {
            log.debug("initialize: maxVarsPerPdu=" + maxVarsPerPdu);
        }

        /*
         * Get database connection in order to retrieve the nodeid, ifIndex
         * and sysoid information from the database for this interface.
         */
        ...
        try {
            /*
             * Prepare & execute the SQL statement to get the 'nodeid' from
             * the ipInterface table 'nodeid' will be used to retrieve the
             * node's system object id from the node table. In addition to
             * nodeid, the interface's ifIndex and isSnmpPrimary fields are
             * also retrieved.
             */
            ...
            if (log.isDebugEnabled())
                log.debug("initialize: db retrieval info: nodeid = " + nodeID
             ...
             * node id is invalid - primaryIfIndex is invalid - Interface is
             * not the primary SNMP interface for the node
             */
            if (nodeID == -1) {
            ...
            if (primaryIfIndex == -1) {
                ...
                if (log.isDebugEnabled()) {
                    log.debug("initialize: db retrieval info: node " + nodeID
            ...
            try {
                ...
                try {
                    ...
                    log.info("initialize: an error occured trying to close "
                            + "an SQL statement", e);
                }
            }
             ...
             * objects pertain to the node itself not any individual
             * interfaces.
             */
            List oidList = DataCollectionConfigFactory.getInstance().getMibObjectList(
                                                                                      collectionName,
                                                                                      sysoid,
                                                                                      ipAddr.getHostAddress(),
                                                                                      -1);
            ...
            try {
                ...
                try {
                    ...
                    log.info("initialize: an error occured while closing "
                            + "an SQL statement", e);
                }
            }

            if (log.isDebugEnabled()) {
                log.debug("initialize: address = " + ipAddr.getHostAddress()
                        + ", nodeid = " + nodeID + ", primaryIfIndex = "
                        + primaryIfIndex + ", isSnmpPrimary = "
                        + isSnmpPrimary + ", SNMPversion = "
             ...
             * physAddr values for all valid interfaces for the specified
             * node. For each interface retrieved from the database: 1.
             * Determine the MIB objects to be collected for the interface as
             * well as the corresponding RRD data source list. 2. Create the
             * RRD file to hold data retrieved for the interface. 3. Add the
             * interface to the interface map for retrieval during the poll.
             */
            ...
            try {
                ...
                /*
                 * The following code does a database lookup on the
                 * ipinterface table and builds a Map of ifIndex and
                 * issnmpprimary values. The issnmpprimary value can then be
                 * checked to see if SNMP collection needs to be done on it.
                 */
                ...
                if (log.isDebugEnabled()) {
                    log.debug("initialize: Attempting to get issnmpprimary "
                ...
                while (rs1.next()) {
                    ...
                    if (currSNMPPriValue == null) {
                        ...
                        snmppriMap.put(snmppriIfIndex, snmppriCollType);
                            ...
                            && (snmppriCollType.equals("P") || snmppriCollType.equals("S"))) {
                        snmppriMap.put(snmppriIfIndex, snmppriCollType);
                    } else {
                        snmppriMap.put(snmppriIfIndex, snmppriCollType);
                    }
                ...
                while (rs.next()) {
                    ...
                    if (log.isDebugEnabled()) {
                        log.debug("initialize: snmpifindex = " + index
                                + ", snmpifname = " + name
                                + ", snmpifdescr = " + descr
                                + ", snmpphysaddr = -" + physAddr + "-");
                    }

                    /*
                     * Determine the label for this interface. The label will
                     * be used to create the RRD file name which holds SNMP
                     * data retreived from the remote agent. If available
                     * ifName is used to generate the label since it is
                     * guaranteed to be unique. Otherwise ifDescr is used. In
                     * either case, all non alpha numeric characters are
                     * converted to underscores to ensure that the resuling
                     * string will make a decent file name and that RRD won't
                    ...
                    if (name != null) {
                        ...
                        log.warn("Interface (ifIndex/nodeId="
                                + index
                                + "/"
                                + nodeID
                    ...
                    if (physAddr != null) {
                        ...
                        if (physAddr.length() == 12) {
                            ...
                            if (log.isDebugEnabled()) {
                                log.debug("initialize: physical address len "
                                        + "is NOT 12, physAddr=" + physAddr);
                            }
                        }
                    }
                     ...
                     */
                    oidList = DataCollectionConfigFactory.getInstance().getMibObjectList(
                                                                                         collectionName,
                                                                                         sysoid,
                                                                                         ipAddr.getHostAddress(),
                                                                                         type);
                    ...
                    /*
                     * Now build a list of RRD data source objects from the
                     * list of mib objects
                     */
             ...
             * interface. If no node objects and no interface objects then
             * throw exception
             */
            ...
            try {
                ...
                log.info("initialize: SQLException while closing database "
                        + "connection", sqle);
            }
        }
        ...
        if (log.isDebugEnabled()) {
            log.debug("initialize: initialization completed for "
                    + ipAddr.getHostAddress());
        }
        return;
    }
     ...
     *            belongs.
     */
    public int collect(NetworkInterface iface, EventProxy eproxy,
            Map parameters) {
        try {
            ...
            if (!getNodeInfo(iface).getOidList().isEmpty()) {
                nodeCollector = new SnmpNodeCollector(
                                                      getInetAddress(iface),
            ...
            boolean rrdError = updateRRDs(getCollectionName(parameters),
                                          iface, nodeCollector, ifCollector,
                                          parameters, eproxy);
    ...
    private void collectData(NetworkInterface iface,
        ...
        try {
            ...
            SnmpWalker walker = SnmpUtils.createWalker(
                                                       ...
                                                       (CollectionTracker[]) trackers.toArray(new CollectionTracker[trackers.size()]));
            walker.start();
    ...
    private NodeInfo getNodeInfo(NetworkInterface iface)
            throws CollectionError {
        NodeInfo nodeInfo = (NodeInfo) iface.getAttribute(NODE_INFO_KEY);
        if (nodeInfo == null) {
            throw new CollectionError(
                                      "Node info not available for interface "
                                              + getHostAddress(iface));
        }
        return nodeInfo;
    }
     ...
     * latest SNMP-collected values into the interface's RRD database.
     * 
    ...
    private boolean updateRRDs(String collectionName, NetworkInterface iface,
         ...
         * node level RRD info will be updated. Secondly the interface level
         * RRD info will be updated.
         */
            ...
            while (iter.hasNext()) {
                ...
                try {
                    ...
                    if (dsVal == null) {
                        ...
                        // ds);
                        if (ds.performUpdate(collectionName,
                                             ipaddr.getHostAddress(),
                                             nodeRepository, ds.getName(),
                                             dsVal)) {
                            log().warn(
        ...
        if (ifCollector != null) {
            String domain = ParameterMap.getKeyedString(parms, "domain",
                                                        "default");
            String storeByNodeID = ParameterMap.getKeyedString(
                                                               parms,
                                                               "storeByNodeID",
                                                               "normal");
            String storeByIfAlias = ParameterMap.getKeyedString(
                                                                parms,
                                                                "storeByIfAlias",
                                                                "false");
            String storFlagOverride = ParameterMap.getKeyedString(
                                                                  parms,
                                                                  "storFlagOverride",
                                                                  "false");
            String ifAliasComment = ParameterMap.getKeyedString(
                                                                parms,
                                                                "ifAliasComment",
                                                                null);
            ...
            if (log().isDebugEnabled() && storeByIfAlias.equals("true")) {
                log().debug(
                            "domain:storeByNodeID:storeByIfAlias:"
                                    ...
                                    + storeByIfAlias + ":" + storFlagOverride
                                    + ":" + ifAliasComment);
            }
            ...
            while (iter.hasNext()) {
                ...
                if (aliasVal != null && !aliasVal.equals("")) {
                    ...
                    if (aliasVal != null && !aliasVal.equals("")) {
                        aliasVal = AlphaNumeric.parseAndReplaceExcept(
                                                                      aliasVal,
                                                                      nonAnRepl,
                                                                      AnReplEx);
                    }
                ...
                while (i.hasNext()) {
                    ...
                    /*
                     * Build path to interface RRD repository. createRRD()
                     * will make the appropriate directories if they do not
                     * already exist.
                     */
        ...
        if (forceRescan) {
            generateForceRescanEvent(ipaddr.getHostAddress(),
                                     nodeInfo.getNodeId(), eproxy);
        }
        return rrdError;
    }
    ...
    private Map getIfAliasesFromDb(int nodeID) {
        ...
        try {
            ...
            try {
                ...
                log.info("SQLException while closing database connection",
                         sqle);
            }
        }
        return ifAliasMap;
