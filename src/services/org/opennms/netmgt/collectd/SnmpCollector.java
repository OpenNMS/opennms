	...
	implements ServiceCollector
{
	 ...
	 */
	private static final String 	INTERFACES_IFNUMBER = ".1.3.6.1.2.1.2.1"; // MIB-II interfaces.ifNumber
	...
	/** 
	 * This defines the default maximum number of variables the
	 * collector is permitted to pack into a single outgoing
	 * PDU.  This value is intentionally kept relatively
	 * small in order to communicate successfully with
	 * the largest possible number of agents.  
	 */
	...
	/** 
	 * Interface attribute key used to store the map of IfInfo objects 
	 * which hold data about each interface on a particular node.
	 */
	static String IF_MAP_KEY = "org.opennms.netmgt.collectd.SnmpCollector.ifMap";
	
	/**
	 * Interface attribute key used to store a NodeInfo object 
	 * which holds data about the node being polled.
	 */
	static String NODE_INFO_KEY = "org.opennms.netmgt.collectd.SnmpCollector.nodeInfo";
	
	/**  
	 * Interface attribute key used to store the data collection scheme
	...
	/**  
	 * Interface attribute key used to store configured value for the
	 * maximum number of variables permitted in a single outgoing SNMP
	 * PDU request.
	 */
	static String MAX_VARS_PER_PDU_STORAGE_KEY = "org.opennms.netmgt.collectd.SnmpCollector.maxVarsPerPdu";
	
	/**
	 * <P>Returns the name of the service that the plug-in collects ("SNMP").</P>
	 *
	 ...
	 * the specified interface in preparation for data collection.
	 * 
	 * @param iface		Network interface to be prepped for collection.
	...
	public void initialize(NetworkInterface iface, Map parameters)
	{
		...
		try
		{
			...
			if (log.isDebugEnabled())
				log.debug("initialize: db retrieval info: nodeid = " + nodeID + ", address = " + ipAddr.getHostAddress()
					  + ", primaryIfIndex = " + primaryIfIndex + ", isSnmpPrimary = " + isSnmpPrimary);
			...
			{
				// Thrown by ResultSet.getString() if database query did not return anything
				if (log.isDebugEnabled())
			...
			// Our implmentation requires that all sysObjectID's must have a leading period ('.').
			...
			// Prepare & execute the SQL statement for retrieving the SNMP version of node
			//		
			try 
			{
			...
			if (log.isDebugEnabled())
			{
				log.debug("initialize: address = " + ipAddr.getHostAddress()
					  ...
					  + ", SNMPversion = " + (snmpv2Supported?"SNMPv2":"SNMPv1"));
			}

			// Build object list for each interface
			//
			// Prepare & execute the SQL statement to retrieve all ifIndex, 
			// ifType, ifName, ifDescr, & physAddr values for all valid interfaces for 
			// the specified node.
			//
			...
			// Verify that we did find at least one eligible interface for the node
			//
			if (ifMap.size() < 1)
		...
		if (log.isDebugEnabled())
			log.debug("initialize: address = " + ipAddr.getHostAddress()
				  + ", nodeID = " + nodeID + ", ifIndex = " + primaryIfIndex
				  + ", sysoid = " + sysoid);
	...
	public int collect(NetworkInterface iface, EventProxy eproxy, Map parameters)
	{
		...
		if (log.isDebugEnabled())
			log.debug("collect: service= " + SERVICE_NAME + " address= " + ipaddr.getHostAddress() + 
		...
		{
			String nl = System.getProperty("line.separator");
			log.debug("collect: SnmpPeer configuration: address: " + peer.getPeer() +
				  nl + "      version: " + ((peer.getParameters().getVersion() == SnmpSMI.SNMPV1)?"SNMPv1":"SNMPv2") +
				  nl + "      timeout: " + peer.getTimeout() +
				  nl + "      retries: " + peer.getRetries() +
				  nl + "      read commString: " + peer.getParameters().getReadCommunity() +
		...
		try
		{
			...
			if (hasInterfaceOids)
			{
				...
				SnmpPduPacket out = new SnmpPduRequest(SnmpPduPacket.GETNEXT,
						new SnmpVarBind[] { new SnmpVarBind(new SnmpObjectId(".1.3.6.1.2.1.2.1")) });
				...
				// Add the interface count to the interface's attributes for retrieval during poll()
				...
				if (savedIfCount != -1)
				{
					...
					{
						log.info("Number of interfaces on primary SNMP interface " + 
							ipaddr.getHostAddress() + 
							" has changed, generating 'ForceRescan' event. " +
							"Data collection will not be performed.");
			...
			{
				BarrierSignaler blocker = new BarrierSignaler(1); // Object to be signaled by SNMPCollector when collection is complete
				synchronized(blocker)
				{
					try
					{
						nodeCollector = new SnmpNodeCollector(session,  
											blocker,  
											nodeOidList, 
											maxVarsPerPdu.intValue());
						...
						// Seen scenario where collector experiences an SnmpInternalError on its
						// SNMP session before the thread even returns from instantiating the collector.
						// In this scenario the "error" flag will already be set for the collector..
			...
			if (hasInterfaceOids)
			{
				...
				synchronized(blocker)
				{
					try
					{
						ifCollector = new SnmpIfCollector(session,  
											blocker, 
											String.valueOf(nodeInfo.getPrimarySnmpIfIndex()),
											ifMap,
											ifCount,
											maxVarsPerPdu.intValue());
	...
	/**
	 * Retrieves ifNumber object from the MIB-II interfaces table 
	 * which is the number of interfaces on the remote node and
	 * then returns this value.
	 * 
	 * @param peer	SnmpPeer object used to communicate with
	...
	private int getInterfaceCount(SnmpPeer peer)
	{
		...
		{
			SnmpResponseHandler handler = new SnmpResponseHandler();
			SnmpPduPacket pkt = new SnmpPduRequest(SnmpPduPacket.GETNEXT,
				new SnmpVarBind[] { new SnmpVarBind(new SnmpObjectId(INTERFACES_IFNUMBER)) });
