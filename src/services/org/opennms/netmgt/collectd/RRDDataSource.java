...
{
	/**
	 * Defines the list of supported (MIB) object types whic may be mapped
	 * to one of the supported RRD data source types.  Currently the 
	 * only two supported RRD data source types are: COUNTER & GAUGE.
	 * A simple string comparison is performed against this list of 
	 * supported types to determine if an object can be represented
	 * by an RRD data source.
	 * NOTE:  String comparison uses String.startsWith() method so 
	 * "counter32" & "counter64" values will match "counter" entry.
	 * Comparison is case sensitive.
	 */
	private static final String[] supportedObjectTypes = new String[] {
			"counter",
			"gauge", 
			"timeticks",
			"integer",
	 ...
	 * Identifies the MIB object which is the source of data values for this
	 * RRD data source.
	 */
	private String m_oid;
	
	/** 
	 * Instance identifier which is appended to the object identifier to identify
	 * a particular MIB entry.  This value may be an integer value or "ifIndex".
	 */
	...
	/**
	 * Data Source Heartbeat.  This is the maximum number of seconds that may pass
	 * between updates of this data source before the value of the data source is 
	 ...
	 * Together with m_max defines the expected range of the data supplied by this data source.
	 * May be set to "U" for 'Unknown'.
	 */
	private String m_min;
	
	/** 
	 * Maximum Expected Range.
	 * Together with m_min defines the expected range of the data supplied by this data source.
	 * May be set to "U" for Unknown.
	 */
	 ...
	 * If the object type cannot be mapped to an RRD type, null is returned.
	 * RRD only supports integer data so MIB objects of type 'octetstring' 
	 * are not supported.
	 *
	 * @param objectType - MIB object type to be mapped. 
	 *
	...
	*/
	...
	/**
	 * Class copy constructor. Constructs a new
	 * object that is an identical to the passed
	 * object, however no data is shared between
	 * the two objects.  Any changes to one will
	 * not affect the other.
	 *
	 * @param second The object to make a duplicate of.
	 *
	 */
	 ...
	 *
	 * @param instance - instance identifier (to be appended to oid)
	 */
	...
	/**
	 * Used to get a duplicate of self. The duplicate
	 * is identical to self but shares no common data.
	 *
	 ...
	 * the content of this RRDDataSource object.  Primarily used for debugging purposes.
	 * 
