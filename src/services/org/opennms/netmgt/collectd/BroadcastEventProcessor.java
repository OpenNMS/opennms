	...
	implements EventListener
{
	 ...
	 * gains a new interface.  In this situation the CollectableService
	 * object representing the primary SNMP interface of the node must
	 * be reinitialized.
	 * 
	 * The CollectableService object associated
	 * with the primary SNMP interface for the node will be marked
	 * for reinitialization.  Reinitializing the CollectableService
	 * object consists of calling the ServiceCollector.release() method 
	 * followed by the ServiceCollector.initialize() method which will refresh
	 * attributes such as the interface key list and number of
	 * interfaces (both of which most likely have changed).
	 *
	 * Reinitialization will take place the next time the CollectableService
	 * is popped from an interval queue for collection.
	 * 
		...
		synchronized(m_collectableServices)
		{
			...
			while (iter.hasNext())
			{
				...
				if (addr.getHostAddress().equals(event.getInterface()))
				{
					synchronized(cSvc)
					{
						// Got a match!  Retrieve the CollectorUpdates object associated
	 ...
	 * event parms.  Any CollectableService objects located in the collectable 
	 ...
	 * examined and new CollectableService objects are created, initialized
	...
	private void primarySnmpInterfaceChangedHandler(Event event)
	{
		...
		if (oldPrimaryIfAddr != null)
		{
			...
			// collection.
			//
			// Iterate over the CollectableService objects in the service updates map 
