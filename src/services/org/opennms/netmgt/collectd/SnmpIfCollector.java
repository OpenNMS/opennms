...
public class SnmpIfCollector implements SnmpHandler {
    ...
    /**
     * Copy of m_objList list that will used solely by version 2. As each object
     * is collected from the remote node via subsequent SNMPv2 GetBulk commands
     * the MibObject is removed and the collection for the next object started.
     */
    ...
    /**
     * Used for keeping track of all the possible indexes. Needed for generating
     * packets for SNMP v1.
     * 
     */
     ...
     * Once all the data is gathered, we can generate maps of the data, one map
     * per interface.
     */
     ...
     * 
     * For SNMPv2, this used for keeping track of the total number of variable
     * bindings returned during each subsequent MIB object collection. This
     * variable is reset back to 0 after the MIB object has been collected for
     * all interfaces.
     */
     ...
     */
    private SnmpIfCollector() throws UnsupportedOperationException {
     ...
     * @param primaryIfIndex
     *            The ifIndex value linked to the primary IP.
     * @param ifMap
     *            Map of org.opennms.netmgt.poller.collectd.IfInfo objects.
     * @param ifCount
     *            Number of interfaces found in node's MIB-II ifTable
     * @param maxVarsPerPdu
     *            Max number of variables permitted in a single PDU.
     */
     ...
     * remote node.
     * 
     * For SNMPv2, GETBULK commands are built. Each constructed PDU contains a
     * single varbind representing the next object to be retrieved. If that
     * object pertains to all the remote node's interfaces (such as ifOctetsIn
     * or ifOctetsOut) then maxRepetitions will be set to the number of
     * interfaces in the ifTable and the object will be collected for every
     * interface with a single GETBULK pdu. If the oid is a singular instance
     * which doesn't pertain to all interfaces then a GETBULK pdu is built with
     * nonRepeaters set to 1 and maxRepetitions set to 0 and only that
     * individual value is collected. Therefore, for SNMPv2 based collection the
     * number of PDUs generated will be equal to (numInterfaces *
     * numInterfaceBasedObjects) + (numSingularObjects).
     * 
     * @param ifAddress
     *            Interface address of the remote agent
     * 
    ...
    private SnmpPduPacket getNextSnmpV2Pdu(String ifAddress) {
        ...
        if (instance == null) {
            ...
            // Create new SnmpObjectId using object's identifer appended with
            // the instance id
            //
        ...
        if (command == SnmpPduPacket.RESPONSE) {
            try {
                ...
                if (m_errorStatus != SnmpPduPacket.ErrNoError) {
                    ...
                    if (log.isEnabledFor(Priority.WARN))
                        log.warn("snmpReceivedPDU: Error during interface SNMP collection for interface " + session.getPeer().getPeer().toString() + ", SNMP error text: " + m_errorText[m_errorStatus]);
