 ...
 * daemon to test if a remote interface is running a functional
 * DHCP server as defined by RFC 2131.
 * 
...
{
	/**
	 * The port where the DHCP server is detected.  This is a well known
	 * port and this integer is always returned in the qualifier map.
	 */
	private final static Integer	PORT_NUMBER	= new Integer(67);

	/**
	 * <P>The protocol name of the plugin.</P>
	 */
	...
	/**
	 * This method returns the name of the protocol supported
	 * by this plugin. 
	 *
	 ...
	 * The remote host is queried using the DHCP protocol by sending a 
	 * formatted datagram to the DHCP server port. If a response is received
	 * by the DHCP listenter that matches our original request then a value
	 * of true is returned to the caller.
	 *
	 * @param host	The remote host to test.
	 *
	 * @return True if the remote interface responds to the DHCP request,
	 * false otherwise
	 */
	 ...
	 *
	 * @param host		The remote host to test.
	 * @param qualifiers	The location where qualifier parameters are read
	 * 	and written.
	 *
	 * @return True if the remote interface responds to the DHCP request,
	 * 	false otherwise
