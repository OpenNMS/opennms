...
public final class DbIpInterfaceEntry
{
	 ...
	 * associated with this interface. Once the identifiers are known then 
	 * a list of DbIfServiceEntry(s) can be returned to the caller.
	 */
	private static final String	SQL_LOAD_IFSVC_LIST = "SELECT serviceID FROM ifServices WHERE nodeID = ? AND ipAddr = ? AND status != 'D'";

	/**
	 * This is the SQL statement used to load a record when the ifIndex
	 * is involved as part of the key lookup. This is mainly used when
	 * a node has multiple <em>unnamed</em> interfaces (i.e. 0.0.0.0)
	 * but each has a different ifIndex.
	 */
	...
	/**
	 * Constructs a new interface, this constructor will only
	 * work for entries loaded from the database!
	 *
	 * @param nid		The node identifier.
	 * @param address	The target interface address.
	 ...
	 *
	 * @param state The new managed state
	 */
	...
	DbIfServiceEntry[] getServices(Connection db)
		throws SQLException
	{
	 ...
	 *
	 */
	static DbIpInterfaceEntry get(int nid, InetAddress addr)
		throws SQLException
	{
		Connection db = null;
		try
	 ...
	 *
	 */
	static DbIpInterfaceEntry get(int nid, InetAddress addr, int ifIndex)
		throws SQLException
	{
		Connection db = null;
		try
