...
/**
 * Provides a collection of utility methods used by the DeleteEvent Processor for dealing with
 * Events
 * 
 * @author brozow
 *  
...
public class EventUtils {
    ...
    /**
     * Make the given listener object a listener for the list of events referenced in the
     * ueiList.
     * 
     * @param listener
     *            the lister to add
    ...
    static public void checkEventId(Event e) throws InsufficientInformationException {
        ...
        if (!e.hasDbid()) throw new InsufficientInformationException("eventID is unavailable");
    }
    ...
    static public void checkNodeId(Event e) throws InsufficientInformationException {
        ...
        if (!e.hasNodeid()) throw new InsufficientInformationException("nodeid for event is unavailable");
    }
    ...
    public static void checkService(Event e) throws InsufficientInformationException {
        ...
        if (e.getService() == null || e.getService().length() == 0) throw new InsufficientInformationException("service for event is unavailable");
    }
     ...
     * @return an Event represent an interfaceDeleted event for the given interface
     */
     ...
     * @param txNo
     *            a transaction number associated with the event
     * @return an Event that represents the serviceDeleted event for the give triple
     */
    ...
    public static long getEventID(Event e) {
        // get eventid
        long eventID = -1;
        if (e.hasDbid()) eventID = e.getDbid();
        return eventID;
    }

    /**
     * Retrieve the value associated with an event parameter and parse it to a long. If the
     * value can not be found, return a default value.
     * 
     * @param e
     ...
     * @param defaultValue
     *            the value to return if the paramter can not be retrieved or parsed
    ...
    public static long getLongParm(Event e, String parmName, long defaultValue) {
        ...
        if (longVal == null) return defaultValue;
    ...
    public static long getNodeId(Event e) {
        // convert the node id
        long nodeID = -1;
        if (e.hasNodeid()) nodeID = e.getNodeid();
        return nodeID;
    }
     ...
     * @return the value of the parameter, or defalutValue if the parameter is not set
     */
    public static String getParm(Event e, String parmName, String defaultValue) {
        Parms parms = e.getParms();
        if (parms == null) return defaultValue;
     ...
     * @throws InsufficientInformationException
     *             if the paramter is not set on the event or if its value has no content
     */
    public static void requireParm(Event e, String parmName) throws InsufficientInformationException {
        Parms parms = e.getParms();
        if (parms == null) throw new InsufficientInformationException("parameter " + parmName + " required but but no parms are available.");
    ...
    public static void sendEvent(Event newEvent, String callerUei, long txNo, boolean isXmlRpcEnabled) {
        ...
        try {
            ...
            if (log.isDebugEnabled()) log.debug("sendEvent: successfully sent event " + newEvent);
        } catch (Throwable t) {
    ...
    /**
     * This method is responsible for generating a nodeGainedInterface event and sending it to
     * eventd..
     * @param nodeEntry
    ...
    public static Event createForceRescanEvent(String hostName, long nodeId) {
        Category log = ThreadCategory.getInstance(EventUtils.class);
        if (log.isDebugEnabled()) log.debug("createdAndSendForceRescanEvent: processing forceRescan event... ");
    ...
    /**
     * This method is responsible for generating an interfaceDeleted event and sending it to
     * eventd...
     * @param source FIXME
     * @param nodeId
    ...
    /**
     * This method is responsible for generating a nodeGainedService event and sending it to
     * eventd..
     * @param nodeEntry
    ...
    public static Event createNodeGainedServiceEvent(DbNodeEntry nodeEntry, InetAddress ifaddr, String service, long txNo) {
        Category log = ThreadCategory.getInstance(EventUtils.class);
        if (log.isDebugEnabled())
                log.debug("createAndSendNodeGainedServiceEvent:  nodeId/interface/service  " + nodeEntry.getNodeId() + "/" + ifaddr.getHostAddress()
                        + "/" + service);
    ...
    /**
     * This method is responsible for generating a deleteService event and sending it to
     * eventd..
     * @param source FIXME
     * @param nodeEntry
    ...
    public static Event createAndSendDeleteServiceEvent(String source, DbNodeEntry nodeEntry, InetAddress ifaddr, String service, String hostName, long txNo) {
        Category log = ThreadCategory.getInstance(EventUtils.class);
        if (log.isDebugEnabled())
                log.debug("createAndSendDeleteServiceEvent:  nodeId/interface/service  " + nodeEntry.getNodeId() + "/" + ifaddr.getHostAddress()
                        + "/" + service);
    ...
    /**
     * This method is responsible for generating an addInterface event and sending it to
     * eventd..
     * @param source FIXME
     * @param nodeLabel
    ...
    /**
     * This method is responsible for generating a deleteInterface event and sending it to
     * eventd..
     * @param source FIXME
     * @param nodeLabel
     *            the node label of the node where the interface resides.
