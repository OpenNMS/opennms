...
/**
 * This class is designed to reparent interfaces in the database
 * based on the SMB protocol. Specifically, if two nodes in the
 * 'node' table have identical NetBIOS names it is assumed that
 * those two nodes actually represent different interfaces (physical
 * or alias'd) on the same box.  The node with the lowest nodeID
 * becomes the "reparent node" and the other nodes are considered
 * duplicates.  All interfaces under each duplicate node are then
 * reparented under the "reparent node" and the duplicate node(s)
 * are flagged in the database as deleted (nodeType='D').  
 * 
...
public final class ReparentViaSmb
{
	...
	/** 
	 * List of LightWeightNodeEntry objects intialized from the content 
	 * of the 'node' table.
	 */
	private List			m_existingNodeList;
	
	/** 
	 * Contains a mapping of reparent nodes and the list of interfaces
	 * which were reparented under them.
	 */
	private Map			m_reparentedIfMap;
	
	/**
	 * Contains of mapping of reparent nodes and the list of duplicate
	 * nodes associated with them.
	 */
	private Map			m_reparentNodeMap;
	
	/**
	 * Contains hard-coded list of NetBIOS names which are not 
	 * subject to reparenting via SMB.
	 */
	...
	*/
	private static final class LightWeightIfEntry
	{
		...
		*/
		...
		/**
		* <P>Returns the IP hostname of the interface.</P>
		*/
		...
		*/
		...
		/**
		* <P>Returns the old parent node id of the interface.</P>
		*/
	...
	/**
	 * This class is a lighter weight version of the DbNodeEntry
	 * class for use in SMB reparenting.    
	 */
	...
	{	
		...
		/**
		* <P>Constructs a new LightWeightNodeEntry object.</P>
		*
		...
		*/
		...
		/**
		 * <P>Returns the node identifer.</P>
		 */
		int getNodeId()
		{
			return m_nodeId;
		...
		/**
		 * <P>Returns the NetBIOS name of the node.</P>
		 */
		...
		/**
		* <P>Sets the duplicate flag for the node..</P>
		*
		...
		*/
		...
		/**
		* <P>Returns true if this LightWeightNodeEntry object has been
		* marked as a duplicate, false otherwise.</P>
		*/
		boolean isDuplicate()
		{
			return m_duplicate;
		...
		/**
		* <P>Node equality test...currently returns true
		* if the LightWeightNodeEntry objects have the same NetBIOS name.</P>
		*
	...
	/**
	 * The class' default constructor. The constructor will always
	 * throw an exception and not designed to be instantianted without
	 * a database connection and CapsReader object.
	 *
	...
	/**
	 * This method is responsible for building a list of existing nodes
	 * from the 'node' table and then processing that list of nodes in 
	 * order to determine if there are any nodes which must be reparented
	 * because they share the same NetBIOS name with another node.  During
	 * this processing the reparent node map is built which contains a
	 * mapping of reparent nodes to their duplicate node lists.
	 *
	...
	{		
		...
		try
		{
			...
			// Process result set
			// Build list of LightWeightNodeEntry objects representing each of the 
		...
		// under it and it's LightWeightNodeEntry object will serve as the map key.
		...
		while(outer.hasNext())
		{
			...
			while(inner.hasNext())
			{
				...
				if (innerNetbiosName.equals(outerNetbiosName))
				{
					...
					duplicateNodeList.add(innerEntry);  // add to current dup list
	...
	/**
	 * Performs reparenting if necessary and generates appropriate
	 * events to inform other OpenNMS processes of any database changes..
	 *
	 * @throws SQLException if error occurs updating the database
	 */
	public void sync()
		throws SQLException
	{
	...
	/**
	 * This method is responsible for reparenting interfaces belonging
	 * to duplicate nodes under the appropriate reparent node id.  During
	 * this processing the reparented interface map is generated.  This map
	 * contains a list of reparented interfaces associated with each 
	 * reparent node.  This list will make it possible to generate
	 * 'interfaceReparented' events for each reparented interface.
	 *
			...
			while(dupIter.hasNext())
			{
				...
				try
				{
					...
					try
					{
						...
						// Process result set
						// Build list of LightWeightIfEntry objects representing each of the 
						...
						while (rs.next())
						{
							...
							LightWeightIfEntry lwIfEntry = new LightWeightIfEntry(ifAddress,
												hostName,
												reparentNodeID,
												dupNodeID);
							...
							if(log.isDebugEnabled())
								log.debug("reparentInterfaces: will reparent " + lwIfEntry.getAddress() + 
									" : oldNodeId: " + lwIfEntry.getOldParentNodeId() + 
									" newNodeId: " + lwIfEntry.getParentNodeId());
						}
					...
					// Update the 'ifservices' table so that all services associated
	...
	/**
	 * Generates appropriate events to inform other OpenNMS processes 
	 * of the database changes.  Loops through the keys of the
	 * reparent interface and generates 'interfaceReparented' events 
	 * for each reparented interface.
	 */
		...
		while(iter.hasNext())
		{
			...
			if (ifList != null && !ifList.isEmpty())
			{
				...
				while (ifIter.hasNext())
				{
					...
					sendInterfaceReparentedEvent(lwIfEntry.getAddress(), 
								lwIfEntry.getHostName(), 
								lwIfEntry.getParentNodeId(), 
