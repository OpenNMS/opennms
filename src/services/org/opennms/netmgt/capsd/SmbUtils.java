 ...
 * while doing data collection via jCIFS and the SMB (Server Message Block)
 * protocol.
 *
...
public class SmbUtils 
{
	...
	public static final int	INTERNET_INFORMATION_SERVER_G	= 0x1C; // INET~SERVICES (GROUP)
	public static final int	INTERNET_INFORMATION_SERVER_U	= 0x00; // IS~<computername> (UNIQUE)
	...
	/**
	 * This method attempts to determine the authentication domain for
	 * a remote host.  The list of NbtAddress objects is processed in 
	 * order to find an entry with a DOMAIN_NAME (0x00) suffix.  
	 * WORKSTATION_SERVICE and INTERNET_INFORMATION_SERVER share the same
	 * 0x00 suffix so these entries must be ignored while processing 
	 * the address list.
	 * 
	 * @param addresses	List of NbtAddress objects associated with the
	 *  			remote host.
	...
	/**
	 * Returns the operating system label to be associated with
	 * a node in 'node' table in the databse. This call should
	 * be made after an attempt to determine if the interface
	 * supports Microsoft Exchange. This is determined by the
	 * {@link MSExchangePlugin MSExchangePlugin} class.
	 * 
	 * @param nativeOS	OS string returned by jCIFS following SMB
	 * 	session establishment with the remote host.
	 * @param addresses	array of NbtAddress objects associated with
	 *         		the remote host being tested.
	 * @param isSamba	<em>true</em> if it has been derived that the
	 * 	remote system is running Samba.  
	 * @param hasExchange	<em>true</em> if the service supports microsoft
	 *	exhange.
	 *
	...
	static String getOsLabel(String nativeOS, NbtAddress[] addresses, boolean isSamba, boolean hasExchange)
	{
		...
		if (nativeOS == null)
		{
			...
			{
				osLabel = "Windows Server"; // Don't know if we have Win 2000 or NT 4.0
			}
		}
		else if (nativeOS.length() == 0)
		{
			// HACK:  Windows 95/98 boxes don't appear to give us the operating
			// system so if we have successfully enumerate the shares on a server
			// but the operating system return is an emtpy string then we will assume
	...
	static boolean isNTServer(NbtAddress[] addresses)
	{
		...
		if (addresses != null)
		{
			for (int i=0; i<addresses.length; i++)
			{
				...
				else if (nbtAddr.getNameType() == INTERNET_INFORMATION_SERVER_U)
				{
					if (nbtAddr.getHostName().startsWith("IS~"))
					{
						isNTServer = true;
						break;
					}
