...
import org.opennms.netmgt.utils.XmlRpcNotifier;
	...
	implements EventListener
{
	...
	private Scheduler 	m_scheduler;
        ...
        /**
         * URL of the external xmlrpc server
         */
	...
	BroadcastEventProcessor(FifoQueue suspectQ, Scheduler scheduler)
	{
		...
		m_scheduler = scheduler;
                ...
                if (m_xmlrpc)
                        m_xmlrpcServerUrl = CapsdConfigFactory.getInstance().getXmlrpcServerUrl();
			...
			while(parmEnum.hasMoreElements())
			{
		        ...
		        if (log.isDebugEnabled())
                        ...
                        if (m_xmlrpc)
                        {
                                ...
                                {
                                        log.warn("addNodeHandler: XMLRPC failure. ", xe);
                                }
                                catch (IOException ioe)
                                {
                                        log.warn("addNodeHandler: IOException on XMLRPC. ", ioe);
                                }
                        }
	 ...
	 */
	private void addNode(java.sql.Connection conn, String nodeLabel, String ipaddr, long txNo)
	        throws SQLException, java.net.UnknownHostException	
	{
		Category log = ThreadCategory.getInstance(getClass());
			...
			log.debug("addNode:  Add a node " + nodeLabel + " to the database");
                ...
                createAndSendNodeAddedEvent(node, txNo);
                if (m_xmlrpc)
                {
                        try
                        {
                                XmlRpcNotifier notifier = new XmlRpcNotifier(m_xmlrpcServerUrl);
                                String message = new String("Successfully added node: ") + nodeLabel;
                                String reply = notifier.notifySuccess(txNo, EventConstants.ADD_NODE_EVENT_UEI, message); 
		                if (log.isDebugEnabled())
			                log.debug("addNode: Notify failure with xmlrpc: \n\t\t" + reply); 
                        }
                        catch (XmlRpcException xe)
                        {
                                log.warn("addNode: XMLRPC failure. ", xe);
                        }
                        catch (IOException ioe)
                        {
                                log.warn("addNode: IOException on XMLRPC. ", ioe);
                        }
                }
		...
		if (log.isDebugEnabled())
			log.debug("addNode:  Add an IP Address " + ipaddr + " to the database");
                ...
                createAndSendNodeGainedInterfaceEvent(node, ifaddress, txNo);
                if (m_xmlrpc)
                {
                        try
                        {
                                XmlRpcNotifier notifier = new XmlRpcNotifier(m_xmlrpcServerUrl);
                                String message = new String("Successfully added interface: ") + ipaddr + " to node: " + nodeLabel;
                                String reply = notifier.notifySuccess(txNo, EventConstants.ADD_NODE_EVENT_UEI, message); 
		                if (log.isDebugEnabled())
			                log.debug("addNode: Notify failure with xmlrpc: \n\t\t" + reply); 
                        }
         ...
         *
         */
        private void createAndSendNodeAddedEvent(DbNodeEntry nodeEntry, long txNo)
        {
		Category log = ThreadCategory.getInstance(getClass());
		if (log.isDebugEnabled())
			log.debug("createAndSendNodeAddedEvent:  nodeId  " + nodeEntry.getNodeId());
                ...
                catch(Throwable t)
                {
                        ...
                        if (m_xmlrpc)
                        {
		                        ...
		                        if (log.isDebugEnabled())
			                        log.debug("createAndSendNodeAddedEvent: Notify failure with xmlrpc: \n\t\t" + reply); 
                                }
                                catch (XmlRpcException xe)
                                {
                                        log.warn("createAndSendNodeAddedEvent: XMLRPC failure. ", xe);
                                }
                                catch (IOException ioe)
                                {
                                        log.warn("createAndSendNodeAddedEvent: IOException on XMLRPC. ", ioe);
                                }
                        }
                }
        }                
	...
	private void deleteNodeHandler(Event event)
	{
		        ...
		        if (log.isDebugEnabled())
                        ...
                        if (m_xmlrpc)
                        {
                                try
                                {
                                        ...
                                        String reply = notifier.notifyFailure(txNo, event.getUei(), "Invalid parameters");
		                        if (log.isDebugEnabled())
			                        log.debug("deleteNodeHandler: Notify failure with xmlrpc: \n\t\t" + reply); 
                                }
                                catch (XmlRpcException xe)
                                {
                                        log.warn("deleteNodeHandler: XMLRPC failure. ", xe);
                                }
                                catch (IOException ioe)
                                {
                                        log.warn("deleteNodeHandler: IOException on XMLRPC. ", ioe);
                                }
                        }
		...
		try
		{
			...
			while(rs.next())
			{
                        ...
                        if (nodeid == -1)  // Sanity check
		        {
			        log.error("DeleteNode: There is no node with node label: " + nodeLabel + " exists in the database.");
                                ...
                                {
                                        XmlRpcNotifier notifier = new XmlRpcNotifier(m_xmlrpcServerUrl);
                                        String reply = notifier.notifySuccess(txNo, event.getUei(), "Node does not exist in the database.");
		                        if (log.isDebugEnabled())
			                        log.debug("deleteNodeHandler: Notify failure with xmlrpc: \n\t\t" + reply); 
                                }
                                catch (XmlRpcException xe)
			...
			if (log.isDebugEnabled())
				log.debug("deleteNodeHandler: deleted the node with node label: " + nodeLabel);
                        ...
                        createAndSendNodeDeletedEvent(nodeid, event.getHost(), nodeLabel, txNo);
                        if (m_xmlrpc)
                        {
                                try
                                {
         ...
         * @parms nodeLabel     the node label of the deleted node.
         */
        private void createAndSendNodeDeletedEvent(int nodeId, String hostName, String nodeLabel, long txNo)
        {
		Category log = ThreadCategory.getInstance(getClass());
		if (log.isDebugEnabled())
                        ...
                        if (m_xmlrpc)
                        {
                                try
                                {
                                        ...
                                        String reply = notifier.notifyFailure(txNo, newEvent.getUei(), 
                                                "caught unexpected throwable exception.");
		                        if (log.isDebugEnabled())
			                        log.debug("createAndSendNodeDeletedEvent: Notify failure with xmlrpc: \n\t\t" + reply); 
                                }
                                catch (XmlRpcException xe)
                                {
                                        log.warn("createAndSendNodedeletedEvent: XMLRPC failure. ", xe);
                                }
                                catch (IOException ioe)
                                {
                                        log.warn("createAndSendNodeDeletedEvent: IOException on XMLRPC. ", ioe);
                                }
                        }
                }
        }                
	...
	private void deleteInterfaceHandler(Event event)
	{
		        ...
		        if (log.isDebugEnabled())
                        ...
                        if (m_xmlrpc)
                        {
                                try
                                {
                                        ...
                                        String reply = notifier.notifyFailure(txNo, event.getUei(), "Invalid parameters");
		                        if (log.isDebugEnabled())
			                        log.debug("deleteInterfaceHandler: Notify failure with xmlrpc: \n\t\t" + reply); 
                                }
                                catch (XmlRpcException xe)
                                {
                                        log.warn("deleteInterfaceHandler: XMLRPC failure. ", xe);
                                }
                                catch (IOException ioe)
                                {
                                        log.warn("deleteInterfaceHandler: IOException on XMLRPC. ", ioe);
                                }
                        }
		...
		try
		{
			        ...
			        log.error("deleteInterfaceHandler: There is no node with node label: " 
                                ...
                                if (m_xmlrpc)
                                {
                                        try
                                        {
                                                XmlRpcNotifier notifier = new XmlRpcNotifier(m_xmlrpcServerUrl);
                                                String reply = notifier.notifySuccess(txNo, event.getUei(), 
                                                        "No node with the specified node label exists.");
		                                if (log.isDebugEnabled())
			                                log.debug("deleteInterfaceHandler: Notify failure with xmlrpc: \n\t\t" + reply); 
                                        }
                                        catch (XmlRpcException xe)
			...
			if (log.isDebugEnabled())
		        ...
		        rs = stmt.executeQuery();
                        ...
                        if (numOfInterface == 1 )
                        {
                                createAndSendDeleteNodeEvent(event.getHost(), nodeLabel, txNo);
                        }
                        else
                        {
        			...
        			if (log.isDebugEnabled())
        				...
        				log.debug("deleteInterfaceHandler: deleted the ipaddress: " 
                                ...
                                createAndSendInterfaceDeletedEvent(nodeid, event.getHost(), ipaddr, txNo);
                                if (m_xmlrpc)
                                {
                                        try
                                        {
         ...
         */
        private void createAndSendDeleteNodeEvent(String hostName, String nodeLabel, long txNo)
        {
		Category log = ThreadCategory.getInstance(getClass());
		if (log.isDebugEnabled())
                        ...
                        if (m_xmlrpc)
                        {
                                try
                                {
                                        ...
                                        String reply = notifier.notifyFailure(txNo, newEvent.getUei(), 
                                                "caught unexpected throwable exception.");
		                        if (log.isDebugEnabled())
			                        log.debug("createAndSendDeleteNodeHandler: Notify failure with xmlrpc: \n\t\t" + reply); 
                                }
                                catch (XmlRpcException xe)
                                {
                                        log.warn("createAndSendDeleteNodeEvent: XMLRPC failure. ", xe);
                                }
                                catch (IOException ioe)
                                {
                                        log.warn("createAndSendDeleteNodeEvent: IOException on XMLRPC. ", ioe);
                                }
                        }
                }
        }                
	...
	private void changeServiceHandler(Event event)
	{
		        ...
		        if (log.isDebugEnabled())
                        ...
                        if (m_xmlrpc)
                        {
                                try
                                {
                                        ...
                                        String reply = notifier.notifyFailure(txNo, event.getUei(), "Invalid parameters");
		                        if (log.isDebugEnabled())
			                        log.debug("changeServiceHandler: Notify failure with xmlrpc:\n\t\t" + reply); 
                                }
                                catch (XmlRpcException xe)
                                {
                                        log.warn("changeServiceHandler: XMLRPC failure. ", xe);
                                }
                                catch (IOException ioe)
                                {
                                        log.warn("changeServiceHandler: IOException on XMLRPC. ", ioe);
                                }
                        }
					...
					log.debug("changeService: service " + serviceName  + " on IPAddress " 
				        ...
				        if (log.isDebugEnabled())
				        {
			                ...
			                stmt.executeUpdate();
                                        ...
                                        createAndSendServiceDeletedEvent(nodeEntry, InetAddress.getByName(ipaddr), serviceName, txNo);
                                        //Create a restartPollingInterface event to eventd. 
                                        createAndSendRestartPollingInterfaceEvent(nodeId, ipaddr, event.getHost(), txNo);
                                }
				        ...
				        if (log.isDebugEnabled())
				        {
					        log.debug("changeServiceHandler: add service " + serviceName  
                                        ...
                                        DbNodeEntry nodeEntry = DbNodeEntry.get(nodeId);
                                        createAndSendNodeGainedServiceEvent(nodeEntry,InetAddress.getByName(ipaddr),serviceName,txNo);
                                        if (m_xmlrpc)
                                        {
                                                try
                                                {
         ...
         *
         */
        private void createAndSendNodeGainedServiceEvent(DbNodeEntry nodeEntry, InetAddress ifaddr, String service, long txNo)
        {
		Category log = ThreadCategory.getInstance(getClass());
		if (log.isDebugEnabled())
                        ...
                        if (m_xmlrpc)
                        {
                                try
                                {
                                        ...
                                        String reply = notifier.notifyFailure(txNo, newEvent.getUei(), 
                                                "caught unexpected throwable exception.");
		                        if (log.isDebugEnabled())
			                        log.debug("createAndSendNodeGainedServiceEvent: Notify failure with xmlrpc: \n\t\t" + reply); 
                                }
                                catch (XmlRpcException xe)
                                {
                                        log.warn("createAndSendNodeGainedServiceEvent: XMLRPC failure. ", xe);
                                }
                                catch (IOException ioe)
                                {
                                        log.warn("createAndSendNodeGainedServiceEvent: IOException on XMLRPC. ", ioe);
                                }
                        }
                }
        }                
         ...
         * @param txNo          the external transaction No of the original event.
         */
        private void createAndSendRestartPollingInterfaceEvent(int nodeId, String ipaddr, String hostName, long txNo)
        {
		Category log = ThreadCategory.getInstance(getClass());
		if (log.isDebugEnabled())
                        ...
                        if (m_xmlrpc)
                        {
                                try
                                {
                                        ...
                                        String reply = notifier.notifyFailure(txNo, newEvent.getUei(), 
                                                "caught unexpected throwable exception.");
		                        if (log.isDebugEnabled())
			                        log.debug("createAndSendRestartPollingInterfaceEvent: Notify failure with xmlrpc:\n\t\t" 
                                                        + reply); 
                                }
                                catch (XmlRpcException xe)
                                {
                                        log.warn("createAndSendRestartPollingInterfaceEvent: XMLRPC failure. ", xe);
                                }
                                catch (IOException ioe)
                                {
                                        log.warn("createAndSendRestartPollingInterfaceEvent: IOException on XMLRPC. ", ioe);
                                }
                        }
                }
        }                
	...
	private void updateServerHandler(Event event)
	{
		...
		if (parms != null)
		{
			...
			while(parmEnum.hasMoreElements())
			{
                ...
                if (m_xmlrpc)
                {
		                ...
		                if (log.isDebugEnabled())
			                log.debug("updateServerHandler: Notify received event with xmlrpc:\n\t\t" + reply); 
                        }
                        catch (XmlRpcException xe)
                        {
                                log.warn("updateServerHandler: XMLRPC failure. ", xe);
                        }
                        catch (IOException ioe)
                        {
                                log.warn("updateServerHandler: IOException on XMLRPC. ", ioe);
                        }
                }
		        ...
		        if (log.isDebugEnabled())
                        ...
                        if (m_xmlrpc)
                        {
                                try
                                {
                                        ...
                                        String reply = notifier.notifyFailure(txNo, event.getUei(), "Invalid parameters");
		                        if (log.isDebugEnabled())
			                        log.debug("updateServerHandler: Notify failure with xmlrpc:\n\t\t" + reply); 
                                }
                                catch (XmlRpcException xe)
                                {
                                        log.warn("updateServerHandler: XMLRPC failure. ", xe);
				...
				if (log.isDebugEnabled())
				{
					log.debug("updateServer: the Interface "  + ipaddr + 
				        ...
				        if (log.isDebugEnabled())
				        {
			                ...
			                stmt.executeUpdate();
                                        ...
                                        createAndSendDeleteInterfaceEvent(nodeLabel, ipaddr, hostName, txNo);
                                }
			        else // could not perform 'ADD' since the service already exists.
                                {
			        ...
			        if (log.isDebugEnabled())
			        {
				        log.debug("updateServerHandler: added interface " + ipaddr  
                                ...
                                createAndSendAddInterfaceEvent(nodeLabel, ipaddr, hostName, txNo);
         ...
         */
        private void createAndSendAddInterfaceEvent(String nodeLabel, String ipaddr, String hostName, long txNo)
        {
		Category log = ThreadCategory.getInstance(getClass());
		if (log.isDebugEnabled())
                        ...
                        if (m_xmlrpc)
                        {
                                try
                                {
                                        ...
                                        String reply = notifier.notifyFailure(txNo, newEvent.getUei(), 
                                                "caught unexpected throwable exception.");
		                        if (log.isDebugEnabled())
			                        log.debug("createAndSendAddInterfaceEvent: Notify failure with xmlrpc:\n\t\t" + reply); 
                                }
                                catch (XmlRpcException xe)
                                {
                                        log.warn("createAndSendAddInterfaceEvent: XMLRPC failure. ", xe);
                                }
                                catch (IOException ioe)
                                {
                                        log.warn("createAndSendAddInterfaceEvent: IOException on XMLRPC. ", ioe);
                                }
                        }
                }
        }                
	...
	private void updateServiceHandler(Event event)
	{
         ...
         * @param txNo          the external transaction No.
         */
        private void createAndSendChangeServiceEvent(String ipaddr, String service, String action, long txNo)
        {
		Category log = ThreadCategory.getInstance(getClass());
		if (log.isDebugEnabled())
                        ...
                        if (m_xmlrpc)
                        {
                                try
                                {
                                        ...
                                        String reply = notifier.notifyFailure(txNo, newEvent.getUei(), 
                                                "caught unexpected throwable exception.");
		                        if (log.isDebugEnabled())
			                        log.debug("createAndSendChangeServiceEvent: Notify failure with xmlrpc:\n\t\t" + reply); 
                                }
                                catch (XmlRpcException xe)
                                {
                                        log.warn("createAndSendChangeServiceEvent: XMLRPC failure. ", xe);
                                }
                                catch (IOException ioe)
                                {
                                        log.warn("createAndSendChangeServiceEvent: IOException on XMLRPC. ", ioe);
                                }
                        }
                }
        }                
