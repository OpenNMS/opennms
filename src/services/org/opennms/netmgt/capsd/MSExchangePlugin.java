	...
	extends AbstractPlugin
{
	...
	/**
	 * <P>The default port on which to check for IMAP service.</P>
	 */
	private static final int	DEFAULT_IMAP_PORT	= 143;
	
	/**
	 * <P>The default port on which to check for MAPI service. To check for
	 * MAPI we check the http-rpc-epmap/ncacn_http service on port 593. 
	 * This port/service is used by exchange for doing RPC over HTTP.</P>
	 */
	 ...
	 *
	 * @return The array of supported protocols by the exchange server
	 *	The values are in the order POP3, IMAP, MAPI in the returned
	 * 	array.
	 */
	private boolean[] isServer(InetAddress host, int[] ports, int retries, int timeout)
	{
		...
		boolean isExMapi = false;  // NOTE:  MAPI protocol check currently disabled...see NOTE below...
	...
	{
		boolean[] result =  isServer(address,
					     new int[] { DEFAULT_POP3_PORT, DEFAULT_IMAP_PORT, DEFAULT_MAPI_PORT },
					     DEFAULT_RETRY,
					     DEFAULT_TIMEOUT);
