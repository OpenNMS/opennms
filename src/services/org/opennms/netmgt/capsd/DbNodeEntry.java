...
/**
 * <p>This class is used to model a row of the <em>node</em> table from
 * the OpenNMS database. The node table is indexed by the elements
 * <em>dpNode</em> and <em>nodeID</em>. When a new element is created
 * using the <code>create</code> call a node id will be automatically
 * defined. If the name of the distribute poller is not passed to the
 * create method, the it also is assigned a default value.</p>
 *
...
final class DbNodeEntry
{
	...
	/**
	 * The default distributed poller name to use if 
	 * one is not supplied
	 */
	...
	/**
	 * The SQL statement used to read the list of IP Addresses
	...
	/**
	 * The SQL statement used to read the list of SNMP interface
	 * entries for this particular node.
	 */
	 ...
	 * node entry with no data. The default distributed
	 * poller name is used internally.
	 *
	 */
	private DbNodeEntry()
	{
	...
	/**
	 * Constructs a new node entry with no data,
	 * except that the distributed poller name is
	 * set to the passed string.
	 *
	 * @param poller	The poller name.
	 *
	 */
	...
	/**
	 * Constructs a new entry with the specific
	 * node identifier. Once set the node identifier
	 * is non-mutable. If this constructor is used
	 * the record must already exists in the database.
	 *
	 ...
	 *
	 * @param nid		The node identifier.
	 * @param dpName	The name of the distributed poller.
	 *
	 */
	 ...
	 *
	 * @param descr	The new system description.
	 */
	 ...
	 *
	 * @param contact	The new system contact
	 */
	 ...
	 *
	 * @param src	The new label source.
	 */
	 ...
	 *
	 * @param name	The new NetBIOS name.
	 */
	...
	/**
	 * Updates the node information in the configured database. If the 
	 * node does not exist the a new row in the table is created. If the
	...
	DbIpInterfaceEntry[] getInterfaces(Connection db)
		throws SQLException
	{
		...
		while(rset.next())
		{
			...
			{
				ThreadCategory.getInstance(getClass()).warn("Failed to convert address " + saddr
									  + " to InetAddress for node " + m_nodeId
									  + ", ifIndex " + ifIndex, e);
				continue;
			}
	...
	DbIpInterfaceEntry[] getManagedInterfaces(Connection db)
		throws SQLException
	{
	...
	DbSnmpInterfaceEntry[] getSnmpInterfaces(Connection db)
		throws SQLException
	{
	 ...
	 * to <code>store</code>.
	 *
	 * @param poller	The name of the distributed poller to use.
	 *
	 * @return A new node record.
	 */
	 ...
	 * key fields of <em>nodeID</em> and <em>dpName</em>. If the
	 ...
	 *
	 */
	static DbNodeEntry get(int nid, String dpName)
		throws SQLException
	{
		Connection db = null;
		try
	 ...
	 *
	 */
	static DbNodeEntry get(Connection db, int nid)
		throws SQLException
	{
	 ...
	 * @param db	The databse connection used to load the entry.
	 * @param nid	The node id key
	 * @param dpName The distribute poller name key
	 *
	 * @return The loaded entry or null if one could not be found.
	 *
	 */
	static DbNodeEntry get(Connection db, int nid, String dpName)
		throws SQLException
	{
