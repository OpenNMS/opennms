	...
	implements Runnable
{
	...
	/**
	 * The list of all the NetBIOS addresses for
	 * the target IP.
	 */
	...
	/**
	 * The operating system name of the remote box, if it
	 * can be recovered.
	 */
	private String			m_osName;

	/**
	 * True if the box has MS Exchange running. This is
	 * set in the constructor.
	 */
	private final boolean		m_hasExchange;

	/**
	 * This method is used to convert a 6 byte MAC
	 * address into a colon separated string. 
	 *
	...
	/**
	 * The default constructor. This constructor is 
	 * marked as private to disallows creation without
	 * an IP address. In addtion this constructo will
	...
	/**
	 * Constructs a new SMB collector targeted
	 * at the passed address. The presence of an
	 * Exchange server is set to false.
	 *
	 ...
	 * Constructs a new SMB collector targeted
	 * at the passed address. The presence of an
	 * Exchange server is set to passed value.
	 *
	 * @param target	The target IP address.
	...
	/**
	 * Returns the primary NetBIOS address for
	 * the target if it was recovered.
	 */
	...
	/** 
	 * Retrns the NetBIOS name associated with the primary
	 * NetBIOS address.
	 */
	String getNbtName()
	{
	...
	/**
	 * Returns the list of all NetBIOS names recovered
	...
	/**
	 * Returns the list of all available shares on the 
	 * target if the call succeeded. If the call failed
	 * then a null or empty list is return.
	 */
	String[] getShares()
	{
		return m_shares;
	...
	/**
	 * Returns the naem of the Operating System if it
	 * was recovered.
	 */
	String getOS()
	{
	...
	/**
	 * Returns the presence of an exchange server. This
	 * is the same value as set in the class' constructor.
	 */
	boolean hasExchange()
	{
	...
	/**
	 * This method inspects the provided netBIOS name
	 * for control characters (chars w/ decimal value
	 * less than 20/<SPACE>
	 * 
	 * @param nbName  NetBIOS name to check
	 *
	 * @return true if string contains control chars,
	 *  	   false otherwise.
	 */
	...
	/**
	 * The main execution method used to collect the SMB information
	 * for the collector. 
	 *
	 */
	public void run()
	{
		...
		{
			if (log.isDebugEnabled())
				log.debug("IfSmbCollector: UnknownHostException: failed to retrieve SMB name, reason:" + 
					e.getMessage());
			m_addr = null;
		}
		...
		{
			log.warn("IfSmbCollector: Retrieved SMB name for address " + 
				m_target.getHostAddress() + " contains control chars: '" + 
				m_addr.getHostName() + "', discarding.");
			m_addr = null;
		}
		...
		{
			if (log.isDebugEnabled())
				log.debug("IfSmbCollector: SMB name of " + m_target.getHostAddress() + 
					" is: " + m_addr.getHostName());
			try
			{
			...
			{
				if (log.isDebugEnabled())
					log.debug("IfSmbCollector: failed to get MAC for " + 
							m_target.getHostAddress() + 
							" due to address failure", e);
			}
			...
			/* where a thread blocked indefinitely waiting for the jCIFS code to 
			...
			try
			{
				...
				// Get the OS name now
				//
				// Note: getOSVersion() method is not part of the standard jCIFS
				// code.  It was added specifically so we could gain access to
