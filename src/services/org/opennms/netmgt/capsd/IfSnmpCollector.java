 ...
 * is initially constructed no information is collected. The SNMP Session
...
{
	/**
	 * The SnmpPeer object used to communicate via SNMP with the remote
	 * host.
	 */
	...
	/**
	 * The default constructor. Since this class requires
	 * an IP address to collect SNMP information the 
	 * default constructor is declared private and will
	...
	/**
	 * Constructs a new snmp collector for a node using the passed 
	 * interface as the collection point. The collection does not
	 * occur until the <code>run</code> method is invoked.
	 *
	 * @param peer		The SnmpPeer object to collect from.
	 *
	 */
	...
	/**
	 * Returns the target address that the collection
	 * occured for.
	 */
	...
	/**
	 * Returns the netmask address at the corresponding index. If
	 * the address cannot be resolved then a null reference is returned.
	 * 
	 * NOTE:  If an interface has more than one IP address associated
	 *        with it only the FIRST match is returned.
	 *
