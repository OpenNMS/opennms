	...
	implements Pollable
{
	...
	/**
	 * Add a PollableService object to the services map keyed
	 * by service name.
	 * 
	 * WARNING:  For node outage processing we are only interested
	 *           in testing a particular service once regardless
	 *           of how many times the service is actually 
	 *           scheduled for polling (due to inclusion by 
	 *           multiple packages), therefore note that only 
	 *           the LAST pollable service added to the interface
	 *           is actually saved in the services map. Any
	 *           existing entry is simply replaced.
	 */
	...
	public synchronized int poll(PollableService pSvc)
	{
		...
		if (m_status == Pollable.STATUS_DOWN)
		{
			...
			if (criticalSvcName != null && this.supportsService(criticalSvcName))
			{
				...
				else
				{
					...
					/* -------------------------------------------- 
					 * Commenting this logic out for now, need to implement
					 * code to create PollableServiceProxy objects and this
					 * doesn't fit in well with that...will revisit this
					 * afterwards.
					 * --------------------------------------------
					...
					criticalSvc = this.getService(criticalSvcName);
					if (criticalSvc != null)
					{
						long timeSinceLastCriticalPoll = System.currentTimeMillis() - criticalSvc.getLastPollTime();
						if (log.isDebugEnabled())
							log.debug("poll: timeSinceLastCriticalPoll=" + timeSinceLastCriticalPoll + 
								" interval for " + pSvc.getInterface().getAddress().getHostAddress() + "/" + 
								pSvc.getServiceName() + " is " + 
								pSvc.getLastScheduleInterval());
						if ( timeSinceLastCriticalPoll >= pSvc.getLastScheduleInterval())
						{
							// Re-poll critical svc
							if (log.isDebugEnabled())
								log.debug("poll: re-polling critical service...");
							svcStatus = criticalSvc.poll();
						}
					}
					------------------------------------------------*/
				}
				...
				if (svcStatus == Pollable.STATUS_UP && criticalSvc.statusChanged())
				{
					...
					while (iter.hasNext())
					{
						...
						// to reschedule the service to be polled at the appropriate
						// interval until the scheduler schedules the PollableService
						...
						if (tmpStatus == Pollable.STATUS_UP)
						{
							...
							m_scheduler.schedule(new PollableServiceProxy(svc, runAt),
										svc.recalculateInterval());
							log.debug("poll: scheduling new PollableServiceProxy for " + 
								m_address.getHostAddress() + "/" + svc.getServiceName() +
								" at interval= " + svc.recalculateInterval());
						}
					}
							...
							if (addr.equals(m_address))
							{
								if (!m_services.containsValue(tmp))
								{
									...
									if (tmpStatus == Pollable.STATUS_UP)
									{
										...
										m_scheduler.schedule(new PollableServiceProxy(tmp, runAt),
													tmp.recalculateInterval());
										log.debug("poll: scheduling new PollableServiceProxy for " + 
											m_address.getHostAddress() + "/" + tmp.getServiceName() +
											" at interval= " + tmp.recalculateInterval());
									}
								}
							}
						}
		...
		else if (m_status == Pollable.STATUS_UP)
		{
			...
			if (svcStatus == Pollable.STATUS_DOWN && pSvc.statusChanged())
			{
				...
				// else if Critical service defined and supported by interface (regardless of pkg)
				else if (criticalSvcName != null && this.supportsService(criticalSvcName))
				{
					...
					// If the service we just polled WAS in fact the critical service
