                ...
                if (invalidParameters)
                {
                        ...
                        if (m_xmlrpc)
                        {
                                try
                                {
                                        ...
                                        if (log.isDebugEnabled())
                                                log.debug("RestartPollingInterfaceHandler: notify failure with xmlrpc:\n\t\t" + reply);
                                }
                                catch (XmlRpcException xe)
                                {
                                        log.warn("restartPollingInterfaceHandler: XMLRPC failure. ", xe);
                                }
                                catch (IOException ioe)
                                {
                                        log.warn("restartPollingInterfaceHandler: IOException on XMLRPC. ", ioe);
                                }
                        }
                }
				...
				nodeid  = rs.getInt(1);
                        ...
                        if (nodeid < 0)
                        {
                                ...
                                if (m_xmlrpc)
                                {
                                        ...
                                        {
                                                XmlRpcNotifier notifier = new XmlRpcNotifier(m_xmlrpcServerUrl);
                                                String reply = notifier.notifyFailure(txNo, event.getUei(), "Node does not exist.");
                                                if (log.isDebugEnabled())
                                                        log.debug("RestartPollingInterfaceHandler: notify failure with xmlrpc:\n\t\t" 
                                                                + reply);
                                        }
                        			...
                        			log.error("Failed to schedule interface " + ipaddr + 
                        					" for service monitor " + serviceName + ", illegal address", ex);
                                                if (m_xmlrpc)
                                                {
                                                        ...
                                                        {
                                                                XmlRpcNotifier notifier = new XmlRpcNotifier(m_xmlrpcServerUrl);
                                                                String reply = notifier.notifyFailure(txNo, event.getUei(), 
                                                                        ex.getMessage());
                                                                if (log.isDebugEnabled())
                                                                        log.debug("RestartPollingInterfaceHandler:" 
                                                                                + "notify failure with xmlrpc:\n\t\t" + reply);
                                                        }
                                                        catch (XmlRpcException xe)
                                                        {
                                                                log.warn("restartPollingInterfaceHandler: XMLRPC failure. ", xe);
                        					...
                        					log.error("Failed to release node lock on nodeid " + 
                        							pNode.getNodeId() + ", thread interrupted.");
                                                                if (m_xmlrpc)
                                                                {
                                                                        ...
                                                                        {
                                                                                XmlRpcNotifier notifier = new XmlRpcNotifier(m_xmlrpcServerUrl);
                                                                                String reply = notifier.notifyFailure(txNo, event.getUei(), "Internal error.");
                                                                                if (log.isDebugEnabled())
                                                                                        log.debug("RestartPollingInterfaceHandler:" 
                                                                                        + "notify failure with xmlrpc:\n\t\t" + reply);
                                                                        }
			...
			log.error("Nodeid " + nodeId + " does not exist in pollable node map, unable to delete node.");
                        if (m_xmlrpc)
                        {
                                try
                                {
                                        XmlRpcNotifier notifier = new XmlRpcNotifier(m_xmlrpcServerUrl);
                                        String reply = notifier.notifyFailure(txNo, event.getUei(), 
                                                "Node does not exist in pollable node map.");
                                        if (log.isDebugEnabled())
                                                log.debug("nodeDeletedHandler:  notify failure with xmlrpc:\n\t\t" + reply);
                                }
                                catch (XmlRpcException xe)
                                {
                                        log.warn("nodeDeletedHandler: XMLRPC failure. ", xe);
                                }
                                catch (IOException ioe)
                                {
                                        log.warn("nodeDeletedHandlerEvent: IOException on XMLRPC. ", ioe);
                                }
                        }
			return;
		}
			...
			else
			{
				...
				log.error("nodeDeletedHandler: failed to obtain lock on nodeId " + nodeId);
                                if (m_xmlrpc)
                                {
                                        ...
                                        {
                                                log.warn("nodeDeletedHandler: IOException on XMLRPC. ", ioe);
                                        }
                                }
			}
		}
		catch (InterruptedException iE)
		{
			...
			log.error("nodeDeletedHandler: thread interrupted...failed to obtain lock on nodeId " + nodeId);
                        if (m_xmlrpc)
                        {
                                ...
                                {
                                        XmlRpcNotifier notifier = new XmlRpcNotifier(m_xmlrpcServerUrl);
                                        String reply = notifier.notifyFailure(txNo, event.getUei(), iE.getMessage());
                                        if (log.isDebugEnabled())
                                                log.debug("nodeDeletedHandler:  notify failure with xmlrpc:\n\t\t" + reply);
                                }
                                catch (XmlRpcException xe)
		...
		catch (Throwable t)
		{
			log.error("exception caught processing nodeDeleted event for " + nodeId, t);
                        if (m_xmlrpc)
                        {
                                ...
                                {
                                        XmlRpcNotifier notifier = new XmlRpcNotifier(m_xmlrpcServerUrl);
                                        String reply = notifier.notifyFailure(txNo, event.getUei(), "Caught unknown exception.");
                                        if (log.isDebugEnabled())
                                                log.debug("nodeDeletedHandler:  notify failure with xmlrpc:\n\t\t" + reply);
                                }
                                catch (XmlRpcException xe)
			...
			log.error("Nodeid " + nodeId + " does not exist in pollable node map, unable to delete interface " 
                        ...
                        if (m_xmlrpc)
                        {
                                try
                                {
                                        ...
                                        String reply = notifier.notifyFailure(txNo, event.getUei(), "Node does not exist.");
                                        if (log.isDebugEnabled())
                                                log.debug("interfaceDeletedHandler:  notify failure with xmlrpc:\n\t\t" + reply);
                                }
                                catch (XmlRpcException xe)
                                {
                                        log.warn("interfaceDeletedHandler: XMLRPC failure. ", xe);
                                }
                                catch (IOException ioe)
                                {
                                        log.warn("interfaceDeletedHandler: IOException on XMLRPC. ", ioe);
                                }
                        }
			return;
		}
								...
								" not in interface map for " + nodeId);
                                        if (m_xmlrpc)
                                        {
                                                try
                                                {
                                                        XmlRpcNotifier notifier = new XmlRpcNotifier(m_xmlrpcServerUrl);
                                                        String reply = notifier.notifyFailure(txNo, event.getUei(), 
                                                                "Interface not in interface map.");
                                                        if (log.isDebugEnabled())
                                                                log.debug("interfaceDeletedHandler: notify failure with xmlrpc:\n\t\t" 
                                                                        + reply);
                                                }
