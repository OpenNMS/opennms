...
/**
 * <P>
 * A proxy to a PollableService which acts as a surrogate for rescheduling
 * purposes. This was created as a temporary fix to allow us to schedule a
 * PollableService for a sooner execution, despite the fact that it is already
 * scheduled for a later time. At the time we created this class, the Scheduler
 * did not provide any means to cancel or reschedule an element on its queue.
 * Therefore, this workaround became necessary.
 * </P>
 * 
 * <P>
 * In order to use this class, create an instance of it with the desired
 * PollableService and the timestamp at which you want it to run. Once you have
 * the instance, you can schedule it with the Scheduler, since it is of type
 * ReadyRunnable.
 * </P>
 * 
 * <P>
 * Each time it runs, it will reschedule itself at the interval currently valid
 * for the PollableService it contains. Once it catches up to the originally
 * scheduled time for its PollableService, it will politely quit rescheduling
 * itself. Assuming that the scheduler was the only object that was keeping a
 * reference to this proxy, it should be noticed by the garbage collector next
 * time runs.
 * </P>
 * 
...
final class ScheduleProxy implements ReadyRunnable {
     ...
     * specified pollable service.
     * 
     * @param psvc
     *            The PollableService to be proxied.
     * @param runAt
    ...
    /**
     * Returns the time (in milliseconds) at which this Proxy is scheduled to
     * run.
     */
