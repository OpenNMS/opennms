...
// Modifications:
//
// 2003 Oct 08: Changed thread pool parameters.
 ...
 * the polling occurs at the expected intervals. The scheduler
	...
	implements Runnable, PausableFiber
{
	 ...
	 */
	private static final String	FIBER_NAME 		= "Poller Scheduler";

	/**
	 * The map of queue that contain {@link ReadyRunnable ready runnable} instances.
	 * The queues are mapped according to the interval of scheduling.
	 */
	private Map	m_queues;

	/**
	 * The total number of elements currently scheduled.
	 * This should be the sum of all the elements in the
	 * various queues.
	 */
	 ...
	 * runnable instances scheduled by the class' instance.
	 */
	 ...
	 *
	 */
	private static final class PeekableFifoQueue
		extends FifoQueueImpl
	{
		/**
		 * The object hold. This holds the last object
		 * peeked at by the application.
		 */
	...
	/**
	 * Constructs a new instance of the scheduler. The maximum
	 * number of executable threads is specified in the constructor.
	 * The executable threads are part of a runnable thread pool
	 * where the scheduled runnables are executed.
	 *
	 ...
	 *
	 * @param maxSize	The maximum size of the thread pool.
	 * @param lowMark	The low water mark ratios of thread size to
	 *	threads when threads are stopped.
	 * @param hiMark	The high water mark ratio of thread size to
	 *	threads when threads are started.
	 *
	 */
	Scheduler(int maxSize, float lowMark, float hiMark)
	{
		m_name = FIBER_NAME + maxSize;
		m_status = START_PENDING;
		m_runner = new RunnableConsumerThreadPool(FIBER_NAME + " Pool", lowMark, hiMark, maxSize);
		m_queues = Collections.synchronizedMap(new TreeMap());
		m_scheduled = 0;
		m_worker = null;
	}
	...
	/**
	 * This method is used to schedule a ready runnable in the
	 * system. The interval is used as the key for determining
	 * which queue to add the runnable.
	 *
	 * @param runnable	The element to run when interval expires.
	 * @param interval	The interval queue (measured in milliseconds)
                        to add the runnable to.
	 *
	 * @throws java.lang.RuntimeException Thrown if an error occurs
	 * 	adding the element to the queue.
	 */
	synchronized void schedule(ReadyRunnable runnable, long interval)
	{
		...
		if(log.isDebugEnabled())
		{
			if (runnable instanceof PollableService)
			{
				...
				log.debug("schedule: adding ready runnable for " +
						svc.getInterface().getAddress().getHostAddress() + "/" +
						svc.getServiceName() + "/" +
						svc.getPackageName() + " at interval " + interval);
			}
			else
			{
							...
							if(readyRun != null && readyRun.isReady())
							{
								if(log.isDebugEnabled())
								{
									if (readyRun instanceof PollableService)
									{
										...
										log.debug("run found ready runnable for " +
												svc.getInterface().getAddress().getHostAddress() + "/" +
												svc.getServiceName()+ "/" +
												svc.getPackageName());
									}
								...
								// Pop the interface/readyRunnable from the queue for execution.
								//
								in.remove();
