...
public class NsclientMonitor extends IPv4LatencyMonitor {
	...
	public int poll(NetworkInterface iface, Map parameters,
		...
		String command = ParameterMap
				.getKeyedString(
						parameters,
						"command",
						NsclientManager
								.convertTypeToString(NsclientManager.CHECK_CLIENTVERSION));
		...
		String rrdPath = ParameterMap.getKeyedString(parameters,
				"rrd-repository", null);
		String dsName = ParameterMap
				.getKeyedString(parameters, "ds-name", null);

		// Validate the graph-related values.
		if (rrdPath == null) {
			log
					.info("poll: RRD repository not specified in parameters, latency data will not be stored.");
		}
		if (dsName == null) {
			dsName = DS_NAME;
		}
		...
		for (int attempts = 0; attempts <= retry
			...
			try {
				...
				NsclientManager client = new NsclientManager(ipv4Addr
						.getHostAddress(), port);
				...
				NsclientCheckParams clientParams = new NsclientCheckParams(
						critPerc, warnPerc, params);

				// Send the request to the server and receive the response.
				response = client.processCheckCommand(NsclientManager
						.convertStringToType(command), clientParams);
				...
				if (response.getResultCode() == NsclientPacket.RES_STATE_OK) {
					...
					if ((m = decimal.matcher(reason)).matches()) {
						...
						log.debug(reason + " Split " + m.group(1) + " Second "
								+ m.group(2) + "  ... has a decimal point.");
					...
					if (responseTime >= 0 && rrdPath != null) {
						...
						try {
							this
									.updateRRD(rrdPath, ipv4Addr, dsName, val1,
											pkg);
