...
/**
 * Defines an abstract strategy for manipulating round robin database file.  This is
 * used by the RrdUtils to implement the appropriate behaviour
 */
interface RrdStrategy {
     ...
     * 
     * @param creator - A string representing who is creating this file for use in log msgs
     ...
     * @param rraList - a List of the round robin archives to create in the database
     * @return an object representing the definition of an round robin database
     * @throws Exception If an error occurs while creating the definition
     */
    Object createDefinition(String creator, String directory, String dsName, int step, String dsType, int dsHeartbeat, String dsMin,
     ...
     * 
     * @param rrdDef
     *     an round robin database definition created using the createDefinition call.
     * @throws Exception if an error occurs create the file
     */
    void createFile(Object rrdDef) throws Exception;
    
    /**
     * Opens the round robin database with the supplied name.  It is assumed the name
     * refers to a round robin database appropriate for this strategy implementation 
     * 
     * @param fileName
     *       the name of the associated rrd file
     * @return an open rrd reference that can by used in calls to updateFile
     *            and closeFile 
     * @throws Exception if an error occurs opening the file
     */
     ...
     * @throws Exception if an error occurs updating the file
     */
     ...
     * @param rrd
     *    an rrd object created using openFile
     * @throws Exception if an error occurs closing the file
     */
    void closeFile(Object rrd) throws Exception;
    
    /**
     * Fetches the last value from the round robin database with the given name.  The
     * interval passed in should be the interval associated with the round robin database.
     * 
     * @param rrdFile
     *     a name the represents a round robin database
     * @param interval
     *     a step interval of the round robin database
     * @return The last value as a Double (if the last value didn't exist returns a Double.NaN)
     ...
     * data.  It accepts an rrdtool graph command.  The underlying implementation
     * converts this command to a format appropriate for it
     * .
     * @param command
     *      the command needed to create the graph
     * @param workDir
     *      the directory that all referenced files are relative to
     * @return an input stream representing the bytes of a graph image as a PNG file
     * @throws IOException if an IOError occurs
     * @throws RrdException if an RRD error occurs
     */
    public InputStream createGraph(String command, File workDir) throws IOException, RrdException;

    /**
     * Provides the round robin database an opportunity to contribute statistics information
     * to the logs file.
     * @return a non-null string representing any staticstics to be included in the logs
     */
