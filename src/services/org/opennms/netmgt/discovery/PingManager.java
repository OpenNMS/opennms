	...
	implements Runnable, PausableFiber
{
	...
	{
		/**
		 * The semaphore that each poller must
		 * acquire before they can send a packet
		 */
		...
		/**
		 * The value stored in the icmp 
		 * identifier field.
		 */
		private final short	m_icmpId;

		/**
		 * The identifier for the fiber. This
		 * is stored in the ping packet.
		 */
		...
		/**
		 * True if the <code>signal</code> method
		...
		/**
		 * This is the constant value that represents
		 * an idle pinger thread.
		 */
		final static int	IDLE	= 0;

		/**
		 * This is the constant value that represents
		 * a polling pinger thread.
		 */
		final static int	POLLING = 1;

		/**
		 * <p>This method does the actual polling of the encapsulated
		 * address using the timeout and retry values. The polling
		 * will continue until either all the retries are exhausted
		 * or the pinger is signaled that the address responded.</p>
		 *
		 * <p><strong>NOTICE: The instance lock must be heald prior
		 * to calling this method.</strong> This method uses the 
		 * {@link java.lang.Object#wait wait} method and lock ownership
		 * is required to invoke the <code>wait</code> method.</p>
		 *
		 * @param addr	The address and information for polling.
		 *
		 * @throws java.io.IOException Thrown if an error occurs sending
		 * 	the ICMP information.
		 * @throws java.lang.InterruptedException Thrown if the thread
		 * 	is interrupted.
		 */
		private /* synchronized */ boolean poll(IPPollAddress addr)
		 ...
		 *
		 * @param socket	The ICMP socket
		 * @param semaphore	The control semaphore
		 * @param addrQ		The input address queue.
		 * @param filterId	The ICMP id for filtering datagrams
		 * @param tid		The Thread ID stored in the Packet.
		 *
		 */
		Pinger(	IcmpSocket		socket,
		...
		/**
		 * Attempts to stop the current polling cycle as
		 * quickly as possbile. If the fiber has never
		...
		/**
		 * <p>Pauses the current fiber. This does not
		 * take effect immediently, but after the 
		 * current polling cycle has completed.</p>
		 *
		 * <p>To determine if the fiber has finished its
		 * current polling cycle check for a status equal
		 * to <code>PAUSED</code> and the extended status
		 * equal to <code>IDLE</code>.</p>
		 *
		 */
		...
		/**
		 * Returns the extends status of the thread. This
		 * will always be one of two values: POLLING or
		 * IDLE.
		 *
		...
		/**
		 * Returns true if this thread is currently polling the
		 * passed IP Address.
		 *
		 * @param addr	The address to check.
		 *
		 * @return True if the thread is currently targeting the
		 * 	passed address.
		 */
		...
		/**
		 * Signals the instance that it's current target 
		 * has responded to an echo check.
		 */
		...
		/**
		 * Returns the identifier that is placed in 
		 * the ping {@link org.opennms.netmgt.ping.Packet packet}.
		 *
	 ...
	 * constructed it must be started with a call to the method <code>start</code>.
	 *
	 * @param addressQ	The input address queue.
	 * @param discoveredQ	The output queue of responding addresses.
	 * @param threads	The number of pinging fibers to create.
	 * @param pktsPerSecond	The maximum packets per second that can be generated.
	 *
	 * @throws java.io.IOException Thrown if the ICMP socket cannot be constructed.
	 *
	 */
	PingManager(FifoQueue addressQ, FifoQueue discoveredQ, short filterId, int threads, int pktsPerSecond)
		throws IOException
	{
		if(threads < 1)
		...
		{
			m_pingers[x] = new Pinger(m_socket,
						  m_semaphore,
						  addressQ,
						  filterId,
						  TID_CONST_KEY | (long)x);
		}
	 ...
	 *
	 * @throws java.lang.IllegalStateException Thrown if the fiber has previously
	 * 	been started.
	 *
	 */
	public synchronized void start()
	...
	/**
	 * Stops the currently running ping manager. If the manager has
	 * never been run then an exception is generated.
	 *
	...
	public synchronized void stop()
	{
		...
		for(int x = 0; x < m_pingers.length; x++)
		{
			try { m_pingers[x].stop(); } catch(Exception e) { }
		}
		try { m_icmpReceiver.stop(); } catch(Exception e) { }
	...
	/**
	 * Pauses the currently running manager. If the manager is already
	 * paused or is stopping, then the call is silently ignored. If 
	 * the fiber is not currently running, either due to it being stopped
	 * or having never been started, then an exception is generated.
	 *
	...
	/** 
	 * Resumes a currently paused manager. If the manager is not 
	 * running, either by never being started or it has stopped then
	 * an exception is generated. If the manager is not currently 
	 * paused then the manager silently discards the request.
	 *
	 * @throws java.lang.IllegalStateException Thrown if the manager
