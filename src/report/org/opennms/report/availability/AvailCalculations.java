 ...
 * The types include
 * Last 30 days daily availability
 * Last 30 days total availability
 * Last 30 days daily service availability
 * Last Months Top 20 offenders
 * Last Months Top 20 Service outages
...
public class AvailCalculations extends Object
{
	 ...
	 * @param report Castor Report class.
	 * @param offenders Map of all offenders -- percent/(list of node) pairs
	 * @param format Value can be "SVG / all"
	 */
	public AvailCalculations(List nodes, 
				long endTime, 
				long lastMonthEndTime, 
				List monitoredServices, 
				Report report, 
				TreeMap offenders,
				double warning, 
				double normal,
				...
				int sectionIndex)
	{
		...
		// Please node the following 4 formats are displayed on the graphical report.
		// (i) last12MoAvail
					...
					if(intf != null)
					{
						...
						while(lstIterSvcs.hasNext())
						{
							...
							if(svc != null)
							{
								...
								if(outages != null)
								{
									IfService ifservice = new IfService(node.getNodeID(),
                                                    							    intf.getName(),
							                                                    -1,
							                                                    node.getName(),
							                                                    svc.getName());
	 ...
	 *
         * @param days Number of days for which the availability computations are made.
         * @param endTime End time
         * @param sections Castors sections
         * @param label Section name in the xml
         * @param descr Section descr.
				...
				while(daysCnt++ < days)
				{
					...
					while(iter.hasNext())
					{
						...
						if(outageList != null)
						{
							...
							// Keep track of the number of services being monitored.
							//
							outageTime += outage;
						}
						serviceCnt++;
