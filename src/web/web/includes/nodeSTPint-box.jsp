...
<%
    ...
    //gets active route entry on node
   	...
   	StpInterface[] stpifs = ExtendedNetworkElementFactory.getStpInterface(nodeId);
...
<% if(stpifs.length == 0) { %>
  <tr>
    <td BGCOLOR="#cccccc" colspan="7">There have been no STP Interfaces info on this node.</b></a></td>
  </tr>
<% } else { %>
              <tr>
			  ...
			  <tr>
                ...
                <td><%=stpifs[i].get_bridgeport()%>/<a href="element/interface.jsp?node=<%=nodeId%>&intf=<%=ExtendedNetworkElementFactory.getIpAddress(nodeId, stpifs[i].get_ifindex())%>&ifindex=<%=stpifs[i].get_ifindex()%>"><%=stpifs[i].get_ifindex()%></a></td>
