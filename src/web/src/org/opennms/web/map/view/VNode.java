...
final public class VNode extends VElement {
    ...
    /* (non-Javadoc)
     * @see org.opennms.web.map.temp.VElement#getStatus()
     */
    public String getStatus() throws MapsException {
        Node element = null;
        try {
            element = NetworkElementFactory.getNode(getId());
        } catch (SQLException e) {
            throw new MapsException("Error accessing the node status", e);
        }
        return String.valueOf(element.getNodeType());
    }

    /* (non-Javadoc)
     * @see org.opennms.web.map.temp.VElement#getSeverity()
     */
    public int getSeverity() throws MapsException {
        try {
            Connection connection = Vault.getDbConnection();
            PreparedStatement preparedStatement = null;
            ResultSet rs = null;
            try {
                preparedStatement = connection.prepareStatement("select eventseverity from events " +
                        "where nodeid = ? " +
                ...
                if (!rs.next()) return -1; //TODO quale valore restituire se la query non restituisce nulla?
                return rs.getInt(1);
            } finally {
                if (rs != null) rs.close();
            ...
            throw new MapsException("Error accessing the node severity", e);
        }        
    }
