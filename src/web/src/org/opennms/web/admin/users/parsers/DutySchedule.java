 ...
 * schedule information are possible, as is the comparision between
 * a Calendar passed in and the start and stop times of each day in
 * a duty schedule.
 * 
...
public class DutySchedule
{
	...
	*/
	public static final String[] DAY_NAMES = { "Mo",
		                                   "Tu",
						   "We",
						   "Th",
						   "Fr",
	...
	/**A mapping between the days of the week as indexed by the DutySchedule class
	   and those of the Calendar class
	*/
	private static final int[] CALENDAR_DAY_MAPPING = { Calendar.MONDAY,
		                                            Calendar.TUESDAY,
							    Calendar.WEDNESDAY,
							    Calendar.THURSDAY,
							    Calendar.FRIDAY,
	...
	/**Create a new DutySchedule.
           Builds the BitSet used to identify the days of the week that are set.
	*/
	public DutySchedule()
	{
		m_days = new BitSet(7);
	...
	/**Create a new DutySchedule. 
           This constructor is designed to convert from a Vector filled
	   with 7 Boolean objects and two String objects into the BitSet
	   ...
	   @param aSchedule Vector filled with 7 Boolean objects and two String objects
	*/
	...
	*/
	...
	/**Test if time is contined in schedule.
	   ...
	   contained in this object. It creates two partial Calendars from
	   the Calendar that is passed in and then sets the start time for
	   one and the end time for the other. Then in a loop it reassigns
	...
	*/
	public boolean isInSchedule(Calendar aTime)
	{
		...
		Calendar startTime = new GregorianCalendar(aTime.get(Calendar.YEAR),
		                                           aTime.get(Calendar.MONTH),
							   aTime.get(Calendar.DATE));
							   
		//the hour will be the integer part of the start time divided by 100 cause it should be
		//in military time
		startTime.set(Calendar.HOUR_OF_DAY, (m_startTime/100));
		
		//the minute will be the start time mod 100 cause it should be in military time
		startTime.set(Calendar.MINUTE, (m_startTime % 100));
		startTime.set(Calendar.SECOND, 0);
		
		Calendar endTime = new GregorianCalendar(aTime.get(Calendar.YEAR),
		                                         aTime.get(Calendar.MONTH),
							 aTime.get(Calendar.DATE));
		...
		//look at the BitSet to see what days are set for this duty schedule, reassign the
		//day of weak for the start and stop time, then see if the argument Calendar is
		//between these times.
		for (int i = 0; i < 7; i++)
		{
			...
			{
				//now check to see if the time given is between these two times inclusive, if it is quit loop
				//we want the begin and end times for the ranges to be includsive, so convert to milliseconds so we
				...
				//return true if the agrument date falls between the start and stop time
