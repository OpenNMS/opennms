...
public class OutageFactory extends Object
{
     ...
     * 
     * <p>Note: This method has been optimized for the simplest query.</p>
     */
    public static int getOutageCount() throws SQLException {
        ...
        try {            
            ...
            ResultSet rs = stmt.executeQuery("SELECT COUNT(OUTAGEID) AS OUTAGECOUNT FROM OUTAGES WHERE IFREGAINEDSERVICE IS NULL");
    ...
    /** 
     * Return all outages (optionally only unresolved outages) sorted by given sort style
     * for the given node.
     */
    ...
    /** 
     * Return all unresloved outaged sorted by time for the given service 
    ...
    /** 
     * Return all outages (optionally only unresolved outages) sorted by given sort style
     * for the given service identifier.
     */
    ...
    protected static Outage[] rs2Outages( ResultSet rs ) throws SQLException {
        ...
        while(rs.next()) {
            ...
            //can be null            
            outage.hostname = rs.getString("iphostname");  //from ipinterface table
            
            //can be null
            outage.nodeLabel = rs.getString("nodelabel");  //from node table            
            
            //can be null
            outage.serviceName = rs.getString("servicename"); //from service table
