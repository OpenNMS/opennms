...
public class AutoAction implements Cloneable
{
	/**A list of values for autoaction states. If this array changes
	   please update any of the AUTO_ACTION_STATE_DEFAULT_INDEX members if needed.
	*/
	public static final String AUTO_ACTION_STATES[] = {"on","off"};
	
	/**The index into the AUTO_ACTION_STATES array indicating
	...
	*/
	...
	{
		/*
		if (index < 0 || index > AUTO_ACTION_STATE_VALUES.length)
			throw new InvalidParameterException("The auto action state index("+index+") must be >= 0 and <= " + CORRELATION_STATE_VALUES.length);
		*/
		m_state = state;
	}
