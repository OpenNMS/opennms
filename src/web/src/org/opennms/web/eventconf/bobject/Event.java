...
public class Event implements Cloneable
{
	...
	*/
	...
	/**The different destination values for the log message. If this array changes
	   please update the LOGMSG_DEST_DEFAULT_INDEX member if needed.
	*/
	public static final String LOGMSG_DEST_VALUES[] = {"suppress",
		                                           "logonly",
							   "displayonly",
	                                            	   "logndisplay"};
	
	/**The index into the LOGMSG_DEST_VALUES array indicating
	   the default destination of the log message . If the values array
	   changes please update this index if needed.
	*/
	public static final int LOGMSG_DEST_DEFAULT_INDEX = 3;
	
	/**A generic list of values for various states. If this array changes
	   please update any of the XXX_STATE_DEFAULT_INDEX members if needed.
	*/
	public static final String CONFIGURATION_STATES[] = {"on","off"};
	
	/**The index into the CONFIGURATION_STATES array indicating
	   the default state auto acknowlege. If the values array
	   changes please update this index if needed.
	*/
	public static final int AUTOACKNOWLEDGE_STATE_DEFAULT_INDEX = 0;
	
	/**The index into the CONFIGURATION_STATES array indicating
	   the default state of an trouble ticket. If the values array
	   changes please update this index if needed.
	*/
	...
	*/
	...
	{
		/*
		if (index < 0 || index > LOGMSG_DEST_VALUES.length)
			throw new InvalidParameterException("The log message destination index("+index+") must be >= 0 and <= " + LOGMSG_DEST_VALUES.length);
		*/
	...
	*/
	...
	{
		/*
		if (index < 0 || index > CONFIGURATION_STATES.length)
			throw new InvalidParameterException("The auto acknowlege state index("+index+") must be >= 0 and <= " + CONFIGURATION_STATES.length);
		*/
	...
	*/
	public void setTticketState(String state)
	{
		/*
		if (index < 0 || index > CONFIGURATION_STATES.length)
			throw new InvalidParameterException("The trouble ticket state index("+index+") must be >= 0 and <= " + CONFIGURATION_STATES.length);
		*/
