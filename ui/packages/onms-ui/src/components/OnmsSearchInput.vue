...
// site stops re-declaring these rules).
//
...
// `--p-icon-size` (1rem) while these glyphs render at 1.5rem, so the stock
// reservation left the text all but touching the glyph.
//
// Everything below is nested for specificity, and the two paddings are
// `!important`, because this component is embedded in pages the seam does not
// control. PrimeVue ships its CSS inside `@layer primevue`, and *any* unlayered
// author rule beats a layer regardless of specificity — the legacy Bootstrap
// rule in opennms-styles.scss (`input, span, … { color: inherit }`) is why the
// colors are re-asserted, and on the Vaadin topology page the menubar mounts
// inside `.v-app`, where a `.v-app input[type="text"]`-shaped rule (0,2,1)
// outranks a plain scoped `.p-inputtext[data-v-x]` (0,2,0) and drops the
// horizontal padding, dropping the search glyph on top of the placeholder
// (NMS-20160). Nesting gets to (0,3,0); `!important` makes the field's geometry
// independent of whatever the host page ships. No call site sets padding on
// this input — the seam owns it.
.p-iconfield {
  ...
  // Re-assert the theme's own input colors (the same tokens PrimeVue's rules
  // use) alongside the padding, for the layer reason above.
  .p-inputtext {
    width: 100%;
    color: var(--p-inputtext-color);
    padding-inline-start: var(--onms-search-input-inset) !important;
    padding-inline-end: var(--onms-search-input-inset) !important;
  }
  ...
  // Mirrors PrimeVue's own .p-inputicon placement (which we can't reuse:
  // InputIcon hard-codes aria-hidden on its wrapper, and a focusable button must
  // not sit inside an aria-hidden subtree).
