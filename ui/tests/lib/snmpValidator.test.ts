...
describe('snmpValidator', () => {
  describe('validateDefinition', () => {
    ...
    describe('IP Address validation', () => {
      it('should return error when first IP address is missing', () => {
        const errors = validateDefinition(validConfig, 'v2c', '', '')
        expect(errors.firstIpAddress).toBe('First IP Address is required')
      })

      it('should return error when first IP address is invalid', () => {
      ...
      it('should pass validation when last IP address is empty', () => {
        const errors = validateDefinition(validConfig, 'v2c', '192.168.1.1', '')
        expect(errors.lastIpAddress).toBeUndefined()
      })

      it('should pass validation with valid last IP address', () => {
