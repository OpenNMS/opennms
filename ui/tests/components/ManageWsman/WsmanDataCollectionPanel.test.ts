...
describe('WsmanDataCollectionPanel.vue', () => {
  it('shows every table with its source column', () => {
  ...
  it('marks the RRAs column as RRD-only when another backend stores the samples', () => {
    ...
    expect(rrd.find('[data-test="collections-table"]').text()).not.toContain('RRD only')
    ...
    expect(mountPanel().find('[data-test="collections-table"]').text()).not.toContain('RRD only')
  })
})
