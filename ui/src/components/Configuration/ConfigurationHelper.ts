...
const validateHost = (host: string) => {
  ...
  const customHostnameRegex = /^(?:(\$\{[^}]+\}|\w+):(\$\{[^}]+\}|\S+)@)?(?!.*\.\.)(?!.*[.-]$)(?!^[-.])((?:25[0-5]|2[0-4][0-9]|1?[0-9]?[0-9])(?:\.(?:25[0-5]|2[0-4][0-9]|1?[0-9]?[0-9])){3}|\[?(?:[a-fA-F0-9]{1,4}:){7}[a-fA-F0-9]{1,4}\]?|([a-zA-Z\d][a-zA-Z\d.-]*[a-zA-Z\d]?))(?::([0-9]{1,5}))?$/;
