<template>
  <template v-if="inspection">
    ...
    <template v-else>
      <div v-if="topLevel.length" class="features" data-test="feature-selection">
        <div class="section-title">Features to start</div>
        <p v-if="!suggested.length" class="hint" data-test="feature-hint">
          This plugin offers several ways to run; tick the feature that matches this server, usually the standalone or core one.
        </p>
        <p v-else class="hint" data-test="feature-hint-suggested">
          The ticked features go into the boot file; untick or add others only if the plugin's documentation says so.
        </p>
        <div v-for="(feature, index) in topLevel" :key="feature.name" class="feature-row" data-test="feature-row">
          <OnmsCheckbox
            :modelValue="selected[feature.name] ?? false"
            :inputId="`plugin-feature-${index}`"
            data-test="feature-checkbox"
            @update:modelValue="(value: boolean) => setSelected(feature.name, value)"
          />
          <label :for="`plugin-feature-${index}`">
            <span class="feature-name" data-test="feature-name">{{ feature.name }}</span>
            <span v-if="feature.version" class="feature-version">{{ feature.version }}</span>
            <span v-if="feature.description" class="feature-description" data-test="feature-description">{{ feature.description }}</span>
          </label>
        </div>
      </div>
      <div v-if="hasWarnings" class="ack-row">
...
// server already holds, the checks, the choice of features, the
// acknowledgement and the load itself.
...
const topLevel = computed(() => (props.inspection?.features ?? []).filter(f => f.topLevel))
const suggested = computed(() => props.inspection?.suggestedFeatures ?? [])
// in declared order, so the boot file reads like the features file
const chosenFeatures = computed(() => topLevel.value.filter(f => selected.value[f.name]).map(f => f.name))

const canLoad = computed(() =>
  props.inspection !== null && !props.disabled && !installing.value && !hasFailures.value
    && chosenFeatures.value.length > 0 && (!hasWarnings.value || acknowledged.value))
...
const loadTitle = computed(() => {
  ...
  if (chosenFeatures.value.length === 0) {
    return 'Choose at least one feature to start'
  }
...
const setSelected = (name: string, value: boolean) => {
  selected.value = { ...selected.value, [name]: value }
}

const resetSelection = () => {
  const initial: Record<string, boolean> = {}
  for (const name of suggested.value) {
    initial[name] = true
  }
