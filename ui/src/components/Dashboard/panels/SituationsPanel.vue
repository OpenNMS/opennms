...
const describe = (s: Situation): string => {
  const parts: string[] = []
  if (s.affectedNodeCount != null) parts.push(`${s.affectedNodeCount} node${s.affectedNodeCount === 1 ? '' : 's'}`)
  if (s.situationAlarmCount != null) parts.push(`${s.situationAlarmCount} alarm${s.situationAlarmCount === 1 ? '' : 's'}`)
  return parts.join(', ')
}
