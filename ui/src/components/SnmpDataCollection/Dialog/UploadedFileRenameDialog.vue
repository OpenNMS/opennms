...
const props = defineProps<{
  visible: boolean,
  fileBucket: UploadSnmpDataCollectionFileType[],
  alreadyExistsNames: SnmpDataCollectionSourceNamesAndIds[],
  index: number
}>()
...
const validateName = () => {
  ...
  } else if (props.alreadyExistsNames.map(source => source.name.replace('.xml', '').toLowerCase()).includes(newFileName.value.trim().replace('.xml', '').toLowerCase())) {
