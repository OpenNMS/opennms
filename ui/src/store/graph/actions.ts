...
import { GraphMetricsPayload, GraphMetricsResponse, PreFabGraph, Resource, ResourceDefinitionsApiResponse, VuexContext } from '@/types'
...
const getGraphDefinitionsByResourceIds = async (context: ActionContext<State, any>, ids: string[]) => {
  ...
  const resources: Resource[] = JSON.parse(JSON.stringify(context.rootState.resourceModule.nodeResource.children.resource))
  ...
  for (const id in resourceAndPromises) {
    ...
    idsWithDefinitions = [...idsWithDefinitions, { id, definitions: uniqueSortedDefinitions, label: getLabelFromId(id) }]
  }
